...
PRODUCT_SIGNAL_ATTR_REJECT_REASONS = (
...
def materialize_product_signal_attr_crop_dataset_from_root(
    ...
    *,
    ...
    """Materialize new targets; sample limit counts raw source images per split."""
    report = discover_pairs(TRAFFIC_DATASET_KEY, Path(dataset_root))
    return materialize_aihub_signal_attr_crop_dataset(
        report.pairs,
        output_root,
        ...
        log_every=log_every,
        log_fn=log_fn,
        product_all_off_is_valid=all_off_is_valid,
    ...
    if total_items:
        if worker_count == 1:
            ...
            with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="signal_attr_dataset") as executor:
                for start in range(0, total_items, worker_count * 2):
                    future_to_scene = {
                        executor.submit(
                        ...
                        for scene_path in scene_paths[start:start + worker_count * 2]
                    }
...
def materialize_aihub_signal_attr_crop_dataset(
    ...
    *,
    ...
    log_every: int = 500,
    log_fn: Callable[[str], None] | None = None,
    product_all_off_is_valid: bool | None = None,
    max_samples_per_split: int | None = None,
) -> dict[str, Any]:
    output = Path(output_root)
    split_names = _normalize_splits(splits)
    ...
    if max_samples_per_split is not None:
        limit = _positive_int(max_samples_per_split, field_name="signal_attr.max_samples_per_split")
        seen_by_split: Counter[str] = Counter()
        selected_pairs: list[PairRecord] = []
        for pair in sorted_pairs:
            if seen_by_split[pair.split] < limit:
                selected_pairs.append(pair)
                seen_by_split[pair.split] += 1
        sorted_pairs = selected_pairs
        ...
        if worker_count == 1:
            for pair in sorted_pairs:
                label_rows, pair_rejections = _materialize_pair_rows(
                    ...
                    source_dataset=source_dataset,
                    crop_config=crop_config,
                    product_all_off_is_valid=product_all_off_is_valid,
                )
            ...
            with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="signal_attr_dataset") as executor:
                for start in range(0, total_items, worker_count * 2):
                    future_to_pair = {
                        ...
                        for pair in sorted_pairs[start:start + worker_count * 2]
                    }
    ...
    manifest = _build_manifest(
        ...
        version=PRODUCT_SIGNAL_ATTR_DATASET_VERSION if product_all_off_is_valid is not None else SIGNAL_ATTR_DATASET_VERSION,
        reject_reasons=PRODUCT_SIGNAL_ATTR_REJECT_REASONS if product_all_off_is_valid is not None else SIGNAL_ATTR_DATASET_REJECT_REASONS,
    )
    if product_all_off_is_valid is not None:
        manifest["state_semantics"] = "left_arrow"
        manifest["all_off_is_valid"] = bool(product_all_off_is_valid)
    write_json_sorted(paths.manifest_path, manifest)
    if log_fn is not None:
        elapsed = max(time.monotonic() - start_time, 1.0e-6)
        log_fn(
    ...
    *,
    ...
    with Image.open(pair.image_path) as image:
        ...
        for annotation_index, annotation in enumerate(extract_annotations(raw)):
            ...
            if product_all_off_is_valid is None:
                label = collapse_aihub_traffic_light_attr(annotation)
                valid = bool(label.tl_attr_valid)
                reason = label.collapse_reason
                row_base.update(_label_payload(label.tl_bits, label.base_color, label.arrow, reason))
            else:
                label = extract_product_signal_attr_target(
                    annotation, all_off_is_valid=product_all_off_is_valid
                )
                valid = label.state_valid
                reason = label.reason
                row_base.update(_label_payload(label.tl_bits, label.base_color, label.left_arrow, reason))
                row_base["light_type"] = label.light_type
                row_base["left_arrow"] = label.left_arrow
            ...
            if not valid:
                rejected_rows.append({**row_base, "reject_reason": reason})
                continue
...
def _build_manifest(
    ...
    *,
    ...
    version: str = SIGNAL_ATTR_DATASET_VERSION,
    reject_reasons: Sequence[str] = SIGNAL_ATTR_DATASET_REJECT_REASONS,
) -> dict[str, Any]:
    ...
    reject_reason_counts = {reason: 0 for reason in reject_reasons}
    reject_reason_counts_by_split = {
        split: {reason: 0 for reason in reject_reasons} for split in splits
    }
    ...
    return {
        ...
        "closed_reject_reasons": list(reject_reasons),
...
__all__ = [
    "PRODUCT_SIGNAL_ATTR_DATASET_VERSION",
    "PRODUCT_SIGNAL_ATTR_REJECT_REASONS",
    "SIGNAL_ATTR_DATASET_REJECT_REASONS",
