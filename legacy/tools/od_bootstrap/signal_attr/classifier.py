...
class SignalAttrCropTorchDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        label_path = (Path(labels_root) if labels_root is not None else self.dataset_root / "labels") / f"{self.split}.jsonl"
...
def signal_attr_collate(batch: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "arrow_target_valid": torch.tensor(
            [float(item.get("arrow_target_valid", 1.0)) for item in batch], dtype=torch.float32
        ),
...
def signal_attr_loss(
    ...
    *,
    ...
    arrow_loss_values = nn.functional.binary_cross_entropy_with_logits(
        outputs["arrow_logit"], batch["arrow_target"], reduction="none"
    )
    arrow_mask = batch.get("arrow_target_valid")
    if arrow_mask is None:
        arrow_loss = arrow_loss_values.mean()
    else:
        arrow_loss = (arrow_loss_values * arrow_mask).sum() / arrow_mask.sum().clamp_min(1.0)
    ...
    *,
    ...
    if light_type == "pedestrian":
        prediction = signal_attr_prediction_from_logits(
            base_color_logits,
            arrow_logit,
            policy=replace(policy, arrow_ambiguity_band=0.0),
        )
        ...
        return replace(
            prediction,
            tl_bits={bit: int(bit == prediction.base_color) for bit in TL_BITS},
            arrow=0,
        )
    if light_type != "car":
...
def train_signal_attr_classifier(
    ...
    *,
    ...
    label_semantics = _load_label_semantics(Path(dataset_root))
...
def evaluate_signal_attr_checkpoint(
    ...
    *,
    ...
    product_mode = loaded["payload"].get("state_semantics") == "left_arrow"
    ...
    by_light_type: dict[str, dict[str, int]] = {}
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for row_index, row in enumerate(batch["rows"]):
                light_type = str(row.get("light_type") or "car")
                base_logits = outputs["base_color_logits"][row_index].detach().cpu()
                arrow_logit = outputs["arrow_logit"][row_index].detach().cpu()
                if product_mode:
                    ...
                    prediction = signal_attr_prediction_from_logits(base_logits, arrow_logit, policy=policy)
                ...
                arrow_scored = not product_mode or light_type == "car"
                combo_hit = bool(prediction.tl_attr_valid and pred_bits == target_bits)
                base_correct += int(base_hit)
                arrow_correct += int(arrow_scored and arrow_hit)
                arrow_sample_count += int(arrow_scored)
                combo_correct += int(combo_hit)
                sample_count += 1
                type_stats = by_light_type.setdefault(
                    light_type, {"sample_count": 0, "base_correct": 0, "arrow_correct": 0, "arrow_count": 0, "combo_correct": 0, "valid_count": 0}
                )
                type_stats["sample_count"] += 1
                type_stats["base_correct"] += int(base_hit)
                type_stats["arrow_correct"] += int(arrow_scored and arrow_hit)
                type_stats["arrow_count"] += int(arrow_scored)
                type_stats["combo_correct"] += int(combo_hit)
                type_stats["valid_count"] += int(prediction.tl_attr_valid)
                for bit in TL_BITS:
                    if bit == "arrow" and not arrow_scored:
                        continue
                ...
                prediction_rows.append(
                    {
                        ...
                        "base_color": prediction.base_color,
                        "base_color_confidence": prediction.base_color_confidence,
                        "arrow_probability": prediction.arrow_probability if arrow_scored else None,
                        "target_base_color": row.get("base_color"),
                        "target_arrow": int(row.get("arrow", 0)) if arrow_scored else None,
                    }
                )
            completed_batches += 1
            _log_progress(
    ...
    report = {
        ...
        "split": str(split),
        "state_semantics": "left_arrow" if product_mode else "legacy_arrow",
        "sample_count": sample_count,
        "threshold_policy": asdict(policy),
        "crop_config": crop_config,
        "base_color_accuracy": base_correct / max(sample_count, 1),
        "arrow_accuracy": arrow_correct / max(arrow_sample_count, 1),
        "combo_accuracy": combo_correct / max(sample_count, 1),
        "by_light_type": {
            light_type: {
                "sample_count": stats["sample_count"],
                "valid_count": stats["valid_count"],
                "base_color_accuracy": stats["base_correct"] / max(stats["sample_count"], 1),
                "arrow_accuracy": stats["arrow_correct"] / max(stats["arrow_count"], 1) if stats["arrow_count"] else None,
                "combo_accuracy": stats["combo_correct"] / max(stats["sample_count"], 1),
            }
            for light_type, stats in sorted(by_light_type.items())
        },
...
def evaluate_signal_attr_classifier(
    ...
    *,
    ...
    """Evaluate once; product macro-F1 includes truth or predicted positives, not TN-only states."""
    model.eval()
    sample_count = 0
    arrow_sample_count = 0
    base_correct = 0
    ...
    state_counts = {
        light_type: {state: {"tp": 0, "fp": 0, "fn": 0, "tn": 0} for state in states}
        for light_type, states in product_states.items()
    }
    type_counts = {light_type: {"sample_count": 0, "valid_count": 0} for light_type in product_states}
    for batch in loader:
        moved = _move_batch(batch, device)
        outputs = model(moved["image"])
        ...
        arrow_mask = moved["arrow_target_valid"].to(dtype=torch.bool)
        batch_size = int(base_target.numel())
        sample_count += batch_size
        arrow_sample_count += int(arrow_mask.sum().item())
        base_hits = base_pred == base_target
        arrow_hits = arrow_pred == arrow_target
        base_correct += int(base_hits.sum().item())
        arrow_correct += int((arrow_hits & arrow_mask).sum().item())
        combo_correct += int((base_hits & (~arrow_mask | arrow_hits)).sum().item())
        if product_semantics:
            for index, row in enumerate(batch["rows"]):
                light_type = str(row["light_type"])
                prediction = product_signal_attr_prediction_from_logits(
                    outputs["base_color_logits"][index],
                    outputs["arrow_logit"][index],
                    light_type=light_type,
                    all_off_is_valid=all_off_is_valid,
                    policy=threshold_policy,
                )
                type_counts[light_type]["sample_count"] += 1
                type_counts[light_type]["valid_count"] += int(prediction.tl_attr_valid)
                for state, bit in product_states[light_type].items():
                    truth = int(row["arrow"]) if state == "left_arrow" else int(row["base_color"] == state)
                    ...
                    state_counts[light_type][state][key] += 1
    report: dict[str, Any] = {
        "sample_count": sample_count,
        "base_color_accuracy": base_correct / max(sample_count, 1),
        "arrow_accuracy": arrow_correct / max(arrow_sample_count, 1),
    ...
    if product_semantics:
        ...
        for light_type, counts in type_counts.items():
            states = {state: _binary_metrics(state_counts[light_type][state]) for state in product_states[light_type]}
            supported_f1.extend(
                float(metrics["f1"])
                for metrics in states.values()
                if int(metrics["tp"]) + int(metrics["fp"]) + int(metrics["fn"]) > 0
            )
            by_light_type[light_type] = {
                **counts,
                "valid_coverage": counts["valid_count"] / max(counts["sample_count"], 1),
                "states": states,
            }
        report.update({
            "valid_coverage": sum(counts["valid_count"] for counts in type_counts.values()) / max(sample_count, 1),
            "macro_state_f1": sum(supported_f1) / len(supported_f1) if supported_f1 else 0.0,
...
def _move_batch(batch: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    return {
        ...
        "arrow_target_valid": batch["arrow_target_valid"].to(device=device),
        "rows": batch.get("rows", []),
    }
...
def _load_label_semantics(dataset_root: Path) -> dict[str, Any]:
    manifest_path = dataset_root / "meta" / "signal_attr_dataset_manifest.json"
    if not manifest_path.is_file():
        return {"state_semantics": "legacy_arrow", "all_off_is_valid": True}
    manifest = read_json(manifest_path)
    if not isinstance(manifest, Mapping):
        raise TypeError(f"signal attr dataset manifest must be an object: {manifest_path}")
    return {
        "state_semantics": str(manifest.get("state_semantics") or "legacy_arrow"),
        "all_off_is_valid": bool(manifest.get("all_off_is_valid", True)),
    }
