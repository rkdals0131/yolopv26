...
class SignalAttrDatasetMaterializationTests(unittest.TestCase):
    def test_product_crop_rows_preserve_type_and_left_arrow_semantics(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            pair = _write_pair(
                ...
                annotations=[
                    _traffic_light(
                        [10, 10, 30, 40],
                        attribute={"red": "on", "yellow": "off", "green": "off", "left_arrow": "off", "others_arrow": "on"},
                    ),
                    _traffic_light(
                        [40, 10, 60, 40],
                        light_type="pedestrian",
                        attribute={"red": "off", "green": "on"},
                    ),
                    _traffic_light(
                        [10, 10, 30, 40],
                        attribute={"red": "off", "yellow": "off", "green": "off", "left_arrow": "off", "others_arrow": "on"},
                    ),
                    _traffic_light([10, 10, 30, 40]),
                ],
            )
            later_pair = _write_pair(
                root / "raw", split="train", relative_id="zz_later",
                annotations=[_traffic_light(
                    [10, 10, 30, 40],
                    attribute={"red": "on", "yellow": "off", "green": "off", "left_arrow": "off"},
                )],
            )
            output = root / "product"
            manifest = materialize_aihub_signal_attr_crop_dataset(
                [pair, later_pair], output, product_all_off_is_valid=False,
                max_samples_per_split=1,
            )
            rows = read_jsonl(output / "labels" / "train.jsonl")
            rejected = read_jsonl(output / "meta" / "rejected_rows.jsonl")
            self.assertEqual(manifest["state_semantics"], "left_arrow")
            self.assertEqual([(row["light_type"], row["base_color"], row["arrow"]) for row in rows],
                             [("car", "red", 0), ("pedestrian", "green", 0), ("car", "off", 0)])
            self.assertEqual([row["reject_reason"] for row in rejected], ["all_off_unverified"])
