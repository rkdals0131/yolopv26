"""Lifetime of one writer for a durable training run."""
...
def training_run_lock(run_dir: str | Path) -> Iterator[None]:
    """Hold an exclusive, nonblocking lock until the run writer exits.

    The lock file remains in place: removing it could create two independent
    inodes and let concurrent processes each believe they own the same run.
    """
    directory = Path(run_dir)
    directory.mkdir(parents=True, exist_ok=True)
    lock_path = directory / ".train.lock"
