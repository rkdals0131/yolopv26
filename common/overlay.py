...
def _append_polyline(command: list[str], points: list[list[float]], color: str) -> None:
    if len(points) < 2:
        return
    command.extend(["-fill", "none", "-stroke", color, "-strokewidth", "3", "-draw", f"polyline {_format_points(points)}"])


def _append_polygon(command: list[str], points: list[list[float]], color: str) -> None:
    if len(points) < 3:
        return
    command.extend(["-fill", "none", "-stroke", color, "-strokewidth", "2", "-draw", f"polygon {_format_points(points)}"])
...
def _append_rectangle(command: list[str], bbox: list[float], color: str) -> None:
    if len(bbox) != 4:
        return
    x1, y1, x2, y2 = bbox
    command.extend(["-fill", "none", "-stroke", color, "-strokewidth", "3", "-draw", f"rectangle {x1},{y1} {x2},{y2}"])
