...
def _lane_row_x_lookup(points: torch.Tensor, *, output_hw: tuple[int, int]) -> dict[int, float]:
    h, w = int(output_hw[0]), int(output_hw[1])
    if points.shape[0] < 2:
        return {}
    values = points.detach().cpu().numpy().astype(np.float32)
    ...
    for start, end in zip(values[:-1], values[1:]):
        ...
        if abs(y1 - y0) <= 1.0e-6:
            continue
        lo = max(0, int(np.ceil(min(y0, y1))))
        hi = min(h - 1, int(np.floor(max(y0, y1))))
        for row_index in range(lo, hi + 1):
            t = (float(row_index) - y0) / (y1 - y0)
            if t < -1.0e-6 or t > 1.0 + 1.0e-6:
                continue
            x_value = x0 + t * (x1 - x0)
            if not np.isfinite(x_value):
                continue
            row_values.setdefault(int(row_index), []).append(float(np.clip(x_value, 0.0, float(w - 1))))
    return {row: float(np.median(values)) for row, values in row_values.items() if values}


def _render_row_link_targets(
    lane_rows: list[dict[str, Any]],
    valid_mask: torch.BoolTensor | list[bool] | tuple[bool, ...],
    *,
    output_hw: tuple[int, int],
    ...
    h, w = int(output_hw[0]), int(output_hw[1])
    row_link_delta = np.zeros((1, h, w), dtype=np.float32)
    row_link_valid = np.zeros((1, h, w), dtype=np.float32)
    row_link_written = np.zeros((h, w), dtype=bool)
    ...
    for lane_index, row in enumerate(lane_rows):
        ...
        for row_index, x_value in lookup.items():
            next_row = int(row_index) - 1
            if row_index <= 0 or next_row not in lookup:
                continue
            col_index = int(np.clip(round(float(x_value)), 0, w - 1))
            delta = float(lookup[next_row]) - float(x_value)
            if max_delta > 0.0:
                delta = float(np.clip(delta, -max_delta, max_delta))
            if row_link_written[row_index, col_index] and abs(float(row_link_delta[0, row_index, col_index]) - delta) > 1.0:
                row_link_valid[0, row_index, col_index] = 0.0
                continue
            row_link_delta[0, row_index, col_index] = delta
            row_link_valid[0, row_index, col_index] = 1.0
            row_link_written[row_index, col_index] = True
...
def render_lane_segfirst_targets(
    ...
    *,
    ...
    row_link_delta, row_link_valid = _render_row_link_targets(
        lane_rows,
        lane_valid_mask,
        output_hw=cfg.output_hw,
        max_delta_px=cfg.row_link_max_delta_px,
    ) if source_enabled else (
...
def _row_link_at(row_link_delta: np.ndarray, *, row: int, col: float) -> float:
    if row_link_delta.ndim != 2:
        return 0.0
    h, w = int(row_link_delta.shape[0]), int(row_link_delta.shape[1])
    row_index = min(max(int(row), 0), h - 1)
    col_index = min(max(int(round(float(col))), 0), w - 1)
    value = float(row_link_delta[row_index, col_index])
    ...
    for y in sorted(np.unique(ys).tolist(), reverse=True):
        ...
        for x in sorted(clusters):
            ...
            for track_index, (last_y, last_x) in enumerate(zip(last_y_by_track, last_x_by_track)):
                ...
                if dx > float(max_link_dx):
                    continue
                link_delta = _row_link_at(row_link_delta, row=int(last_y), col=float(last_x))
                expected_dx = float(np.clip(link_delta * max(1, y_gap), -float(max_link_dx), float(max_link_dx)))
                expected_x = float(last_x) + expected_dx
                link_cost = abs(float(x) - expected_x)
                if link_cost > float(max_link_dx):
                    continue
                cost = 0.85 * link_cost + 0.15 * dx + 0.1 * float(y_gap)
                if cost < best_cost:
                    best_track = track_index
                    best_cost = cost
            if best_track is None:
...
def _trace_seed_direction(
    centerline: np.ndarray,
    tangent_axis: np.ndarray | None,
    *,
    start_row: int,
    start_col: int,
    direction: int,
    ...
    height = int(centerline.shape[0])
    width = int(centerline.shape[1])
    row_stride = max(1, int(cfg.row_stride))
    step = row_stride if int(direction) > 0 else -row_stride
    stop_row = height if step > 0 else -1
    search_radius = max(1, int(round(float(cfg.max_link_dx))))
    max_gap = max(row_stride, int(cfg.max_row_gap))
    current_x = float(start_col)
    current_row = int(start_row)
    gap = 0
    points: list[list[float]] = []
    for row_index in range(int(start_row), stop_row, step):
        expected_x = current_x
        if row_index != current_row and tangent_axis is not None:
            tangent = _tangent_at(tangent_axis, row=current_row, col=current_x)
            if tangent is not None and abs(float(tangent[1])) >= 0.05:
                dy = float(row_index) - float(current_row)
                expected_dx = float(tangent[0]) / float(tangent[1]) * dy
                expected_dx = float(np.clip(expected_dx, -float(search_radius), float(search_radius)))
                expected_x = current_x + expected_dx
        center_col = int(round(expected_x))
        start = max(0, center_col - search_radius)
        end = min(width, center_col + search_radius + 1)
        if start >= end:
            break
        row_values = centerline[int(row_index), start:end]
        valid_cols = np.nonzero(row_values >= float(cfg.centerline_threshold))[0]
        if valid_cols.size == 0:
            gap += row_stride
            if gap > max_gap:
                break
            continue
        absolute_cols = valid_cols + start
        distances = np.abs(absolute_cols.astype(np.float32) - float(expected_x))
        scores = row_values[valid_cols].astype(np.float32) - 0.02 * distances
        best_local = int(valid_cols[int(np.argmax(scores))])
        current_x = float(start + best_local)
...
def _bidirectional_seed_trace_polylines(
    centerline: np.ndarray,
    seed_map: np.ndarray,
    tangent_axis: np.ndarray | None,
    *,
    ...
    if centerline.shape != seed_map.shape:
        return []
    local_max = seed_map >= ndimage.maximum_filter(seed_map, size=3, mode="nearest")
    seed_mask = local_max & (seed_map >= float(cfg.seed_threshold)) & (centerline >= float(cfg.centerline_threshold))
    seed_rows, seed_cols = np.nonzero(seed_mask)
    if seed_rows.size == 0:
        return []
    seed_scores = seed_map[seed_rows, seed_cols]
    ...
    for seed_index in order.tolist():
        seed_row = int(seed_rows[seed_index])
        seed_col = int(seed_cols[seed_index])
        up_points = _trace_seed_direction(
            centerline,
            tangent_axis,
            start_row=seed_row,
            start_col=seed_col,
        ...
        down_points = _trace_seed_direction(
            centerline,
            tangent_axis,
            start_row=seed_row + max(1, int(cfg.row_stride)),
            start_col=seed_col,
            direction=1,
            cfg=cfg,
        )
        points_by_row: dict[int, list[float]] = {}
        for x_value, y_value in down_points + up_points:
            points_by_row[int(round(float(y_value)))] = [float(x_value), float(y_value)]
        if len(points_by_row) < int(cfg.min_polyline_points):
            continue
        ordered = [points_by_row[row] for row in sorted(points_by_row.keys(), reverse=True)]
...
def vectorize_lane_segfirst_maps(
    ...
    *,
    ...
    row_bidirectional_seed_trace_modes = {
        "row_scan_bidirectional_seed_trace",
    ...
    if track_mode in seed_trace_only_modes:
        ...
        return predictions
    if track_mode in bidirectional_seed_trace_only_modes:
        use_tangent = tangent_axis if track_mode == "bidirectional_seed_trace" else None
        for map_points in _bidirectional_seed_trace_polylines(centerline, seed_map, use_tangent, cfg=cfg):
            if cfg.max_predictions is not None and len(predictions) >= int(cfg.max_predictions):
                break
            if len(map_points) < int(cfg.min_polyline_points):
                continue
    ...
    if track_mode in {
        ...
        *row_bidirectional_seed_trace_modes,
        ...
        if (
            track_mode in row_tangent_modes
            or track_mode == "row_scan_tangent_seed_trace"
            or track_mode == "row_scan_tangent_bidirectional_seed_trace"
        ):
        ...
        elif track_mode in row_link_modes:
            map_polylines = _component_to_polylines_row_link(
                binary,
                row_link_delta,
                row_stride=max(1, int(cfg.row_stride)),
                max_row_gap=max(1, int(cfg.max_row_gap)),
        ...
        if track_mode in row_bidirectional_seed_trace_modes:
            bidir_tangent = tangent_axis if track_mode == "row_scan_tangent_bidirectional_seed_trace" else None
            for map_points in _bidirectional_seed_trace_polylines(centerline, seed_map, bidir_tangent, cfg=cfg):
                if cfg.max_predictions is not None and len(predictions) >= int(cfg.max_predictions):
                    break
                if len(map_points) < int(cfg.min_polyline_points):
                    continue
