...
def _lane_segfirst_loss(
    ...
    *,
    ...
    anchor_offset_aux_weight: float = 0.0,
    row_link_aux_weight: float = 0.0,
    ...
    centerline_core = aux["lane_seg_centerline_core"].to(device=centerline_logits.device, dtype=centerline_logits.dtype)
    ...
    centerline_target_mode = str(centerline_target_mode).strip().lower()
    if centerline_target_mode == "soft":
        centerline_target = centerline_soft
    elif centerline_target_mode == "core":
        centerline_target = centerline_core
    elif centerline_target_mode == "hybrid":
        centerline_target = torch.maximum(centerline_core, centerline_soft * 0.5)
    else:
        raise ValueError(f"unsupported lane_segfirst_centerline_target_mode: {centerline_target_mode}")
    ...
    anchor_offset_loss = _zero_graph(centerline_logits)
    row_link_loss = _zero_graph(centerline_logits)
    ...
    if float(anchor_offset_aux_weight) > 0.0:
        anchor_target = aux.get("lane_seg_anchor_offset")
        anchor_valid = aux.get("lane_seg_anchor_offset_valid")
        if not isinstance(anchor_offset, torch.Tensor):
            raise KeyError("anchor-offset aux loss requires lane_seg_anchor_offset prediction")
        if isinstance(anchor_target, torch.Tensor) and isinstance(anchor_valid, torch.Tensor):
            anchor_target = anchor_target.to(device=anchor_offset.device, dtype=anchor_offset.dtype)
            anchor_valid = anchor_valid.to(device=anchor_offset.device, dtype=torch.bool)
            anchor_ignore = ignore.to(device=anchor_offset.device, dtype=torch.bool)
            anchor_mask = (
                source.to(device=anchor_offset.device, dtype=torch.bool)[:, None, None, None]
                & anchor_valid
                & (~anchor_ignore)
            )
            anchor_mask = anchor_mask.expand_as(anchor_offset)
            if bool(anchor_mask.any()):
                anchor_offset_loss = F.smooth_l1_loss(
                    anchor_offset[anchor_mask],
                    anchor_target[anchor_mask],
                    reduction="mean",
                    beta=1.0,
                )
    if float(row_link_aux_weight) > 0.0:
        link_target = aux.get("lane_seg_row_link_delta")
        link_valid = aux.get("lane_seg_row_link_valid")
        if not isinstance(row_link_delta, torch.Tensor):
            raise KeyError("row-link aux loss requires lane_seg_row_link_delta prediction")
        if isinstance(link_target, torch.Tensor) and isinstance(link_valid, torch.Tensor):
            link_target = link_target.to(device=row_link_delta.device, dtype=row_link_delta.dtype)
            link_valid = link_valid.to(device=row_link_delta.device, dtype=torch.bool)
            link_ignore = ignore.to(device=row_link_delta.device, dtype=torch.bool)
            link_mask = (
                source.to(device=row_link_delta.device, dtype=torch.bool)[:, None, None, None]
                & link_valid
                & (~link_ignore)
            )
            link_mask = link_mask.expand_as(row_link_delta)
            if bool(link_mask.any()):
                row_link_loss = F.smooth_l1_loss(
                    row_link_delta[link_mask],
                    link_target[link_mask],
                    reduction="mean",
                    beta=1.0,
                )
    if float(task_conflict_negative_weight) > 0.0:
        ...
        include_cross = mode in {"crosswalk", "stop_line_crosswalk", "stopline_crosswalk", "both", "all", "task_masks"}
        if not include_stop and not include_cross:
            raise ValueError(f"unsupported lane_segfirst_task_conflict_negative_mode: {task_conflict_negative_mode}")
        conflict_mask = torch.zeros_like(ignore, dtype=torch.bool, device=centerline_logits.device)
        if include_stop:
            stop_mask = aux.get("lane_seg_stop_line_ignore")
            if isinstance(stop_mask, torch.Tensor):
                conflict_mask = conflict_mask | stop_mask.to(device=centerline_logits.device, dtype=torch.bool)
        if include_cross:
            cross_mask = aux.get("lane_seg_crosswalk_ignore")
            if isinstance(cross_mask, torch.Tensor):
                conflict_mask = conflict_mask | cross_mask.to(device=centerline_logits.device, dtype=torch.bool)
        conflict_mask = (
            source[:, None, None, None].expand_as(centerline_logits)
            & conflict_mask.expand_as(centerline_logits)
            & (centerline_core <= 0.5)
        )
        if bool(conflict_mask.any()):
            conflict_prob = centerline_logits.sigmoid()
    ...
    if float(conditional_row_aux_weight) > 0.0:
        conditional_row_loss = _lane_conditional_row_loss(
            predictions,
            encoded,
    ...
    if float(conditional_seed_aux_weight) > 0.0:
        conditional_seed_loss = _lane_conditional_seed_loss(
            predictions,
            encoded,
            seed_target_mode=str(conditional_seed_target_mode),
            target_source=str(conditional_target_source),
    ...
    total = (
        ...
        + float(anchor_offset_aux_weight) * anchor_offset_loss
        + float(row_link_aux_weight) * row_link_loss
        ...
        breakdown.update(
            {
                ...
                "seg_anchor_offset": anchor_offset_loss,
                "seg_row_link": row_link_loss,
...
def _lane_conditional_bottom_anchor_seed_target(
    seed_logits: torch.Tensor,
    lane_target: torch.Tensor,
    lane_valid: torch.Tensor,
    lane_source: torch.Tensor,
    ignore: torch.Tensor | None,
) -> tuple[torch.Tensor, torch.Tensor]:
    batch_size, _, height, width = seed_logits.shape
    target = torch.zeros_like(seed_logits)
    valid_mask = lane_source.to(device=seed_logits.device, dtype=torch.bool)[:, None, None, None].expand_as(seed_logits).clone()
    if isinstance(ignore, torch.Tensor):
        ignore_mask = F.interpolate(
            ignore.to(device=seed_logits.device, dtype=seed_logits.dtype),
            size=(height, width),
            mode="nearest",
        ).to(dtype=torch.bool)
        valid_mask = valid_mask & (~ignore_mask.expand_as(seed_logits))
    ...
    for batch_index in range(batch_size):
        ...
        for gt_row in gt_rows:
            ...
            col = int(torch.round(x_value * x_scale).clamp(0, width - 1).item())
            row = int(torch.round(y_value * y_scale).clamp(0, height - 1).item())
            target[batch_index, 0, row, col] = 1.0
            valid_mask[batch_index, 0, row, col] = True
    return target, valid_mask


def _lane_conditional_row_loss(
    predictions: dict[str, torch.Tensor],
    encoded: dict[str, Any],
    *,
    ...
    for batch_index in range(batch_size):
        ...
        if int(pred_indices.numel()) == 0:
            continue
        matched_pred = conditional_rows[batch_index, pred_indices]
        matched_gt = gt_rows[gt_indices]
        if objectness_target_mode == "metric_quality":
            obj_target[batch_index, pred_indices] = _lane_match_quality(matched_pred, matched_gt).detach()
        else:
            obj_target[batch_index, pred_indices] = 1.0
        matched_pred_rows.append(matched_pred)
        matched_gt_rows.append(matched_gt)
    ...
    if isinstance(seed_logits, torch.Tensor) and isinstance(aux, dict):
        ...
        if seed_target_mode == "centerline_core":
        ...
        elif seed_target_mode == "bottom_anchor":
    ...
    return obj_loss + 0.25 * seed_loss + float(row_x_weight) * x_loss + vis_loss + color_loss + 0.5 * type_loss


def _lane_conditional_seed_loss(
    predictions: dict[str, torch.Tensor],
    encoded: dict[str, Any],
    *,
    seed_target_mode: str = "centerline_core",
...
def _stop_line_segment_set_loss(
    *,
    ...
    if segment_logits.ndim != 2 or segment_points.ndim != 4 or segment_points.shape[2:] != (2, 2):
    ...
    denom = segment_points.new_tensor([float(NETWORK_HW[1]), float(NETWORK_HW[0])]).view(1, 1, 2)
    batch_losses: list[torch.Tensor] = []
    for batch_index in range(int(segment_logits.shape[0])):
        if not bool(stop_source[batch_index]):
            continue
        logits_b = segment_logits[batch_index]
        points_b = segment_points[batch_index].clamp(0.0, 1.0)
        valid_b = stop_valid[batch_index]
        ...
        if valid_count > 0:
            ...
            target_segments = (target_segments / denom).clamp(0.0, 1.0)
            with torch.no_grad():
                pred_detached = points_b.detach()
                direct_cost = (pred_detached[:, None, :, :] - target_segments[None, :, :, :]).abs().mean(dim=(-1, -2))
                flipped_targets = target_segments[:, [1, 0], :]
                flipped_cost = (pred_detached[:, None, :, :] - flipped_targets[None, :, :, :]).abs().mean(dim=(-1, -2))
                cost = torch.minimum(direct_cost, flipped_cost).detach().cpu().numpy()
                query_indices_np, target_indices_np = linear_sum_assignment(cost)
            if len(query_indices_np) > 0:
                query_indices = torch.as_tensor(query_indices_np, device=device, dtype=torch.long)
                target_indices = torch.as_tensor(target_indices_np, device=device, dtype=torch.long)
                objectness_target[query_indices] = 1.0
                matched_pred = points_b[query_indices]
                matched_direct = target_segments[target_indices]
                matched_flipped = matched_direct[:, [1, 0], :]
                direct_distance = (matched_pred.detach() - matched_direct).abs().mean(dim=(-1, -2))
                flipped_distance = (matched_pred.detach() - matched_flipped).abs().mean(dim=(-1, -2))
    ...
    seed_loss = _zero_graph(segment_logits)
    seed_target = aux.get("stop_line_center_heatmap")
    if isinstance(segment_seed_logits, torch.Tensor) and isinstance(seed_target, torch.Tensor):
        seed_target = seed_target.to(device=segment_seed_logits.device, dtype=segment_seed_logits.dtype)
        seed_source = stop_source[:, None, None, None].to(device=segment_seed_logits.device).expand_as(segment_seed_logits)
        seed_loss = _masked_binary_ce_balanced(
            segment_seed_logits,
            seed_target,
            seed_source,
...
def _stop_line_mask_loss_with_selector_weight(
    ...
    *,
    ...
    context_segment_set_aux_weight: float = 0.0,
    context_segment_verifier_aux_weight: float = 0.0,
    ...
    context_segment_seed_logits = predictions.get("stop_line_context_segment_seed_logits")
    context_segment_logits = predictions.get("stop_line_context_segment_logits")
    context_segment_points = predictions.get("stop_line_context_segment_points")
    context_segment_verifier_logits = predictions.get("stop_line_context_segment_verifier_logits")
    ...
    patch_segment_seed_logits = predictions.get("stop_line_patch_segment_seed_logits")
    patch_segment_logits = predictions.get("stop_line_patch_segment_logits")
    patch_segment_points = predictions.get("stop_line_patch_segment_points")
    patch_segment_verifier_logits = predictions.get("stop_line_patch_segment_verifier_logits")
    ...
    if float(patch_segment_set_aux_weight) > 0.0:
        patch_segment_set_loss = _stop_line_segment_set_loss(
            ...
            segment_seed_logits=patch_segment_seed_logits,
            aux=aux,
            encoded=encoded,
            fallback_tensor=mask_logits,
    ...
    context_segment_set_loss = _zero_graph(mask_logits)
    if float(context_segment_set_aux_weight) > 0.0:
        context_segment_set_loss = _stop_line_segment_set_loss(
            ...
            segment_seed_logits=context_segment_seed_logits,
            aux=aux,
            encoded=encoded,
            fallback_tensor=mask_logits,
    ...
    return (
        ...
        * (
        ...
        + float(patch_segment_set_aux_weight) * patch_segment_set_loss
        + float(segment_denoise_aux_weight) * segment_denoise_loss
        + float(context_segment_set_aux_weight) * context_segment_set_loss
    )
...
class PV26MultiTaskLoss(nn.Module):
    def __init__(
        ...
        *,
        ...
        lane_segfirst_row_link_aux_weight: float = 0.0,
        ...
        stopline_context_segment_set_aux_weight: float = 0.0,
        stopline_context_segment_verifier_aux_weight: float = 0.0,
        ...
        self.lane_segfirst_anchor_offset_aux_weight = float(lane_segfirst_anchor_offset_aux_weight)
        if self.lane_segfirst_anchor_offset_aux_weight < 0.0:
            raise ValueError("lane_segfirst_anchor_offset_aux_weight must be >= 0")
        self.lane_segfirst_row_link_aux_weight = float(lane_segfirst_row_link_aux_weight)
        if self.lane_segfirst_row_link_aux_weight < 0.0:
            raise ValueError("lane_segfirst_row_link_aux_weight must be >= 0")
        ...
        self.lane_conditional_seed_aux_weight = float(lane_conditional_seed_aux_weight)
        ...
        if self.lane_conditional_objectness_target_mode not in {"binary", "metric_quality"}:
            raise ValueError(
                "unsupported lane_conditional_objectness_target_mode: "
                f"{self.lane_conditional_objectness_target_mode}"
            )
        ...
        self.stopline_context_segment_set_aux_weight = float(stopline_context_segment_set_aux_weight)
        self.stopline_context_segment_verifier_aux_weight = float(stopline_context_segment_verifier_aux_weight)
        ...
        self.stopline_patch_segment_set_aux_weight = float(stopline_patch_segment_set_aux_weight)
        self.stopline_patch_segment_verifier_aux_weight = float(stopline_patch_segment_verifier_aux_weight)
    ...
    def export_config(self) -> dict[str, Any]:
        return {
            ...
            "lane_segfirst_anchor_offset_aux_weight": float(self.lane_segfirst_anchor_offset_aux_weight),
            "lane_segfirst_row_link_aux_weight": float(self.lane_segfirst_row_link_aux_weight),
            ...
            "lane_conditional_seed_aux_weight": float(self.lane_conditional_seed_aux_weight),
            ...
            "stopline_context_segment_set_aux_weight": float(self.stopline_context_segment_set_aux_weight),
            "stopline_context_segment_verifier_aux_weight": float(self.stopline_context_segment_verifier_aux_weight),
    ...
    def _lane_loss(self, predictions: dict[str, torch.Tensor] | torch.Tensor, encoded: dict[str, Any]) -> torch.Tensor:
        ...
        if "lane_seg_centerline_logits" in prediction_dict:
            ...
            lane_loss = _lane_segfirst_loss(
                ...
                anchor_offset_aux_weight=self.lane_segfirst_anchor_offset_aux_weight,
                row_link_aux_weight=self.lane_segfirst_row_link_aux_weight,
    ...
    def _stop_line_loss(self, predictions: dict[str, torch.Tensor] | torch.Tensor, encoded: dict[str, Any]) -> torch.Tensor:
        ...
        if self.task_mode in {STOPLINE_ONLY_TASK_MODE, ROADMARK_JOINT_TASK_MODE} and "stop_line_mask_logits" in prediction_dict:
            ...
            if (
                ...
                and float(self.stopline_context_segment_set_aux_weight) == 0.0
                and float(self.stopline_context_segment_verifier_aux_weight) == 0.0
            ...
            return _stop_line_mask_loss_with_selector_weight(
                ...
                context_segment_set_aux_weight=float(self.stopline_context_segment_set_aux_weight),
                context_segment_verifier_aux_weight=float(self.stopline_context_segment_verifier_aux_weight),
