...
ROADMARK_ARCHITECTURES = (
    ...
    "current_family_dense_seed_geometry_refine_denoise_sigmoid",
)
...
def train_defaults_from_mapping(payload: dict[str, Any]) -> TrainDefaultsConfig:
    ...
    return TrainDefaultsConfig(
        ...
        lane_segfirst_row_link_aux_weight=_coerce_float(
            data.get("lane_segfirst_row_link_aux_weight", defaults.lane_segfirst_row_link_aux_weight),
            field_name="train_defaults.lane_segfirst_row_link_aux_weight",
        ),
        ...
        lane_conditional_denoise_hard_negative_count=_coerce_int(
            data.get(
                "lane_conditional_denoise_hard_negative_count",
                defaults.lane_conditional_denoise_hard_negative_count,
            ),
            field_name="train_defaults.lane_conditional_denoise_hard_negative_count",
        ),
        lane_conditional_denoise_hard_negative_offset_px=_coerce_float(
            data.get(
                "lane_conditional_denoise_hard_negative_offset_px",
                defaults.lane_conditional_denoise_hard_negative_offset_px,
            ),
            field_name="train_defaults.lane_conditional_denoise_hard_negative_offset_px",
        ),
        ...
        lane_family_cross_stitch_enabled=_coerce_bool(
            data.get("lane_family_cross_stitch_enabled", defaults.lane_family_cross_stitch_enabled),
            field_name="train_defaults.lane_family_cross_stitch_enabled",
        ),
        stopline_lane_context_fusion_enabled=_coerce_bool(
            data.get(
                "stopline_lane_context_fusion_enabled",
                defaults.stopline_lane_context_fusion_enabled,
            ),
            field_name="train_defaults.stopline_lane_context_fusion_enabled",
        ),
        ...
        stopline_context_segment_set_aux_weight=_coerce_float(
            data.get(
                "stopline_context_segment_set_aux_weight",
                defaults.stopline_context_segment_set_aux_weight,
            ),
            field_name="train_defaults.stopline_context_segment_set_aux_weight",
        ),
        stopline_context_segment_verifier_aux_weight=_coerce_float(
            data.get(
                "stopline_context_segment_verifier_aux_weight",
                defaults.stopline_context_segment_verifier_aux_weight,
            ),
            field_name="train_defaults.stopline_context_segment_verifier_aux_weight",
        ),
        ...
        stop_line_context_segment_set_enabled=_coerce_bool(
            data.get("stop_line_context_segment_set_enabled", defaults.stop_line_context_segment_set_enabled),
            field_name="train_defaults.stop_line_context_segment_set_enabled",
        ),
        stop_line_context_segment_set_score_threshold=_coerce_float(
            data.get(
                "stop_line_context_segment_set_score_threshold",
                defaults.stop_line_context_segment_set_score_threshold,
            ),
            field_name="train_defaults.stop_line_context_segment_set_score_threshold",
        ),
        stop_line_context_segment_set_max_segments=_coerce_int(
            data.get(
                "stop_line_context_segment_set_max_segments",
                defaults.stop_line_context_segment_set_max_segments,
            ),
            field_name="train_defaults.stop_line_context_segment_set_max_segments",
        ),
        stop_line_context_segment_verifier_score_weight=_coerce_float(
            data.get(
                "stop_line_context_segment_verifier_score_weight",
                defaults.stop_line_context_segment_verifier_score_weight,
            ),
            field_name="train_defaults.stop_line_context_segment_verifier_score_weight",
        ),
    ...
    *,
    ...
    for index, (phase_config, expected_stage) in enumerate(
        ...
        if int(phase_train.lane_conditional_denoise_hard_negative_count) < 0:
            raise ValueError(
                f"phase {index} lane_conditional_denoise_hard_negative_count must be >= 0"
            )
        if float(phase_train.lane_conditional_denoise_hard_negative_offset_px) <= 0.0:
            raise ValueError(
                f"phase {index} lane_conditional_denoise_hard_negative_offset_px must be > 0"
            )
