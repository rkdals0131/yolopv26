...
if not hasattr(modal, "App"):  # pragma: no cover - runtime guidance path
    raise SystemExit(
        "Detected a non-SDK `modal` package. Install/upgrade Modal SDK with: pip install -U modal"
    )


def _detect_repo_root() -> Path:
    # Local execution path: .../tools/train/modal/launcher.py
...
from tools.train.modal.profiles import apply_profile_env_defaults


apply_profile_env_defaults(os.getenv("PV26_MODAL_PROFILE", "a10g"))
...
RUNTIME_DEFAULTS = ModalRuntimeDefaults(
    ...
    cpu=float(os.getenv("PV26_MODAL_CPU", "16.0")),                          # 컨테이너 CPU 코어 할당량
    memory_mb=int(os.getenv("PV26_MODAL_MEMORY_MB", "65536")),               # 컨테이너 RAM(MB)
)

TRAIN_DEFAULTS = ModalTrainDefaults(
    epochs=int(os.getenv("PV26_MODAL_EPOCHS", "5")),                         # 총 학습 epoch 수
    batch_size=int(os.getenv("PV26_MODAL_BATCH_SIZE", "32")),                # 스텝당 배치 크기
    workers=int(os.getenv("PV26_MODAL_WORKERS", "8")),                       # DataLoader worker 수
    prefetch_factor=int(os.getenv("PV26_MODAL_PREFETCH_FACTOR", "4")),       # worker당 prefetch 배치 수
    ...
    seg_output_stride=1,          # segmentation output stride
