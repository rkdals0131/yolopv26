...
_DEFAULT_TORCH_SPEC, _DEFAULT_TORCHVISION_SPEC = default_torch_specs_for_gpu(
    os.getenv("PV26_MODAL_GPU", "H100")
)

RUNTIME_DEFAULTS = ModalRuntimeDefaults(
    app_name=os.getenv("PV26_MODAL_APP_NAME", "pv26-train-h100-w12-pf3"),    # Modal app 이름
    ...
    gpu_name=os.getenv("PV26_MODAL_GPU", "H100"),                             # 예: H100 | H200 | B200
    ...
    cpu=32.0,                                                                 # 컨테이너 CPU 코어 할당량
    memory_mb=131072,                                                         # 컨테이너 RAM(MB)
)

TRAIN_DEFAULTS = ModalTrainDefaults(
    epochs=2,                     # 총 학습 epoch 수
    batch_size=128,               # 스텝당 배치 크기
    workers=14,                   # H100 튜닝 worker 수
    prefetch_factor=3,            # H100 튜닝 prefetch
