...
# =========================
APP_NAME             = os.getenv("PV26_MODAL_APP_NAME", "pv26-train-h100-w12-pf3")     # Modal app 이름 (보통 수정 불필요)
...
GPU_NAME             = os.getenv("PV26_MODAL_GPU", "H100")	# 예: "A10G", "L4", "A100", "H100", "B200"
...
DEFAULT_BATCH_SIZE   = 128        # 스텝당 배치 크기 (크면 GPU 사용률↑, 너무 크면 OOM)
DEFAULT_WORKERS      = 14        # H100 튜닝값(현재 베이스라인)
...
DEFAULT_CPU          = 32.0      # H100 튜닝값(입력 파이프라인 안정화)
DEFAULT_MEMORY_MB    = 131072     # H100 튜닝값(입력 파이프라인 안정화)

DEFAULT_PREFETCH_FACTOR = 3      # H100 시작점(입력 파이프라인 여유)
...
DEFAULT_EVAL_MAP_EVERY = 5       # validation mAP 계산 주기(epoch 단위, 마지막 epoch는 항상 계산)
