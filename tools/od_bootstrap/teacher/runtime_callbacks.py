...
def build_teacher_runtime_callbacks(
    *,
    ...
    def on_train_start(trainer: Any) -> None:
        ...
        trainer.od_tensorboard_writer, trainer.od_tensorboard_status = support.maybe_build_summary_writer_fn(trainer.od_tensorboard_dir)
        train_loader_profile = support.loader_profile_payload_fn(trainer.train_loader)
        val_loader_profile = support.loader_profile_payload_fn(trainer.test_loader)
        trainer.od_log(
    ...
    def on_train_epoch_start(trainer: Any) -> None:
        trainer.od_epoch_started_at = support.time_module.perf_counter()
        ...
        trainer.od_epoch_timing_window = support.deque_type(maxlen=max(1, int(runtime_params["profile_window"])))

    def on_train_batch_start(trainer: Any) -> None:
        support.sync_timing_device_fn(trainer.device, bool(runtime_params["profile_device_sync"]))
        now = support.time_module.perf_counter()
    ...
    def on_train_batch_end(trainer: Any) -> None:
        support.sync_timing_device_fn(trainer.device, bool(runtime_params["profile_device_sync"]))
        ended_at = support.time_module.perf_counter()
        ...
        profile_summary = support.timing_profile_fn(list(trainer.od_epoch_timing_window))
        remaining_steps = max(0, total_steps - step_index)
        eta_sec = float(profile_summary["iteration_sec"]["mean"]) * float(remaining_steps)
        support.set_progress_postfix_fn(
            trainer.od_pbar,
            support.build_live_postfix_fn(
        ...
        if trainer.od_profile_log_path is not None:
            support.append_jsonl_fn(trainer.od_profile_log_path, payload)
        support.write_tensorboard_scalars_fn(
            trainer.od_tensorboard_writer,
            "train_step",
            support.build_train_step_tensorboard_payload_fn(
                losses=losses,
                profile_summary=profile_summary,
                elapsed_sec=elapsed_sec,
            ),
        ...
        if trainer.od_pbar is None:
            trainer.od_log(
                ...
                f"{support.build_live_postfix_fn(elapsed_sec=elapsed_sec, eta_sec=eta_sec, profile_summary=profile_summary)}"
            )

    def on_train_epoch_end(trainer: Any) -> None:
        epoch_profile = {
            "epoch": int(trainer.epoch) + 1,
            "timing_profile": support.timing_profile_fn(list(trainer.od_epoch_timing_window)),
            "loader": support.loader_profile_payload_fn(trainer.train_loader),
        }
        trainer.od_profile_history.append(epoch_profile)

    def on_fit_epoch_end(trainer: Any) -> None:
        epoch_index = int(trainer.epoch) + 1
        epoch_payload = support.build_epoch_tensorboard_payload_fn(
            losses=trainer.label_loss_items(trainer.tloss, prefix="train"),
    ...
    def on_train_end(trainer: Any) -> None:
        payload = {
            ...
            "train_loader": support.loader_profile_payload_fn(trainer.train_loader),
            "val_loader": support.loader_profile_payload_fn(trainer.test_loader),
