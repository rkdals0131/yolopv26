...
from . import progress as runtime_progress
from .callbacks import TeacherRuntimeSupport, build_teacher_runtime_callbacks
from .resume import checkpoint_resume_metadata, resolve_resume_checkpoint_path
...
def build_teacher_dataloader_kwargs(
    dataset: Any,
    *,
    batch: int,
    workers: int,
...
def build_teacher_dataloader(
    ...
    *,
    ...
    loader_kwargs = build_teacher_dataloader_kwargs(
        dataset,
        batch=batch,
        workers=workers,
        shuffle=shuffle,
...
def build_teacher_callbacks(
    *,
    ...
    support = TeacherRuntimeSupport(
        time_module=time,
        deque_type=deque,
        append_jsonl_fn=runtime_progress.append_jsonl,
        sync_timing_device_fn=_sync_timing_device,
        timing_profile_fn=runtime_progress.timing_profile,
        build_live_postfix_fn=runtime_progress.build_live_postfix,
        set_progress_postfix_fn=runtime_progress.set_progress_postfix,
        loader_profile_payload_fn=runtime_progress.loader_profile_payload,
...
def make_teacher_trainer(
    *,
    ...
    class TeacherDetectionTrainer(DetectionTrainer):
        ...
        def get_dataloader(self, dataset_path: str, batch_size: int = 16, rank: int = 0, mode: str = "train"):
            ...
            if getattr(dataset, "rect", False) and shuffle and not np.all(dataset.batch_shapes == dataset.batch_shapes[0]):
                ...
                shuffle = False
            return build_teacher_dataloader(
                dataset,
                batch=batch_size,
        ...
        def _do_train(self):
            ...
            while True:
                ...
                if ultra_trainer.RANK in {-1, 0}:
                    ...
                    rich_pbar = runtime_progress.build_rich_progress_bar(
    ...
    callbacks = build_teacher_callbacks(runtime_params=runtime_params)
    TeacherDetectionTrainer._od_bootstrap_callbacks = callbacks
    return TeacherDetectionTrainer, callbacks


_build_teacher_callbacks = build_teacher_callbacks
_build_teacher_dataloader = build_teacher_dataloader
_build_teacher_dataloader_kwargs = build_teacher_dataloader_kwargs
_make_teacher_trainer = make_teacher_trainer


__all__ = [
    "build_teacher_callbacks",
    "build_teacher_dataloader",
    "build_teacher_dataloader_kwargs",
    "make_teacher_trainer",
]
