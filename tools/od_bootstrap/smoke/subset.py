...
from tools.od_bootstrap.sweep.image_list import ImageListEntry, load_image_list, write_image_list


DEFAULT_SMOKE_QUOTAS: dict[str, dict[str, int]] = {
...
def _normalize_quotas(quotas: Mapping[str, Mapping[str, int]] | None) -> dict[str, dict[str, int]]:
    payload = quotas or DEFAULT_SMOKE_QUOTAS
...
def select_smoke_entries(
    ...
    *,
    ...
    if missing:
        raise RuntimeError(f"smoke image list is missing required samples: {', '.join(missing)}")
...
def build_smoke_image_list(
    *,
    ...
    selected_entries = select_smoke_entries(entries, quotas=quotas)
