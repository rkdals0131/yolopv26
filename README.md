...
- `config/pv26_meta_train.default.yaml`은 `seg_dataset/pv26_exhaustive_od_lane_dataset`를 입력으로 쓰는 최종 merged dataset 전용이다.
- canonical-only 상태라면 먼저 `4번`부터 `10번`까지 실행해 exhaustive OD + lane dataset을 만들어야 한다.
...
mobility와 signal은 기본적으로 `yolo26s`를, obstacle은 기본적으로 `yolo26m`을 사용한다. `yolo26n` 대안 설정도 같은 디렉토리에 있다.
...
python3 tools/od_bootstrap/train/run_train_teacher.py --config tools/od_bootstrap/config/train/obstacle_yolo26m.default.yaml
```
...
- AIHUB나 BDD100K 원본만 있으면 먼저 `2번`, `3번`으로 canonical 전처리를 끝내고, default PV26 학습을 원하면 이어서 `4번`부터 `10번`까지 실행해 최종 merged dataset을 만들어야 한다.
