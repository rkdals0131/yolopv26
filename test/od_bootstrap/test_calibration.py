...
class ODBootstrapCalibrationTests(unittest.TestCase):
    ...
    def test_calibration_rejects_malformed_teacher_label_rows(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            dataset_root = root / "teacher_dataset"
            for sample_id in ("frame_001", "frame_002"):
                ...
                (dataset_root / "images" / "val" / f"{sample_id}.jpg").write_bytes(b"img")
            (dataset_root / "labels" / "val").mkdir(parents=True, exist_ok=True)
            (dataset_root / "labels" / "val" / "frame_001.txt").write_text(
                "0 0.200000 0.200000 0.200000\n",
                encoding="utf-8",
            )
            (dataset_root / "labels" / "val" / "frame_002.txt").write_text(
            ...
            with patch("tools.od_bootstrap.teacher.calibrate.YOLO", _FakeYOLO):
                with self.assertRaisesRegex(ValueError, "calibration malformed label row"):
                    calibrate_class_policy_scenario(scenario, scenario_path=root / "preset_calibration")
