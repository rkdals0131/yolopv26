...
from tools.od_bootstrap.sweep.image_list import load_image_list
...
class ODBootstrapImageListTests(unittest.TestCase):
    def test_load_image_list_resolves_relative_paths_and_sorts_entries(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual([entry.image_id for entry in entries], ["scene_a", "scene_b"])
            self.assertEqual(entries[0].image_path, (images_dir / "a.png").resolve())
            self.assertEqual(entries[1].dataset_key, "aihub_traffic_seoul")
    ...
    def test_load_image_list_rejects_duplicate_image_paths(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manifest_path = root / "image_list.jsonl"
            rows = [
                {"image_id": "one", "image_path": str(image_path), "scene_path": str(scene_path), "dataset_root": str(root)},
                {"image_id": "two", "image_path": str(image_path), "scene_path": str(scene_path), "dataset_root": str(root)},
            ]
            manifest_path.write_text(
                "\n".join(json.dumps(row, ensure_ascii=True) for row in rows) + "\n",
                encoding="utf-8",
