...
class PV26TargetEncoderTests(unittest.TestCase):
    def test_encode_batch_builds_fixed_shape_targets(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first_stop_line = encoded["stop_line"][lane_index, 0]
            self.assertEqual(float(first_stop_line[0]), 1.0)
            stop_line_points = first_stop_line[1:].reshape(STOP_LINE_POINT_COUNT, 2)
            self.assertEqual(tuple(stop_line_points.shape), (STOP_LINE_POINT_COUNT, 2))
            self.assertTrue(torch.isfinite(stop_line_points).all())
            self.assertNotIn("stop_line_width_valid", encoded["mask"])
