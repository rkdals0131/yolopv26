...
class PV26LossSpecTests(unittest.TestCase):
    def test_loss_spec_matches_approved_multitask_design(self) -> None:
        ...
        self.assertEqual(spec["heads"]["lane"]["target_encoding"]["anchor_rows"], 16)
        self.assertEqual(spec["heads"]["lane"]["target_encoding"]["x_coordinates"], 16)
        ...
        self.assertEqual(spec["sampler"]["type"], "task_positive_multi")
        self.assertEqual(spec["sampler"]["task_positive_task"], "multi:lane,stopline,crosswalk")
        self.assertAlmostEqual(spec["sampler"]["task_positive_fraction"], 0.75)
        self.assertEqual(spec["sampler"]["fallback_ratios"]["aihub_obstacle"], 0.15)
    ...
    def test_build_loss_spec_returns_fresh_nested_mutables(self) -> None:
        ...
        spec["heads"]["lane"]["target_encoding"]["anchor_rows"] = -1
        ...
        self.assertEqual(fresh["heads"]["lane"]["target_encoding"]["anchor_rows"], 16)
