...
class DocsSyncTests(unittest.TestCase):
    ...
    def test_refactoring_map_tracks_boundaries_and_probe_taxonomy(self) -> None:
        ...
        self.assertIn("phase weights-only handoff path/policy/load-count/checkpoint-metadata scalars", refactor_map)
        self.assertIn("checkpoint migration evidence must not disappear after analysis export", refactor_map)
        self.assertIn("Runtime status scan drift", refactor_map)
        self.assertIn("from `meta_manifest.json` phase entries while a run is in progress", refactor_map)
    ...
    def test_refactoring_map_tracks_e2e_pipeline_audit(self) -> None:
        ...
        self.assertIn("manifest `output_root`, rows, and `dataset_counts` must match", refactor_map)
        ...
        self.assertIn("Runtime scene-record drift", refactor_map)
        self.assertIn("must revalidate the live scene `source.dataset`, `source.split`, and `image.file_name`", refactor_map)
        self.assertIn("stale scene files must fail at the loader boundary", refactor_map)
        self.assertIn("Raw task-mask drift", refactor_map)
        self.assertIn("Raw image-shape drift", refactor_map)
        self.assertIn("float32 `[B,3,608,800]` image tensors", refactor_map)
        self.assertIn("PV26Heads` input expectations", refactor_map)
        self.assertIn("missing keys must fail at encoder boundary", refactor_map)
        self.assertIn("Source det row-order drift", refactor_map)
        self.assertIn("resume scans must reject stale reused outputs", refactor_map)
        self.assertIn("`labels_det` row count, or row class ids", refactor_map)
        self.assertIn("Loader det-label row drift", refactor_map)
        self.assertIn("malformed `labels_det` YOLO rows", refactor_map)
        self.assertIn("invalid normalized center/size values", refactor_map)
        self.assertIn("Target TL source-mask drift", refactor_map)
        self.assertIn("source_mask.tl_attr` true unless `source_mask.det` is also true", refactor_map)
        self.assertIn("foreign-keyed to detector row order", refactor_map)
        self.assertIn("Source bbox finite drift", refactor_map)
        self.assertIn("must hold non-finite or malformed raw bbox annotations", refactor_map)
        self.assertIn("canonical scene detections or `labels_det`", refactor_map)
        self.assertIn("Source geometry finite drift", refactor_map)
        self.assertIn("must hold non-finite or malformed raw points", refactor_map)
        self.assertIn("canonical scene geometry", refactor_map)
        self.assertIn("Target geometry validity drift", refactor_map)
        self.assertIn("valid lane/stop_line rows need at least 2 unique points", refactor_map)
        self.assertIn("valid crosswalk rows need at least 3 unique points", refactor_map)
        self.assertIn("Target lane visibility drift", refactor_map)
        self.assertIn("loader-facing lane visibility is clamped into `[0,1]`", refactor_map)
        self.assertIn("stale out-of-range visibility values", refactor_map)
        self.assertIn("Source pair ownership drift", refactor_map)
        self.assertIn("must not reuse one image file for multiple label pairs", refactor_map)
        self.assertIn("multiple canonical samples that claim the same source frame", refactor_map)
        self.assertIn("Source output sample-id drift", refactor_map)
        self.assertIn("must reject duplicate output sample ids before resume scans or worker writes", refactor_map)
        self.assertIn("slug collisions can overwrite canonical scene/image/det outputs", refactor_map)
        self.assertIn("Source resume task drift", refactor_map)
        self.assertIn("Canonical det-label task drift", refactor_map)
        self.assertIn("Teacher label-row drift", refactor_map)
        self.assertIn("reject malformed non-empty `labels_det` YOLO rows", refactor_map)
        self.assertIn("Calibration label-row drift", refactor_map)
        self.assertIn("reject malformed teacher-local `labels` YOLO rows", refactor_map)
        self.assertIn("Loaded image-list manifest drift", refactor_map)
        self.assertIn("explicit `tasks.has_det` no longer matches `labels_det` presence", refactor_map)
        self.assertIn("`image.file_name`", refactor_map)
        self.assertIn("current canonical output image", refactor_map)
        self.assertIn("must revalidate `dataset_key`, `split`, `sample_uid`, `scene_path`, `image_path`", refactor_map)
        self.assertIn("sample manifests, or exhaustive materialization can reuse stale manifest rows", refactor_map)
        ...
        self.assertIn("positive integer `det_feature_strides`", refactor_map)
        self.assertIn("stride-shape drift tests", refactor_map)
        self.assertIn("letterbox `meta.transform`", refactor_map)
        self.assertIn("non-finite scale, negative padding, resized/canvas drift", refactor_map)
        self.assertIn("non-canonical raw/network letterbox values", refactor_map)
        self.assertIn("must match exported `det`/`tl_attr` row counts", refactor_map)
        self.assertIn("Export metadata drift", refactor_map)
        self.assertIn("must reject exported lane/stop_line/crosswalk query/vector tails", refactor_map)
        self.assertIn("current loss spec before publishing an artifact", refactor_map)
        ...
        self.assertIn("rejecting metric raw bundles that omit required supervision fields", refactor_map)
        self.assertIn("rejecting non-4D encoded/prediction image payloads", refactor_map)
        self.assertIn("image batch dimension no longer matches `meta`", refactor_map)
        self.assertIn("Phase runtime postprocess drift", refactor_map)
        self.assertIn("same `postprocess_config` installed on the constructed trainer", refactor_map)
        self.assertIn("not a separately rebuilt config that can diverge from evaluator behavior", refactor_map)
        self.assertIn("Phase weights-only handoff drift", refactor_map)
        self.assertIn("shape-aware load reports, and checkpoint metadata in manifest-ready fields", refactor_map)
        self.assertIn("without embedding checkpoint tensors", refactor_map)
        self.assertIn("Raw head contract drift", refactor_map)
        self.assertIn("emitted `det`/`tl_attr` tensor dims", refactor_map)
        self.assertIn("Loss raw-head shape drift", refactor_map)
        self.assertIn("must receive tensor heads for `det`, `tl_attr`, `lane`, `stop_line`, and `crosswalk`", refactor_map)
        self.assertIn("must fail at loss entry before lower-level tensor ops", refactor_map)
