...
- 2026-06-02: letterbox meta handoff now rejects non-finite scale, negative padding, resized/canvas drift, raw/scale mismatch, and internally consistent but non-canonical raw/network letterbox metadata before postprocess can decode coordinates through malformed runtime metadata.
- 2026-06-02: engine raw-batch helpers now reject non-4D encoded/prediction image payloads and raw-batch merge inputs whose image batch dimension no longer matches `meta`, preventing metric summaries from pairing predictions with a shifted raw supervision bundle.
- 2026-06-02: target encoding now rejects `source_mask.tl_attr` without `source_mask.det`, preserving the loader contract that TL attributes are attached to detector row-order supervision.
- 2026-06-02: target encoding now revalidates loader geometry validity at the raw boundary, rejecting valid lane/stop_line rows with fewer than 2 unique points and valid crosswalk rows with fewer than 3 unique points before dense/query target construction.
- 2026-06-02: target encoding now rejects raw lane visibility values outside the loader-facing `[0,1]` range before lane anchor/query visibility targets can silently clamp stale raw payloads.
...
- 2026-06-02: phase weights-only handoff now records the loaded checkpoint path, `shape_aware_partial` policy, load reports, and checkpoint metadata in the phase manifest/result without embedding checkpoint tensors.
- 2026-06-02: durable run analysis now preserves phase weights-only handoff path/policy/load-count/checkpoint-metadata scalars in `phase_overview.csv` so checkpoint migration evidence survives export.
- 2026-06-02: `check_env` PV26 status scan now derives in-progress phase count, latest stage, selection metric, and backbone from `meta_manifest.json` when `summary.json` has not been written yet.
...
- 2026-06-02: BDD/AIHUB source-standardization resume now revalidates detector row-order ids plus `labels_det` row count/class-id order before reusing detector-supervised canonical outputs.
...
- 2026-06-02: loaded bootstrap image-list manifests now revalidate `dataset_key`, `split`, `sample_uid`, `image_path`, and explicit `tasks.has_det` against scene metadata plus `det_path`/canonical `labels_det` before sweep inference, canonical debug-vis, sample manifests, or exhaustive materialization can reuse stale manifest rows.
...
- Phase execution contract: `_execute_phase()` must apply phase overrides before constructing loaders/trainer, pass phase/runtime/history/PCGrad settings into `trainer.fit()`, load the previous phase best checkpoint as weights-only handoff when no local last checkpoint exists, preserve that handoff's checkpoint path, load policy, shape-aware load reports, and checkpoint metadata in manifest-ready fields without embedding checkpoint tensors, generate best/last preview bundles when checkpoint files exist, and return manifest-ready checkpoint/selection/postprocess/head-channel/run-summary fields. The recorded `head_channels` must come from the constructed `PV26Heads.in_channels`, not only from the backbone-variant fallback.
...
- Loss raw-head shape drift: `PV26MultiTaskLoss` must receive tensor heads for `det`, `tl_attr`, `lane`, `stop_line`, and `crosswalk` with batch/query/vector dimensions derived from the current loss spec; missing keys or malformed tail dimensions must fail at loss entry before lower-level tensor ops. Guarded by loss runtime raw prediction contract tests.
...
- Raw image-shape drift: direct encoder callers must still provide float32 `[B,3,608,800]` image tensors, matching `collate_pv26_samples()` and `PV26Heads` input expectations, instead of letting wrong network-space tensors pass to the model. Guarded by target encoder raw image shape/dtype tests.
- Raw task-mask drift: `source_mask` and `valid_mask` must carry the full `det`/`tl_attr`/`lane`/`stop_line`/`crosswalk` key set before task-mode filtering; missing keys must fail at encoder boundary instead of silently disabling supervision or surfacing as late `KeyError`. Guarded by target encoder raw-mask contract tests.
...
- Final-manifest loader drift: final dataset publication manifest `output_root`, rows, and `dataset_counts` must match the actual records that `PV26CanonicalDataset` discovers, including image/det paths and source trace metadata, before train/eval dataloaders can read the dataset. Guarded by loader manifest drift tests and final dataset publication-through-loader tests.
- Runtime scene-record drift: `PV26CanonicalDataset.__getitem__()` must revalidate the live scene `source.dataset`, `source.split`, and `image.file_name` against the discovered record before deriving source masks, image paths, or task supervision; stale scene files must fail at the loader boundary. Guarded by loader runtime scene-record identity tests.
...
- Loader det-label row drift: malformed `labels_det` YOLO rows, out-of-range class ids, non-finite normalized coordinates, and invalid normalized center/size values must fail before detector targets are constructed. Guarded by loader malformed detector-label row tests.
- Target TL source-mask drift: raw target encoding callers must not mark `source_mask.tl_attr` true unless `source_mask.det` is also true, because TL attribute rows are foreign-keyed to detector row order. Guarded by target encoder TL-without-det source-mask tests.
- Source bbox finite drift: BDD/AIHUB source standardizers must hold non-finite or malformed raw bbox annotations before writing canonical scene detections or `labels_det`; otherwise NaN/Inf coordinates can cross from raw preprocessing into model targets. Guarded by BDD non-finite bbox tests and shared AIHUB raw bbox helper tests.
- Source geometry finite drift: AIHUB lane/stop/crosswalk source standardizers must hold non-finite or malformed raw points before writing canonical scene geometry; otherwise NaN/Inf points can cross from preprocessing into target encoding. Guarded by AIHUB non-finite lane-point tests and shared raw point helper tests.
- Target geometry validity drift: raw target encoding callers must preserve the loader's valid-row geometry contract; valid lane/stop_line rows need at least 2 unique points, and valid crosswalk rows need at least 3 unique points. Guarded by target encoder degenerate valid roadmark row tests.
- Target lane visibility drift: loader-facing lane visibility is clamped into `[0,1]`, so raw target encoding callers must not pass stale out-of-range visibility values that would silently change lane anchor/query supervision. Guarded by target encoder lane visibility range tests.
- Source pair ownership drift: raw source discovery must not reuse one image file for multiple label pairs; otherwise preprocessing can create multiple canonical samples that claim the same source frame while downstream duplicate-image guards only see already-materialized outputs. Guarded by shared source pair-discovery ownership tests.
- Source output sample-id drift: BDD/AIHUB source standardizers must reject duplicate output sample ids before resume scans or worker writes; otherwise slug collisions can overwrite canonical scene/image/det outputs before loader or final-dataset duplicate guards see them. Guarded by BDD/AIHUB duplicate source output sample-id tests.
- Source det row-order drift: BDD and AIHUB detector-supervised source standardizers must keep `labels_det` rows aligned with `labels_scene.detections[].id`, and resume scans must reject stale reused outputs whose detector ids, `labels_det` row count, or row class ids no longer match scene detections. AIHUB traffic TL attributes must reference those same ids. Guarded by BDD/AIHUB standardization row-order and resume row-drift tests plus loader TL linkage tests.
- Source resume task drift: resume must not reuse stale canonical scene files whose `tasks`, `image.file_name`, lane-family fields, traffic fields, or stale `labels_det` files contradict the source kind or current canonical output image. Guarded by BDD/AIHUB standardization resume tests.
...
- Teacher label-row drift: teacher dataset publication must reject malformed non-empty `labels_det` YOLO rows instead of silently dropping rows and publishing zero-detection labels for detector-supervised samples. Guarded by teacher dataset malformed-row tests.
- Calibration label-row drift: calibration must reject malformed teacher-local `labels` YOLO rows instead of silently dropping ground-truth boxes and tuning class policies against corrupted precision/recall. Guarded by calibration malformed-row tests.
- Loaded image-list manifest drift: explicit bootstrap image-list manifests must revalidate `dataset_key`, `split`, `sample_uid`, `scene_path`, `image_path`, `tasks.has_det`, `det_path`, and canonical `labels_det` presence instead of trusting a stale JSONL row. Guarded by image-list load tests and od_bootstrap sweep/debug-vis/sample-manifest tests.
...
- Coordinate drift: source raw xyxy/points, online letterbox network space, letterbox `meta.transform`, and postprocess inverse raw space must remain separate and internally consistent. Malformed runtime metadata with non-finite scale, negative padding, resized/canvas drift, raw/scale mismatch, or non-canonical raw/network letterbox values must fail before coordinate decode. Guarded by loader, transform roundtrip, target encoder, and postprocess tests that cover both detector boxes and lane-family points.
...
- Detector feature metadata drift: `det`, `tl_attr`, `det_feature_shapes`, and positive integer `det_feature_strides` must describe the same detector query layout before postprocess anchor decode. Guarded by postprocess metadata-required, query-count, and stride-shape drift tests.
- Export metadata drift: TorchScript metadata must reject detector feature shape/stride rows that disagree with exported `det` and `tl_attr` raw head rows, and must reject exported lane/stop_line/crosswalk query/vector tails that no longer match the current loss spec before publishing an artifact. Guarded by model export metadata tests.
- Prepare-batch raw-bundle drift: evaluator/trainer must keep encoded eval `_raw_batch` as the same unmoved no-image supervision bundle while regenerating seg-first dense targets, preserving `meta` alignment, rejecting metric raw bundles that omit required supervision fields, rejecting non-4D encoded/prediction image payloads, and rejecting raw-batch merge inputs whose image batch dimension no longer matches `meta`. Guarded by evaluator/trainer prepare-batch rehydration tests and engine raw-batch helper tests.
- Phase runtime postprocess drift: phase manifests, phase results, trainer-built evaluators, and preview/runtime summaries must record the same `postprocess_config` installed on the constructed trainer, not a separately rebuilt config that can diverge from evaluator behavior. Guarded by train runtime phase handoff tests.
- Phase weights-only handoff drift: cross-phase migration must stay visibly weights-only by recording the previous best checkpoint path, `shape_aware_partial` load policy, adapter/head load reports, and checkpoint metadata in phase manifest/result fields while keeping checkpoint tensors out of JSON artifacts. Guarded by train runtime phase handoff tests.
...
- Durable analysis drift: `tools/analyze_pv26_run.py` must preserve trainer summary `best_epoch`, `best_metric_value`, selection mode, and phase weights-only handoff path/policy/load-count/checkpoint-metadata scalars when exporting phase overview CSVs; min-selection loss phases must not be reinterpreted as max `phase_objective` phases, and checkpoint migration evidence must not disappear after analysis export. Guarded by analyze-run I/O tests.
- Runtime status scan drift: `tools/check_env.py` must report PV26 run progress, latest stage, selection metric, and backbone from `summary.json` when present and from `meta_manifest.json` phase entries while a run is in progress; otherwise resume/derived-run decisions can hide the active training contract. Guarded by portability runtime scan tests.
