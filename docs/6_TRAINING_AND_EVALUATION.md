...
본체 표본 선택의 기본값은 source별 균등 복원추출인 `random_with_replacement`다. 소규모 방법론 비교에서는 `least_used_of_two`를 선택할 수 있다. 이 방식은 같은 source에서 무작위 후보 두 개를 뽑고 현재까지 덜 사용한 표본을 골라 노출 편차를 줄인다. 사용 횟수는 완료한 draw id에서 결정적으로 복원하므로 DataLoader가 미리 읽은 배치는 재개 위치에 포함하지 않는다.
...
기본 설정은 detector head에 `head_lr`을 적용하고, `roadmark_lr`가 없으면 도로표식 decoder에도 같은 값을 적용한다. 태스크별 실험에서는 `detector_head_lr`와 `roadmark_lr`로 각 헤드의 학습률을 따로 지정할 수 있다. 현재 다음 실행 후보는 backbone `1e-4`, detector head `1e-3`, roadmark decoder `3e-3`이다. 소규모 3-seed 실험에서 도로표식 개선을 확인한 후보이며, 전체 데이터와 촬영 구간이 분리된 평가에서의 개선은 별도로 확인해야 한다.
...
`data.hard_negative_mining`을 설정하면 학습용 정지선 음성에서 현재 모델이
정지선을 예측하는 이미지를 주기적으로 다시 모은다. `positive_index`는 같은
학습 집합의 정지선 양성 목록이며, 실제 라벨에 정지선이 있는 이미지는 음성으로
채굴하지 않는다. 검증 이미지는 채굴하지 않는다.

`initial_samples: 0`은 첫 회 전체 음성 탐색, `refresh_samples`는 이후 순환 탐색량,
`interval_steps`는 갱신 간격이다. `hard_fractions`와 `positive_fraction`은
도로표식 내부가 아닌 **전체 학습 추출에서의 비율**이다. 새 오탐은 추가하고,
재검사에서 해결된 이미지는 일반 음성으로 돌린다. 오탐 풀이 비면 해당 비율도
일반 음성으로 돌린다. 같은 이미지의 반복 인덱스는 채굴 풀에서 중복 제거한다.

풀·탐색 위치·비율 단계는 옵티마이저와 함께 체크포인트에 저장한다. 갱신 뒤에는
이전 풀에서 미리 읽어 둔 배치를 버리고 소비 완료 위치부터 다시 뽑는다.
채굴이 중단되면 완료되지 않은 결과를 반영하지 않는다. 최근 풀과 규모는
실행 디렉터리의 `hard_negative_pool.json`, 진행 상황은 학습 로그에서 확인한다.
...
학습 산출물은 kai의 `/home/kai/yolopv26/runs/<run>/`에 저장한다. 긴 실행 전 이 파일시스템의 여유 공간을 확인한다. 자동 정리는 해당 실행에서 만든 파일만 대상으로 한다. 저장소에 포함한 SignalAttr 기준 가중치는 유지한다.
...
주기 검증과 전체 검증을 구분한다. 기본 주기 검증은 source당 1,024장이고, 계획 종료 시 최종·태스크별 최고 체크포인트를 저장된 검증 목록 전체에서 평가한다. `validation_full_*.json`에는 평가한 source별 이미지 수, 체크포인트 step, 지표를 기록한다. 전체 평가 결과를 표본 기반 best 선택 점수와 섞지 않는다. Joint에서는 기존 신호등 우선 `best.pt` 외에 `best_roadmark.pt`를 비교용으로 보존한다.
