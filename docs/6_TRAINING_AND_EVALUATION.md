...
현재 다음 실행 후보는 backbone `1e-4`, detector head `1e-3`, roadmark decoder `3e-3`이다. Decoder 학습률은 `roadmark_lr`로 분리한다. 소규모 3-seed 실험에서 도로표식 개선을 확인한 후보이며, 전체 데이터와 촬영 구간이 분리된 평가에서의 개선은 별도로 확인해야 한다.
