...
기존 단일 `data` 패키지 안에는 사실상 네 종류의 일이 섞여 있었다.
...
- `tools/od_bootstrap/build/checkpoint_audit.py` 안의 **날짜 박힌 고정 checkpoint path**
...
| 기존 AIHUB wrapper/impl 쌍 | `source/aihub.py` | 공개/구현 이중화 제거 |
...
| 기존 final-dataset wrapper/impl 쌍 | `build/final_dataset.py` | wrapper 제거 |
| 기존 debug/review/audit 묶음 | `build/qa.py` 또는 `qa/` | QA/debug/inspection은 core pipeline과 분리 |
...
`tools/od_bootstrap/build/artifacts.py`, `build/sweep_types.py` 류는 방향이 좋다.  
