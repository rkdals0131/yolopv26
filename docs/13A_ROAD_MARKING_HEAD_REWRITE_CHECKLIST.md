...
- [ ] lane query count `20`, stop_line query count `8`, crosswalk query count `8`이 문서에 고정돼 있다
...
- [ ] lane internal contract를 `B x 20 x 38`로 전환
- [ ] lane target encoder를 row-anchor 기반으로 전환
- [ ] raw visibility를 loader -> target encoder -> loss까지 보존
