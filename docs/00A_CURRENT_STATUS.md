...
- projection selector-audit code commit: `4d16e2f`.
...
- projection selector-audit result: best single-feature no-GT gate is `component_svd_thickness_mean >= 4.681936370001899`, stop-line F1 `0.5250`, TP/FP/FN `126 / 83 / 145`; GT-presence oracle control reaches `0.5575` but is not a production selector.
- 판단: low-score extension regresses and is closed. Length competition is a small partial-positive over fragment union (`0.4948 -> 0.5031`, `+1 TP`, `-5 FP`) but the delta audit shows the gain is confined to six samples and mostly fallback suppression, not broad geometry recovery. Multi-instance and second-instance gating are weak partial positives. Projection splitting gives a real FP improvement, and projection-competition is the current ungated local stop-line readout reference (`0.5164`). The single-feature selector audit can suppress only 8 FP (`0.5250`) and is still far from `0.60`, so it is not a deployable selector path. Do not repeat these as feature-rank, single-score, top-K, second-prediction threshold, projection-gap, projection-competition length/min-score, or projection-competition single-feature selector sweeps. The next stop-line step must improve candidate generation/midpoint recovery or introduce a materially richer no-GT selector, while preserving crosswalk hull retention and lane composition explicitly.
