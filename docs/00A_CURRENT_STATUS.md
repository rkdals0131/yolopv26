...
- 판단: low-score extension regresses and is closed. Length competition is a small partial-positive over fragment union (`0.4948 -> 0.5031`, `+1 TP`, `-5 FP`) but the delta audit shows the gain is confined to six samples and mostly fallback suppression, not broad geometry recovery. Multi-instance and second-instance gating are weak partial positives. Projection splitting is the current local stop-line readout reference (`0.5112`) and gives a real FP improvement over gated multi-instance (`95 -> 93`), but it is still far from `0.60`. Do not repeat these as feature-rank, single-score, top-K, second-prediction threshold, or projection-gap sweeps. The next stop-line step must improve candidate generation/midpoint recovery or introduce a stronger no-GT selector, while preserving crosswalk hull retention and lane composition explicitly.
