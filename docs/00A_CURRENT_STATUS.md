...
- 남은 stop-line 방향은 새 query row, endpoint-delta channel, local score window, pairwise component split, selector-map gate, row/x span proposal이 아니라, mask/row/x signal을 valid line proposal로 학습 단계부터 결합하는 stronger geometry contract를 찾는 것이다.
