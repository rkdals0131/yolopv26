...
Latest stop-line crosswalk-hull edge candidate verifier changed candidate generation again rather than re-ranking current stop-line candidates: predicted crosswalk hull edges generate stop-line-like segment candidates at fixed edge offsets, current dense stop-line/crosswalk maps provide no-GT features, and a train-split MLP verifier selects candidates. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, and created no dataset copy. Smoke train64/val4 was coverage-negative with `0` train and `0` val oracle-positive crosswalk-edge candidates. The user-requested larger train512/exact-val128 exposure was also negative: train had `6338` scored candidates and `0` oracle-positive labels; val had only `2` oracle-positive candidates out of `378` scored candidates. Exact baseline was lane/stop/cross `0.5660 / 0.5289 / 0.5988`, stop-line `32 / 29 / 28`; baseline-plus-crosswalk-edge MLP stayed exactly `32 / 29 / 28`, F1 `0.5289`; baseline-plus-oracle-crosswalk-edge worsened to `32 / 30 / 28`, F1 `0.5246`. Broader-val512 is skipped because exact failed and candidate coverage is far too low. Artifacts are CSV/summary-only (`1.3M` smoke, `8.6M` exact), no verifier checkpoint was saved, no dataset copy was created, and root weights were pruned.
...
- latest stop-line crosswalk-hull edge candidate verifier train512 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_crosswalk_edge_candidates_train512_exact_val128_20260531/summary.json`
