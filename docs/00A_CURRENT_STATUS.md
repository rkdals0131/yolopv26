...
- 다음 lane 축은 row-scan over-link guard 또는 targeted visual review로 default 승격 가능성을 더 확인한 뒤, 여전히 남은 gap을 side/truncated/near-vertical lane의 predicted centerline coverage와 fragment separation을 같이 다루는 방향으로 좁힌다. stop-line을 재개한다면 dense signal을 line geometry로 바꾸는 readout/target contract 쪽으로 제한한다.
