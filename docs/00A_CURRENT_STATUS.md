...
- storage/runtime contract: the probe reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root and created no dataset copy. The stress-audit update added chunked validation replay plus save/load for the small verifier model so val512 can be evaluated under runtime limits without holding all raw batches in memory. The ensemble-stability update also reused the existing `429350`-record root in place and wrote CSV/summary artifacts only. Retained output sizes are `36K` smoke, `152K` train256 exact, `216K` train384 exact, `580K` saved-model val4, about `472K` across the four val512 chunks plus `8K` aggregate, and `40K / 192K / 436K` for ensemble smoke/exact/broader.
