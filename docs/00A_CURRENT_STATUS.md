...
- 남은 stop-line 방향은 새 query row, endpoint-delta channel, local score window, pairwise component split, selector-map gate, row/x span proposal, simple rowx-band selector target이 아니라, center/proposal 선택을 개선한 뒤 predicted angle + mask extent로 길이를 읽는 production readout contract다.
