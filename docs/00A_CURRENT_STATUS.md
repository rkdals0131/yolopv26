...
Latest lane-family det-source distill sample-selection exposure changed the previous full-root distill contract so teacher distillation is applied only on det-source-only rows, not on lane-family supervised rows. It trained real CUDA `2x64` under a static trunk on the existing `429350`-record root, created no dataset copy, and logged epoch source counts `det_source_samples=64` plus lane/stop/cross `192` each. It is still fixed-val4 negative: best `flip_centerline_avg_lane_cross_comp050` replay gives lane/stop/cross `0.5672 / 0.0000 / 0.5455`, lane TP/FP/FN `38 / 10 / 48`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`. This is less damaging than unmasked det-source distill (`36 / 12 / 50`) but still below the retained fixed lane reference `0.5839`, `40 / 11 / 46`, and it recovers no stop-line TP. Exact-val128 and broader-val512 are skipped. Negative checkpoints, TensorBoard, and root weights were pruned; retained run size is about `836K`.
