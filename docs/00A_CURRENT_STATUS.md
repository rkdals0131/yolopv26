...
Latest lane union-pool ranker changed the dropped-candidate lane verifier contract instead of retuning append/suppress thresholds. It trains one no-GT line-ROI MLP over retained row-scan lanes plus raw bbox/area-dropped lane candidates, then reselects a fixed-count lane set from the union pool. It reused the existing `pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, trained on `64` canonical train batches, and created no dataset copy. The first count-expanding sanity run selected `73` candidates and recovered lane TP `+4`, but FP grew `+18`, moving fixed val4 lane `0.5839 -> 0.5535`. The intended fixed-count run selected `51` candidates, but still regressed lane `0.5839 -> 0.5547`, TP/FP/FN `40 / 11 / 46 -> 38 / 13 / 48`; stop-line and crosswalk stayed `0.0000 / 0.5455`. Exact-val128, broader-val512, and larger training are skipped because the first fixed gate lost TP and added FP. Retained artifacts are CSV/summary only, about `184K` total, and the temporary root `yolo26s.pt` was pruned.
