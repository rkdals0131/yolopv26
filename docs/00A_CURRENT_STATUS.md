...
- 다음 lane 축은 row-scan micro-guard가 아니라, side/truncated/near-vertical lane의 predicted centerline coverage와 fragment separation을 같이 다루는 방향으로 좁힌다. stop-line을 재개한다면 dense signal을 line geometry로 바꾸는 readout/target contract 쪽으로 제한한다.
