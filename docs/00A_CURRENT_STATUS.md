...
2026-06-02 정식 편입 후 현재 best 상태와 최신 실패 이력은 `develop`에 포함된다. 직전에는 `origin/develop...current = 0 / 170`으로 develop이 current search branch 뒤에 있었지만, current frontier를 fast-forward 방식으로 `develop`에 편입했다. 이후 exp branch anchor는 live branch가 아니라 archive tag로 전환했다.
...
- `archive/lane-family-router-best-20260529`: best numeric two-checkpoint/router stop-line tradeoff anchor.
...
이 60% objective 돌파는 raw model만으로 만든 결론이 아니고, 세 task F1이 모두 0.6을 넘었다는 뜻도 아니다. core-centerline/refinement checkpoint 위에 row-scan/tangent-link vectorizer, small-fragment FP postprocess filters, lane-head transplant, flip-centerline TTA, crosswalk-mask lane competition, projection-competition stop-line runtime decode, 또는 hull-based crosswalk decode가 붙어서 만든 partial success다. Latest stop/cross lane-frozen trained composite는 broader-val512 objective `0.6421`, lane/stop-line/crosswalk F1 `0.5571 / 0.5278 / 0.6142`이고, retained lane-preserving task-balance composite는 `0.5628 / 0.5164 / 0.6187`이다. User-requested larger-range `2048` train-batch scale audit of the same lane-frozen axis regressed to broader lane/stop/cross `0.5564 / 0.5122 / 0.6162`, so it is negative. Latest lane seed-trace instance decoder larger-slice train reached exact-val128 epoch-2 lane/stop/cross `0.5488 / 0.5000 / 0.5644`, so it is also negative. Latest input-scale `672x896` dense-target train reached fixed broader-val512 lane/stop/cross `0.5519 / 0.4734 / 0.6257`, so it is negative despite crosswalk passing. Latest stop-line axis-distance field train reached fixed broader-val512 lane/stop/cross `0.5431 / 0.2629 / 0.6081`, so it is negative and far below the projection-competition stop-line reference. Latest lane center-offset field train reached fixed broader-val512 lane/stop/cross `0.5390 / 0.5164 / 0.6166`, so it is also negative: stop-line only matches the retained projection-competition runtime reference while lane regresses sharply. Latest stop-line focus-crop feeding train reached fixed broader-val512 lane/stop/cross `0.5454 / 0.4061 / 0.5910`, so crop/zoom feeding is negative: stop-line TP rises versus raw runtime but FP nearly doubles versus projection-comp. Latest upper-trunk retention-distill train reached fixed broader-val512 lane/stop/cross `0.5336 / 0.5195 / 0.6200`, so retention distill preserves crosswalk and gives only a tiny stop-line bump over projection-comp while lane collapses further. Latest two-checkpoint stop-line router audit preserves retained lane/crosswalk while routing stop-line from the stop-line-priority specialist, giving broader lane/stop/cross `0.5628 / 0.5309 / 0.6187`; however a newly trained stop-line-only upper-trunk router specialist regressed to `0.5628 / 0.4826 / 0.6187`. Latest lane-router specialist upper-trunk smoke was rejected: fixed router val4 lane regressed `0.5839 -> 0.5735`, TP/FP/FN `40 / 11 / 46 -> 39 / 11 / 47`, so it was not broadened. Latest stop-line dual-source arbitration was exact-only positive but broader-negative: exact `primary_absent_specialist` moved stop-line `32 / 28 / 28 -> 33 / 28 / 27`, but broader fell below the specialist router at `129 / 103 / 142`, F1 `0.5129` versus `0.5309`. Latest V3 stop-line isolated-neck train was internal-val positive but fixed exact-negative: training val epoch 3 stop-line reached `34 / 28 / 19`, F1 `0.5913`, but fixed router exact-val128 epoch-2 fell to `27 / 42 / 33`, F1 `0.4186`, so broader was skipped. Latest stop-line midpoint proposal head train added a dedicated midpoint proposal logit and trained it at real `3`-epoch/`512` train-batch scale, but fixed router exact-val128 fell to lane/stop/cross `0.5888 / 0.1739 / 0.5988`, stop-line `6 / 3 / 54`, so broader was skipped. Latest heterogeneous endpoint-pair router audit fixed the specialist scenario/postprocess contract plumbing, but exact-val128 endpoint-pair specialist was only stop-line `21 / 43 / 39`, F1 `0.3387`, and all dual-source modes stayed below primary projection-comp exact `0.5333`. User-requested larger-range `2048` train-batch stop-line-priority positive-sampler audit also failed fixed exact: internal epoch-2 stop-line reached `63 / 46 / 44`, F1 `0.5833`, but router exact-val128 was only `30 / 34 / 30`, F1 `0.4839`, so broader was skipped. Latest stop-line-only mask-first specialist train was also internal-val positive but fixed exact-negative: training val epoch 3 stop-line reached `34 / 29 / 19`, F1 `0.5862`, but fixed router exact-val128 fell to `28 / 42 / 32`, F1 `0.4308`, so broader was skipped. Latest learned source-router trained a small MLP over primary/specialist output statistics, but exact-val128 learned-router stop-line was only `30 / 29 / 30`, F1 `0.5042`, below primary projection-comp `32 / 28 / 28`, F1 `0.5333`; oracle routing would reach `34 / 2 / 26`, F1 `0.7083`, so source headroom exists but the current no-GT output-stat signal is not enough. Latest dense-aligned source-router reopened only the permitted materially different runtime quality signal premise by adding line-aligned dense-map/raw-image features and training on `256` batches, but exact-val128 learned-router fell to stop-line `27 / 23 / 33`, F1 `0.4909`, versus primary `32 / 28 / 28`, F1 `0.5333`; broader was skipped. Latest raw-patch geometry-repair replay trained a no-GT MLP to predict stop-line candidate repair score plus endpoint deltas, but exact held-out collapsed to stop-line `5 / 34 / 26`, F1 `0.1429`, and all-split train-threshold replay was only `25 / 34 / 35`, F1 `0.4202`, below baseline `0.4483` and projection-comp `0.5167`; broader was skipped. Latest fixed-alpha stop-line-head checkpoint interpolation/model-soup axis trained real stop-line-priority heads-only checkpoints at `2x64` and user-requested larger `2x256` scale on the existing `429350`-record dataset root, but fixed val4 stayed stop-line `0` TP for both raw and interpolated checkpoints. A previously trained larger stop-line-priority checkpoint interpolated at fixed `alpha=0.50` did reach exact-val128 lane/stop/cross `0.5888 / 0.4793 / 0.5988`, stop-line `29 / 32 / 31`, still below projection-comp exact `0.5167` and primary `0.5333`; broader was skipped. All of these still leave lane/stop-line below `0.60`.

Latest lane temporal-neighbor verifier reopened the old temporal lane idea only with a new train-split no-GT FP-control signal: neighboring-frame lane predictions become candidates, current-frame dense lane evidence plus geometry features feed a small MLP verifier, and validation preserves stop-line/crosswalk while adding only verified lanes. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, saved no verifier checkpoint, and pruned temporary root weights. Smoke train64/val4 was negative: lane `41 / 12 / 45`, F1 `0.5899`, moved to `41 / 16 / 45`, F1 `0.5734`; selected verifier candidates recovered `0` baseline FNs. The user-requested larger train256/exact-val128 run was also negative: lane `1200 / 510 / 1190`, F1 `0.5854`, moved to `1235 / 632 / 1155`, F1 `0.5802`; TP rose `+35`, but FP rose `+122`. Stop-line/crosswalk were unchanged at exact `0.5333 / 0.5988`. Broader-val512 is skipped because exact FP-control failed. Retained artifacts are CSV/summary-only (`1.7M`, `9.2M`).

Current artifact cleanup status on 2026-05-31: historical negative `phase_4/checkpoints/*.pt` files and TensorBoard event files were pruned after their metrics were documented. The only retained phase-4 checkpoint files under `runs/pv26_exhaustive_od_lane_train` are the base `lane60_core_centerline_refine_cross_retain.../phase_4/checkpoints/best.pt` and the stop-line-priority router specialist `lane60_stopline_priority_positive_sampler...20260529_101745/phase_4/checkpoints/best.pt`; `merged_lane_head.pt` remains in the lane-head transplant run. Root `yolo26*.pt` files are absent. This cleanup does not change the F1 status or success criteria.

Latest lane phase-correlation temporal alignment reopened lane temporal only under a materially different candidate-geometry premise: current/neighbor encoded frames are converted to compact grayscale, phase-correlated, and neighbor-frame lane polylines are translated into current raw coordinates before the same train-split no-GT verifier. The first train256 attempt exposed a cache-memory problem because full encoded image tensors were retained; the tool now stores compact uint8 grayscale neighbor images, then the exact run completes. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, saved no verifier checkpoint, and pruned temporary root weights. Smoke train64/val4 is negative: baseline lane `41 / 12 / 45`, F1 `0.5899`, moves to `40 / 18 / 46`, F1 `0.5556`; selected verifier candidates recover `0` baseline FNs. Larger train256/exact-val128 is also negative: baseline lane `1200 / 510 / 1190`, F1 `0.5854`, moves to `1236 / 686 / 1154`, F1 `0.5733`; TP rises `+36`, but FP rises `+176`. Stop-line/crosswalk stay unchanged at exact `0.5333 / 0.5988`. Broader-val512 is skipped because exact FP-control is worse than the unaligned temporal verifier. Retained artifacts are CSV/summary-only (`2.1M`, `12M`).

Latest lane area-ROI stacked-quality verifier tested a stronger no-GT instance-quality signal on the known dropped-candidate rescue surface by stacking alignment-context, dense side-contrast, row-peak alignment, cross-task conflict, TTA consistency, and a 3-member `mean_minus_std` ensemble over the same append contract. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, saved no verifier checkpoint, and pruned temporary root weights. Smoke train64/fixed-val4 passed: lane moved `40 / 11 / 46`, F1 `0.5839`, to `43 / 13 / 43`, F1 `0.6056`. The larger train256/exact-val128 gate set a new area-ROI exact frontier but still missed the target: lane moved `1202 / 491 / 1188`, F1 `0.5888`, to `1240 / 513 / 1150`, F1 `0.5986`; TP rose `+38` and FP rose `+22`. Stop-line/crosswalk stayed unchanged at exact `0.5333 / 0.5988`. A single broader-val512 process exited before summary emission around val batch `180`, so broader was evaluated as four deterministic 128-batch chunks and aggregated: lane moved `4634 / 1983 / 4871`, F1 `0.5749`, to `4801 / 2105 / 4704`, F1 `0.5851`; stop-line/crosswalk stayed `0.5302 / 0.5969`. This is the current learned area-ROI replay frontier, but it is not success: lane, stop-line, and crosswalk are still below `0.60` on the aggregate slice. Retained artifacts are CSV/summary-only (`48K`, `204K`, `212K`, `208K`, `216K`, `12K`).

Latest lane area-ROI focal-BCE verifier tested a training-methodology change on the known dropped-candidate rescue surface: the verifier keeps the same no-GT line-ROI candidate features and runtime append contract, but trains with focal BCE so hard misclassified candidates contribute more than easy examples. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, saved no verifier checkpoint, and pruned temporary root weights. Smoke train64/fixed-val4 was weak-positive: lane moved `40 / 11 / 46`, F1 `0.5839`, to `42 / 14 / 44`, F1 `0.5915`. The larger train256/exact-val128 gate stayed below target and below the retained area-ROI frontier: lane moved `1202 / 491 / 1188`, F1 `0.5888`, to `1241 / 544 / 1149`, F1 `0.5945`; TP rose `+39`, but FP rose `+53`. Stop-line/crosswalk stayed unchanged at exact `0.5333 / 0.5988`. Broader-val512 is skipped because exact remains below `0.60` and below the previous area-ROI exact frontiers. Retained artifacts are CSV/summary-only (`48K`, `208K`).

Latest stop-line copy-paste static-trunk smoke tested whether the previously negative real donor patch signal was being masked by lane/cross/trunk drift. It reuses `lane_family_stopline_static_trunk`, trains only stop-line modules, keeps frozen trunk/non-stop heads in eval mode, enables real stop-line copy-paste augmentation, and reuses the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place. CUDA `2x64` train completed with skipped steps `0`, but fixed val4 was lane/stop/cross `0.5507 / 0.0000 / 0.5455`, with TP/FP/FN lane `38 / 14 / 48`, stop-line `0 / 3 / 2`, and crosswalk `3 / 1 / 4`. Exact-val128, broader-val512, and larger-range scaling are skipped because the first fixed gate recovered no stop-line TP. Negative checkpoints, TensorBoard, and temporary root weights were pruned; retained run size is about `7.3M`.

Latest stop-line temporal endpoint-envelope probe changed the temporal candidate source, not only the selector: sparse-affine aligned neighbor stop-lines are paired with retained projection-comp stop-lines, and when the pair supports one axis the runtime candidate spans the along-axis endpoint envelope. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and trained no-GT MLP verifiers on train split candidates. Smoke train64/fixed-val4 had no validation oracle-positive temporal/envelope candidates, so it was broadened to train256/exact-val128. Exact did create new oracle headroom: temporal candidates `134` with `29` positives, endpoint-envelope candidates `36` with `8` positives, and baseline-plus-oracle temporal moved stop-line to `35 / 32 / 25`, F1 `0.5512`. The learned paths did not convert that headroom: baseline stop-line was `32 / 28 / 28`, F1 `0.5333`; baseline-plus-temporal MLP regressed to `33 / 43 / 27`, F1 `0.4853`; temporal union selector was only `31 / 25 / 29`, F1 `0.5345`, effectively the same as the prior sparse-affine temporal union selector and below the needed improvement. Broader-val512 and train-batch scaling are skipped. Retained artifacts are CSV/summary-only (`820K`, `3.6M`), with root `yolo26s.pt` pruned.

Latest stop-line temporal dense-component probe changed the temporal candidate source again: neighbor-frame stop-line dense mask/center/selector maps are converted into PCA component segments, then sparse-affine aligned into the current frame before train-split MLP and union-selector scoring. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was not decision-useful because validation dense temporal oracle positives were `0`, but train candidates had coverage (`305` dense candidates, `92` positives). The larger train256/exact-val128 gate remained negative. With source flag only, baseline was `30 / 30 / 30`, F1 `0.5000`, while baseline-plus-temporal MLP was `33 / 56 / 27`, F1 `0.4430`, and temporal union selector was `27 / 29 / 33`, F1 `0.4655`. After adding dense-component own-support/geometry features into the learned selector, exact candidate coverage was still real (`217` validation dense candidates, `23` positives; baseline-plus-oracle temporal `36 / 37 / 24`, F1 `0.5414`), but deployable learned paths still failed: baseline was `32 / 28 / 28`, F1 `0.5333`; baseline-plus-temporal MLP was `33 / 54 / 27`, F1 `0.4490`; temporal union selector was `30 / 33 / 30`, F1 `0.4878`. Broader-val512 and further train scaling are skipped because exact learned TP/FP/FN is below primary projection-comp.

Latest stop-line ORB/RANSAC homography temporal alignment changed the motion model, not only a temporal selector threshold: neighbor-frame stop-line segments and dense/envelope candidates are warped into the current raw frame by ORB feature matching plus RANSAC homography before the same train-split MLP/union selector. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only compact CSV/summary artifacts. Smoke train64/fixed-val4 was not decision-useful because validation temporal oracle positives were `0`, although ORB alignment applied to `387` train and `11` val candidate rows. The larger train256/exact-val128 gate was negative: baseline stop-line was `32 / 28 / 28`, F1 `0.5333`; baseline-plus-temporal MLP was `32 / 63 / 28`, F1 `0.4129`; temporal union selector was `30 / 37 / 30`, F1 `0.4724`; and even baseline-plus-oracle temporal reached only `35 / 39 / 25`, F1 `0.5224`. ORB homography had real application counts (`1603` train, `236` val candidate rows) and temporal oracle coverage (`56` val positives), but the no-GT selector lost retained TP and added FP. Broader-val512 is skipped, and this closes image-plane homography temporal alignment as a 0.6 path.

Latest stop-line retained-suppressor + temporal endpoint-envelope two-stage replay tested the allowed pairing of FP-control and new candidate recovery: a line-support suppressor first filters retained projection-comp stop-lines, then sparse-affine endpoint-envelope temporal candidates are added by learned MLP or oracle replay. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, trained real MLPs, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was not decision-useful because validation temporal oracle positives were `0`. Exact train256/val128 showed the split clearly: baseline stop-line was `32 / 28 / 28`, F1 `0.5333`; suppress-only improved precision but lost recall at `29 / 16 / 31`, F1 `0.5524`; learned suppress+temporal regressed to `30 / 32 / 30`, F1 `0.4918`; oracle suppress+temporal reached `33 / 20 / 27`, F1 `0.5841`. The user-requested broader train512/val512 check was also negative: baseline was `125 / 91 / 146`, F1 `0.5133`; suppress-only fell to `107 / 55 / 164`, F1 `0.4942`; learned suppress+temporal was `119 / 123 / 152`, F1 `0.4639`; oracle suppress+temporal was `142 / 84 / 129`, F1 `0.5714`. Broader validation had temporal oracle coverage (`152` positives; `51` endpoint-envelope positives), but the no-GT learned selector still paid too much FP and the suppressor dropped too many retained TP. This closes the same retained-suppressor plus sparse-affine endpoint-envelope selector/training-scale surface as a 0.6 path. Retained artifacts are CSV/summary-only (`884K`, `3.8M`, `8.4M`), and temporary root `yolo26s.pt` was pruned.

Latest stop-line lane-affine temporal alignment tested a different motion premise from image-plane phase/sparse-affine/ORB: current/neighbor predicted lane polylines are matched, a small affine is estimated from lane track correspondence, and neighbor stop-line/envelope/dense-component temporal candidates are warped before the existing train-split MLP/union selector. It reused `seg_dataset/pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was not decision-useful because validation temporal oracle positives were `0`, though lane-affine alignment applied to `23` val neighbor rows. Exact train256/val128 had real candidate coverage and real alignment application: val temporal candidates `386`, oracle positives `64`, endpoint-envelope positives `24`, dense-component positives `21`, and lane-affine applied count `282`. The deployable path failed: primary baseline was stop-line `32 / 28 / 28`, F1 `0.5333`; temporal MLP was `18 / 45 / 42`, F1 `0.2927`; baseline-plus-temporal MLP was `33 / 63 / 27`, F1 `0.4231`; temporal union selector was `28 / 31 / 32`, F1 `0.4706`. Even baseline-plus-oracle temporal was only `36 / 40 / 24`, F1 `0.5294`, below the primary baseline. Broader-val512 is skipped. Temporary root `yolo26s.pt` was pruned; retained artifacts are compact (`1.9M`, `8.5M`).

Latest stop-line temporal pair-consensus candidate source tested the allowed learned temporal segment-emitter premise without changing the retained lane/crosswalk path: opposite-offset sparse-affine temporal candidates (`-1` and `+1`) must agree on axis and normal position before a new along-axis consensus segment is emitted, then the existing train-split temporal MLP and retained+temporal union selector provide FP control. It reused `seg_dataset/pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, created no dataset copy, saved only CSV/summary artifacts, and pruned temporary root weights. Smoke train64/fixed-val4 created train pair-consensus coverage (`103` pair candidates, `47` positives) but had no validation pair or temporal positives, so it was not decision-useful. Exact train256/val128 did create validation pair-consensus coverage (`28` pair candidates, `4` positives) plus temporal coverage (`419` temporal candidates, `66` positives), but deployable learned paths still failed versus primary projection-comp: baseline was stop-line `32 / 28 / 28`, F1 `0.5333`; temporal MLP was `15 / 45 / 45`, F1 `0.2500`; baseline-plus-temporal MLP was `33 / 61 / 27`, F1 `0.4286`; temporal union selector was `30 / 33 / 30`, F1 `0.4878`. Even baseline-plus-oracle temporal was only `37 / 41 / 23`, F1 `0.5362`, a tiny F1 gain with too much FP. Broader-val512 is skipped because exact learned TP/FP/FN is below primary.

Latest stop-line lane-homography temporal alignment tested the projective predicted-lane motion model that was implemented after the lane-affine failure. It samples matched current/neighbor lane tracks, estimates a RANSAC homography, and warps neighbor stop-line candidates through the existing homography path before the same train-split MLP verifier. It reused `seg_dataset/pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, created no dataset copy, and saved only compact CSV/summary artifacts. Smoke train64/fixed-val4 was not decision-useful because validation temporal oracle positives were `0`, though train had `204` temporal candidates, `59` positives, and lane-homography applied to `79` train / `3` val candidate rows. Exact train256/val128 was negative: validation temporal candidates were `98` with `19` positives and homography applied count `47`, but primary baseline stayed stop-line `32 / 28 / 28`, F1 `0.5333`; temporal MLP alone was `9 / 14 / 51`, F1 `0.2169`; baseline-plus-temporal MLP was `32 / 40 / 28`, F1 `0.4848`; baseline-plus-oracle temporal was only `34 / 32 / 26`, F1 `0.5397`. Broader-val512 is skipped because exact learned TP/FP/FN is below primary and oracle headroom is tiny. Temporary root `yolo26s.pt` was pruned; retained artifacts are compact (`640K`, `2.8M`).

Latest lane area-ROI gap-fill union selector tested a materially different no-GT set-occupancy signal on the known dropped-candidate rescue surface: retained lanes are preserved, and dropped candidates can enter only when they look like they fill a lateral gap in the retained lane set. It reused `seg_dataset/pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, created no dataset copy, saved only CSV/summary artifacts, and pruned the temporary root `yolo26s.pt`. Smoke train64/fixed-val4 was negative: baseline lane `40 / 11 / 46`, F1 `0.5839`, moved to `42 / 20 / 44`, F1 `0.5676`. The selector admitted `11` dropped-area candidates, but only `3` were oracle-positive and `8` were negative, so gap-fill support did not provide enough FP-control. Exact/broader scaling is skipped because the first gate lost F1 and added FP faster than TP.

Latest stop-line lane-pair topology candidate probe tested a materially different no-GT candidate-generation contract: predicted lane polyline pairs create stop-line segments, and dense stop-line proposal maps choose additional y-anchors before a train-split MLP verifier scores candidates. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. The naive fixed-y train64 smoke had too little coverage (`2` train oracle positives, `0` val positives), so the same axis was upgraded to dense-anchor generation. Dense-anchor train64 improved train oracle positives to `45`, but fixed val4 still had no oracle-positive candidates. The larger train256/exact-val128 gate was negative: baseline stop-line was `32 / 28 / 28`, F1 `0.5333`; lane-pair candidate-only was `7 / 75 / 53`, F1 `0.0986`; baseline-plus-lane-pair MLP was `31 / 74 / 29`, F1 `0.3758`; even baseline-plus-oracle lane-pair was only `36 / 37 / 24`, F1 `0.5414`. The oracle upper bound adds only `+4` TP for `+9` FP over baseline, so broader-val512 is skipped. Do not repeat this as lane-pair y-fraction, dense-anchor count/stride, verifier threshold, hidden size, train-batch scaling, or baseline merge sweep unless the lane-derived candidate geometry changes materially.

Latest stop-line flip-consistency source-router tested a new no-GT source-quality signal rather than another output-stat/dense/profile/lane-topology sweep: each primary/specialist/source-mode candidate group is compared against horizontally flipped replay decoded back into raw coordinates, adding source stability features to the MLP router. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Exact-val128 rejected both train scales: train64 learned-router stop-line was `30 / 27 / 30`, F1 `0.5128`, and train256 was `30 / 26 / 30`, F1 `0.5172`, both below primary projection-comp `32 / 28 / 28`, F1 `0.5333`. Oracle routing stayed high at `35 / 2 / 25`, F1 `0.7216`, so source-choice headroom remains, but flip replay consistency does not learn a deployable selector. Broader-val512 is skipped. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `64K` each.

Latest stop-line dense-map union selector tested a stricter learned set contract over both retained projection-comp stop-lines and dense-map set-decoder candidates: instead of preserving baseline and appending verified decoder rows, the train-split MLP scores one union pool and emits only the selected fixed-size set. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was flat for union selector (`0 / 3 / 2`, F1 `0.0000`) while unverified baseline-plus-decoder added FP (`0 / 7 / 2`). The larger train256/exact-val128 gate was strongly negative: baseline stop-line was `32 / 28 / 28`, F1 `0.5333`; decoder-only was `7 / 70 / 53`, F1 `0.1022`; baseline-plus-decoder was `32 / 88 / 28`, F1 `0.3556`; union selector was only `17 / 57 / 43`, F1 `0.2537`. Lane/crosswalk stayed unchanged at exact `0.5660 / 0.5988`, but the stop-line selector rejected too many retained positives and selected too many decoder negatives, so broader-val512 is skipped. Temporary root `yolo26s.pt` and the failed first smoke scratch directory were pruned; retained artifacts are about `76K` and `340K`.

Latest lane ridge-path DP repair tested a different no-GT geometry-repair contract for already decoded retained lanes: a train-split repairability ranker selects candidate unmatched lanes, then dynamic programming follows a coherent local dense centerline/support ridge across the whole polyline instead of pointwise snapping or translation. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. The initial train64 ranker was weak (`repairable_le120_any_center` OOF AUC/AP `0.4412 / 0.0679`), and fixed val4 stayed flat after selecting `4` candidates. A larger train256 audit/ranker was then built on the same root with the retained `flip_centerline_avg_lane_cross_comp050` runtime path; it improved the ranker to OOF AUC/AP `0.6286 / 0.1506` over `2099` unmatched train predictions with `176` repairable labels, but fixed val4 still stayed flat. Exact-val128 confirmed the axis is negative: `125` selected lanes all moved, yet lane regressed from `1202 / 491 / 1188`, F1 `0.5888`, to `1201 / 492 / 1189`, F1 `0.5883`. Stop-line/crosswalk stayed unchanged at exact `0.4483 / 0.5988`. Broader-val512 is skipped because exact lost TP and added FP; do not repeat this as ridge radius, smoothness, topK, or ranker-label scaling on the same candidate surface.

Latest lane area-ROI row-peak verifier tested a materially different no-GT instance-quality feature on the existing dropped-candidate rescue surface: each candidate lane is scored by whether its row-wise x positions sit on local dense centerline/support peaks, not just broad line-ROI support or side-band contrast. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was weak-positive: lane moved `40 / 11 / 46`, F1 `0.5839`, to `43 / 16 / 43`, F1 `0.5931`. The larger train256/exact-val128 gate stayed below target and below the previous area-ROI frontier: lane moved `1202 / 491 / 1188`, F1 `0.5888`, to `1246 / 552 / 1144`, F1 `0.5950`; TP rose `+44`, but FP rose `+61`. Stop-line/crosswalk stayed unchanged at exact `0.5333 / 0.5988`. Train384/train512 larger attempts were tried without copying data, but the process was killed by the OS during val collection (`137`) before summaries were produced, so they are not retained as metrics. Broader-val512 is skipped because exact remains below `0.60` and FP still grows faster than TP. Temporary root `yolo26s.pt` and failed larger-run scratch folders were pruned; retained artifacts are about `48K` and `212K`.

Latest stop-line baseline-slot refiner changed the dense-map decoder contract from global free queries to baseline-aware slots: projection-comp baseline lines become trainable slots and dense top-support fallback anchors can add missing candidates, with objectness plus endpoint refinement learned from train split only. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Fallback slot labels were distance-gated in raw space before the final run, so unmatched GT is not assigned just by order. Smoke train64/fixed-val4 was negative: baseline stop-line `0 / 3 / 2`, baseline-plus-slot-refiner `0 / 5 / 2`. The larger train256/exact-val128 gate was also negative: baseline `32 / 28 / 28`, F1 `0.5333`; slot-refiner-only `9 / 47 / 51`, F1 `0.1552`; baseline-plus-slot-refiner `32 / 81 / 28`, F1 `0.3699`. TP did not improve and FP moved `+53`, so broader-val512 is skipped. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `28K` and `84K`.

Latest lane conditional-union quality verifier tested the allowed conditional-row reopen premise with a learned no-GT instance-quality/matching contract rather than appending conditional rows directly. `tools/probe_pv26_lane_area_roi_verifier.py` can now score retained row-scan lanes and model-emitted `lane_conditional_rows` together, then use the existing fixed-count `select_topk_union_geometry` replay so lane count does not grow. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was negative for both BCE and sample-pairwise ranking losses: lane moved `34 / 15 / 52 -> 30 / 19 / 56`, F1 `0.5037 -> 0.4444`. Because the user requested a larger-range check, a train256/exact-val128 pairwise run was also executed; it remained negative, moving lane `1090 / 423 / 1300`, F1 `0.5585`, to `1087 / 426 / 1303`, F1 `0.5570`. Stop-line/crosswalk stayed unchanged at exact `0.5299 / 0.6135`. Broader-val512 is skipped because exact TP fell and FP rose. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `264K`, `264K`, and `2.1M`.

Latest lane conditional teacher-runtime row target trained the existing conditional row branch from retained runtime teacher lane rows instead of encoded GT rows, without enabling conditional-row emit during training validation. `PV26MultiTaskLoss` now lets conditional row/seed auxiliary losses consume `lane_family_query_target_source=teacher_runtime`, and `tools/run_pv26_lane60_probe.py` has a `lane_conditional_teacher_runtime_row` preset. A real CUDA `2x64` smoke train reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, had skipped steps `0`, and created no dataset copy. Fixed val4 epoch-2 with conditional rows enabled was lane/stop/cross `0.5000 / 0.0000 / 0.5455`, TP/FP/FN lane `33 / 13 / 53`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`; disabling conditional rows produced the same metrics. This means the teacher-runtime row training path is mechanically valid but did not make runtime conditional rows contribute any matched lane TP, so exact-val128, broader-val512, and larger training are skipped. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `13M`.

Latest stop-line HAF candidate-verifier tested the previously allowed HAF reopen premise with a different candidate surface and FP-control contract: decoded HAF consensus segments are not used by threshold alone; a train-split no-GT verifier scores them, and validation preserves the projection-comp baseline while adding only verified HAF candidates. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was negative: baseline stop-line `0 / 3 / 2`, baseline-plus-verified-HAF `0 / 5 / 2`. The larger train256/exact-val128 gate was also negative on the HAF checkpoint: baseline `27 / 32 / 33`, F1 `0.4538`; raw baseline-plus-HAF `30 / 66 / 30`, F1 `0.3846`; verifier-filtered baseline-plus-HAF `29 / 49 / 31`, F1 `0.4203`. The verifier added `+2` TP but `+17` FP versus its baseline and stayed far below the primary projection-comp exact reference `32 / 28 / 28`, F1 `0.5333`; broader-val512 is skipped. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `56K` and `168K`.

Recent stop-line endpoint-envelope source-router changed source geometry without copying data or changing the retained lane/crosswalk path: when primary projection-comp and specialist lines pair, it emits an `endpoint_envelope` source spanning the min/max along-axis extent across both sources instead of averaging endpoints. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was stop-line-uninformative but did not add FP over primary (`0 / 3 / 2`). The larger train256/exact-val128 gate rejected it: primary stayed `32 / 28 / 28`, F1 `0.5333`; fixed `endpoint_envelope` was `30 / 30 / 30`, F1 `0.5000`; learned-router was `32 / 29 / 28`, F1 `0.5289`. Oracle routing remained high at `35 / 2 / 25`, F1 `0.7216`, so source headroom remains, but this no-GT envelope geometry and current router features do not learn a deployable selector. Broader-val512 is skipped. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `60K` and `64K`.

Latest lane TTA-consistency union-pool verifier added a new no-GT instance-quality feature to the retained+dropped lane candidate pool: each lane candidate is compared against an alternate same-sample decode (`baseline` versus retained flip-centerline average) and the verifier receives nearest alternate retained/raw-candidate consistency features. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 looked promising, moving lane `40 / 11 / 46 -> 42 / 13 / 44`, F1 `0.5839 -> 0.5957`, while stop-line/crosswalk stayed unchanged. The required larger train256/exact-val128 check rejected it: lane moved `1202 / 491 / 1188 -> 1213 / 606 / 1177`, F1 `0.5888 -> 0.5764`; TP increased only `+11` while FP increased `+115`. Stop-line and crosswalk stayed at `0.5333 / 0.5988`. Broader-val512 is skipped because exact FP control fails. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `96K` and `632K`.

Latest stop-line dense-map decoder candidate-verifier tested the allowed calibrated-verifier/baseline-preserving merge premise after the dense-map set decoder added TP with severe FP. The decoder still emits learned dense-map stop-line candidates, but a second train-split MLP verifier scores every decoder query, and validation preserves the retained projection-comp baseline while adding only verified decoder candidates. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was immediately FP-negative: baseline stop-line `0 / 3 / 2`, baseline-plus-verified `0 / 7 / 2`. The larger train256/exact-val128 confirmed the verifier is insufficient: baseline `32 / 28 / 28`, F1 `0.5333`; baseline-plus-verified `34 / 86 / 26`, F1 `0.3778`. The verifier removed only `4` FP versus unverified baseline-plus-decoder (`34 / 90 / 26`) and still added `+58` FP for `+2` TP. Lane/crosswalk stayed unchanged at `0.5888 / 0.5988`. Broader-val512 is skipped, and dense-map decoder verifier work is closed unless the candidate generator itself changes materially. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `52K` and `212K`.

Latest stop-line retained-output suppressor tested the allowed baseline-preserving FP-control premise after repeated new-candidate decoders failed. It trains a small no-GT MLP on train-split retained stop-line candidates, using candidate geometry plus frozen dense `mask/center/selector/axis-valid/row/x` line-support features, then removes low-confidence runtime candidates only; it never emits new stop-lines. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 was flat: stop-line stayed `0 / 3 / 2`, with all three negative candidates kept. The larger train256/exact-val128 run was only marginally positive by F1 but negative by recall: stop-line moved from `32 / 28 / 28`, F1 `0.5333`, to `27 / 13 / 33`, F1 `0.5400`. The verifier dropped `15` negatives but also dropped `5` positives; lane and crosswalk stayed unchanged at `0.5888 / 0.5988`. Broader-val512 is skipped because suppress-only cannot recover the missing stop-line TP needed for `0.60`, and this exact result trades away recall. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `28K` and `68K`.

Latest stop-line dense-map metric-quality objectness decoder reopened the dense-map set decoder only under the allowed stronger segment-quality premise: matched query objectness is no longer a hard `1.0`, but `exp(-endpoint_loss / tau)` so badly localized segments should not learn high confidence. It still reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, created no dataset copy, and saved only CSV/summary artifacts. Smoke train64/fixed-val4 stayed negative: baseline stop-line `0 / 3 / 2`, decoder-only `0 / 4 / 2`, baseline-plus-decoder `0 / 7 / 2`. The larger train256/exact-val128 gate also failed: baseline stop-line `32 / 28 / 28`, F1 `0.5333`; baseline-plus-decoder `33 / 93 / 27`, F1 `0.3548`. TP rose by only `+1` while FP rose by `+65`, so broader-val512 is skipped. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `28K` and `80K`.

Recent stop-line dense-map set decoder changed the stop-line runtime contract from candidate re-ranking to a learned structured decoder over frozen dense stop-line maps. A small MLP reads pooled `mask/center/selector/row/x/axis-valid` maps and emits `2` stop-line segments with set-style objectness and endpoint loss. It trained/evaluated on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, created no dataset copy, and saved only CSV/summary artifacts. Train64/fixed-val4 was negative: baseline stop-line stayed `0 / 3 / 2`, decoder-only was `0 / 4 / 2`, and baseline-plus-decoder was `0 / 7 / 2`. Because val4 is tiny, one larger train256/exact-val128 check was run; it recovered `+2` stop-line TP but added `+62` FP, moving stop-line from `32 / 28 / 28`, F1 `0.5333`, to `34 / 90 / 26`, F1 `0.3696`. Broader-val512 is skipped because exact precision collapses. Temporary root `yolo26s.pt` was pruned; retained artifacts are about `28K` and `80K`.

Recent stop-line axis-bin endpoint profile head changed the scalar axis-profile segment head into an endpoint-distribution contract: the sampled stop-line axis profile now predicts start/end bins along the axis before softargmax endpoint decode. It trained real CUDA `2x64` on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, created no dataset copy, and had skipped steps `0`. Fixed val4 rejected both checkpoints: epoch-1 best reached lane/stop/cross `0.5038 / 0.0000 / 0.4000`, TP/FP/FN lane `33 / 12 / 53`, stop-line `0 / 3 / 2`, crosswalk `2 / 1 / 5`; epoch-2 last reached `0.4885 / 0.0000 / 0.5455`, lane `32 / 13 / 54`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`. Exact-val128, broader-val512, and larger training are skipped because the first fixed gate still recovers no stop-line TP and regresses lane. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `16M`.

Recent current-family anchor-query zero-residual vector decoder tested the remaining warm-start/pretraining premise for the current-family query-vector architecture: lane/stop-line/crosswalk predictor residual layers are zero-initialized so runtime starts from anchor geometry templates instead of random residual offsets. It trained real CUDA `2x64` on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, created no dataset copy, and had skipped steps `0`. Fixed val4 rejected both checkpoints: epoch-1 best was lane/stop/cross `0.0000 / 0.0000 / 0.0000`, TP/FP/FN lane `0 / 107 / 86`, stop-line `0 / 0 / 2`, crosswalk `0 / 0 / 7`; epoch-2 last was also all-zero TP with lane `0 / 314 / 86`. Exact-val128, broader-val512, and larger training are skipped because even the first fixed gate recovered no matched lane-family TP. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `16M`.

Latest lane-family det-source confident distill exposure reopened the previous full-root det-source distill path only under a stronger pseudo-label-quality condition: teacher distill applies only on det-source-only rows and only at teacher-positive dense pixels (`distill_confidence_mode=teacher_positive`, threshold `0.65`). It trained real CUDA `2x64` under a static trunk on the existing `429350`-record root, created no dataset copy, and had skipped steps `0`. It is still fixed-val4 negative: best `flip_centerline_avg_lane_cross_comp050` replay gives lane/stop/cross `0.5672 / 0.0000 / 0.5455`, lane TP/FP/FN `38 / 10 / 48`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`. This matches the previous det-source-only sample-selection gate, remains below the retained fixed lane reference `0.5839`, `40 / 11 / 46`, and recovers no stop-line TP. Exact-val128 and broader-val512 are skipped. Negative checkpoints, TensorBoard, and root weights were pruned; retained run size is about `844K`.

Latest stop-line mask-first + det-source hard-negative specialist combined the previously internal-positive mask-first stop-line-only architecture with existing full-root det-source hard negatives. It trained a real CUDA `2x64` run on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, exposed BDD/traffic/obstacle det-source records without copying data, and had skipped steps `0`. Single-checkpoint val4 is not a valid final gate for this stop-line-only architecture because lane/crosswalk are absent, so the real fixed gate used the retained primary lane/crosswalk checkpoint plus source-router comparison on exact-val128. That gate is negative: primary projection-comp was lane/stop/cross `0.5888 / 0.5333 / 0.5988`, stop-line TP/FP/FN `32 / 28 / 28`, while the trained specialist was `0.5888 / 0.4833 / 0.5988`, stop-line `29 / 31 / 31`; learned-router was only `0.4821`, `27 / 25 / 33`. Oracle routing remained high at stop-line `0.6809`, `32 / 2 / 28`, so source headroom still exists, but this combined train/data contract does not learn a deployable no-GT selector or specialist. Broader-val512 and larger training are skipped because exact failed. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `5.2M`, with CSV/summary/history only.

Latest stop-line utility-regression source-router changed the learned source-router objective, not the source features or thresholds: instead of cross-entropy on a single best source label, the MLP predicts each source mode's per-sample stop-line metric utility and chooses the best predicted utility. It trained/evaluated on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, and created no dataset copy. Exact-val128 is negative at both train coverages: train64 learned utility routing was lane/stop/cross `0.5888 / 0.4918 / 0.5988`, stop-line `30 / 32 / 30`; train128 was `0.5888 / 0.4878 / 0.5988`, stop-line `30 / 33 / 30`; both are below primary projection-comp `0.5888 / 0.5333 / 0.5988`, stop-line `32 / 28 / 28`. Oracle and oracle-utility routing still reach stop-line `0.7083`, `34 / 2 / 26`, so the source-choice headroom remains but this output-stat utility objective does not learn a deployable selector. Broader-val512 is skipped, artifacts are CSV/summary-only at about `64K` each, and root `yolo26s.pt` was pruned.

Latest stop-line temporal-neighbor candidate verifier changed candidate source rather than re-ranking the same current-frame sources: neighboring-frame stop-line predictions become candidates, current-frame dense stop-line maps provide no-GT quality features, and a train-split MLP verifier selects temporal candidates. It trained/evaluated on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, and created no dataset copy. Smoke val4 was not decision-useful because validation temporal oracle positives were `0`. Exact-val128 is negative: primary projection-comp baseline was lane/stop/cross `0.5660 / 0.5333 / 0.5988`, stop-line `32 / 28 / 28`; temporal-only was `0.5660 / 0.2340 / 0.5988`, stop-line `11 / 23 / 49`; baseline-plus-temporal MLP was `0.5660 / 0.4823 / 0.5988`, stop-line `34 / 47 / 26`. Oracle temporal union also only reached stop-line `0.5397`, `34 / 32 / 26`, so temporal neighbor candidates add too much FP and have weak exact headroom on this image-space no-ego-motion contract. Broader-val512 is skipped, artifacts are CSV/summary-only at about `240K` and `392K`, and root `yolo26s.pt` was pruned.

Latest stop-line phase-correlation temporal alignment reopened temporal stop-line only under a materially different no-GT alignment contract: raw current/neighbor frames are downsampled, phase-correlated, and the neighbor-frame stop-line candidates are translated before the same train-split verifier/eval. It again reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, and created no dataset copy. Train64 exact-val128 was negative: baseline-plus-temporal MLP was lane/stop/cross `0.5660 / 0.4928 / 0.5988`, stop-line `34 / 44 / 26`, below primary projection-comp `32 / 28 / 28`, F1 `0.5333`; oracle union was only `34 / 33 / 26`, F1 `0.5354`. The user-requested larger train256 exposure was also negative: baseline-plus-temporal MLP was `0.5660 / 0.5116 / 0.5988`, stop-line `33 / 36 / 27`, while oracle union stayed `34 / 33 / 26`, F1 `0.5354`. Broader-val512 is skipped because exact failed at both training coverages. Artifacts are CSV/summary-only (`256K`, `416K`, `1.1M`), no verifier checkpoint was saved, no dataset copy was created, and root weights were pruned.

Latest stop-line crosswalk-hull long-edge candidate verifier reopened the crosswalk-derived stop-line idea under a materially broader candidate surface: predicted crosswalk hulls are converted to PCA long-axis segments, both long-edge sides are expanded over offsets `0,8,16,24,32,48` and length scales `0.85,1.0,1.15`, and a train-split MLP verifier scores the resulting no-GT candidates. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, and created no dataset copy. Smoke train64/val4 had train coverage (`141` scored positives) but no validation positives. The user-requested larger train256/exact-val128 exposure improved candidate coverage (`459 / 5580` train scored positives, `77 / 672` val scored positives), but the deployable selector failed FP-control: primary exact stayed lane/stop/cross `0.5660 / 0.5333 / 0.5988`, stop-line `32 / 28 / 28`; crosswalk-edge-only was only `6 / 35 / 54`, F1 `0.1188`; baseline-plus-crosswalk-edge MLP regressed to `33 / 49 / 27`, F1 `0.4648`; even baseline-plus-oracle-crosswalk-edge was `34 / 34 / 26`, F1 `0.5313`, still below primary. Broader-val512 is skipped because exact failed and oracle union headroom is below the current primary. Artifacts are CSV/summary-only (`2.9M` smoke, `12M` exact), no verifier checkpoint was saved, no dataset copy was created, and root weights were pruned.

Latest stop-line sparse-affine temporal alignment reopened temporal stop-line again under a stronger no-GT image-plane motion contract than phase correlation: sparse optical flow tracks neighbor-frame features into the current frame, estimates an affine transform with RANSAC, warps neighbor-frame stop-line candidates, then trains the same train-split MLP verifier over current-frame dense evidence. It reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, and created no dataset copy. Smoke train64/val4 was not decision-useful because validation temporal oracle positives were `0`. Exact train64/val128 was runtime-negative: baseline-plus-temporal MLP reached lane/stop/cross `0.5660 / 0.5000 / 0.5988`, stop-line `34 / 42 / 26`, below primary projection-comp `32 / 28 / 28`, F1 `0.5333`; oracle union reached `35 / 32 / 25`, F1 `0.5512`. The larger train256 exact run was also runtime-negative: baseline-plus-temporal MLP fell to stop-line `33 / 44 / 27`, F1 `0.4818`, while oracle union was `35 / 31 / 25`, F1 `0.5556`. Broader-val512 is skipped because exact failed after larger train exposure. Artifacts are CSV/summary-only (`264K`, `428K`, `1.2M`), no verifier checkpoint was saved, no dataset copy was created, and root weights were pruned.

Latest stop-line sparse-affine temporal union selector paired that temporal candidate source with FP-control instead of only appending verified temporal rows: retained projection-comp candidates and sparse-affine temporal candidates are placed in one candidate set, assignment labels are built on the train split, and a train-split MLP emits a fixed-size selected set. It reused the same existing `429350`-record root without copying data. Smoke train64/val4 again had no validation positives. The larger train256/exact-val128 gate is only tiny F1-positive against this run's baseline but negative against the real primary gate: temporal-union selector reached lane/stop/cross `0.5660 / 0.5345 / 0.5988`, stop-line `31 / 25 / 29`; this is above the same run's baseline `32 / 31 / 28`, F1 `0.5203`, but it loses one TP versus the primary projection-comp reference `32 / 28 / 28`, F1 `0.5333`. Oracle temporal union was only `35 / 35 / 25`, F1 `0.5385` in this exact run, so broad-val512 is skipped. Artifacts are CSV/summary-only (`596K` smoke, `2.7M` exact), no verifier checkpoint was saved, no dataset copy was created, and root weights were pruned.

Latest stop-line hard-mined distance-heatmap static train combined two previously weak axes, rather than repeating either alone: the train-split hard stop-line sample-id manifest (`92` samples) was fed under `lane_family_stopline_static_trunk`, while stop-line center and selector targets used distance heatmaps. It trained real CUDA `2x64` on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, had skipped steps `0`, and created no dataset copy. Fixed val4 moved stop-line from the retained `0 / 3 / 2` smoke reference to `1 / 5 / 1`, but exact-val128 rejected it: `flip_centerline_avg_lane_cross_comp050` reached lane/stop/cross `0.5888 / 0.2825 / 0.5988`, stop-line `25 / 92 / 35`, far below projection-comp exact `31 / 29 / 29`, F1 `0.5167`, and primary exact `32 / 28 / 28`, F1 `0.5333`. Broader-val512 and larger training are skipped because the exact gate failed by adding too many FP and losing TP. Negative checkpoints, TensorBoard, and temporary root `yolo26s.pt` were pruned; retained run size is about `1.3M`.

Latest stop-line lane-topology utility-regression source-router combined two previously separate router ideas without copying data: the router used lane-topology source-quality features (`293` dims) but trained against per-source metric utility instead of a single best-source CE label. It trained on `256` train batches from the existing `429350`-record root and evaluated exact-val128. The oracle utility route still shows source-choice headroom at stop-line `33 / 2 / 27`, F1 `0.6947`, but the learned utility router is negative: lane/stop/cross `0.5888 / 0.5082 / 0.5988`, stop-line `31 / 31 / 29`, below the same run's primary/projection-comp `31 / 29 / 29`, F1 `0.5167`, and below the primary exact reference `32 / 28 / 28`, F1 `0.5333`. Broader-val512 is skipped. Output is CSV/summary only (`64K`), no verifier checkpoint was saved, no dataset copy was created, and root `yolo26s.pt` was pruned.

Latest lane-family det-source distill-only exposure changed full-root data feeding without treating unlabeled det-only records as empty lane-family negatives: det-source-only BDD/traffic/obstacle rows enter the train view only when dense teacher distill is enabled, while validation remains lane-family-only. It trained real CUDA `2x64` under a static trunk on the existing `429350`-record root, with logged train source counts showing `1` det-source-only sample plus `3` lane/stop/cross source samples in sampled batches, and no dataset copy. It is fixed-val4 negative despite internal phase objective `0.6451`: the fixed `flip_centerline_avg_lane_cross_comp050` replay gives lane/stop/cross `0.5373 / 0.0000 / 0.5455`, lane TP/FP/FN `36 / 12 / 50`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`, versus retained fixed lane `0.5839`, `40 / 11 / 46`. Exact-val128 and broader-val512 are skipped because the first fixed gate loses `4` lane TP, adds `1` lane FP, and recovers no stop-line TP. Negative checkpoints, TensorBoard, and root weights were pruned; retained run size is about `836K`.

Latest lane set-context union-pool verifier changed the lane candidate selection model from independent MLP scoring to a sample-local set-context transformer over retained+dropped candidates, then reused the fixed-count union-geometry selection contract. It trained on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root in place, indexed `429350` records, and created no dataset copy. CUDA train64/fixed-val4 was flat: lane/stop/cross stayed `0.5839 / 0.0000 / 0.5455`, lane TP/FP/FN stayed `40 / 11 / 46`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`. The verifier selected `51` candidates, all retained lanes, selected `0` dropped-area candidates, and left `5` dropped-area oracle-positive candidates unselected. Exact-val128, broader-val512, and larger training are skipped because the first fixed gate had zero TP/FP/FN movement. Output artifact size is about `96K`; no verifier checkpoint was saved, root `yolo26s.pt` was pruned, and no dataset copy was created.

Latest stop-line crosswalk-context fusion train changed the stop-line dense head layer contract again, not a postprocess threshold: stop-line dense features get an opt-in zero-initialized residual from detached crosswalk mask/center probability maps. It trained a real CUDA `2x64` stop-line-only static-trunk smoke on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, and created no dataset copy. Fixed val4 epoch-1 best was negative: lane/stop/cross `0.5507 / 0.0000 / 0.5455`, TP/FP/FN lane `38 / 14 / 48`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`. Fixed exact-val128 was also below the stop-line gate: lane/stop/cross `0.5660 / 0.4590 / 0.5988`, stop-line TP/FP/FN `28 / 34 / 32`, below projection-competition exact `0.5167` (`31 / 29 / 29`) and primary exact `0.5333` (`32 / 28 / 28`). Broader-val512 and larger training are skipped because exact failed. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `15M`.

Latest current-family self-conditioned geometry-refine vector decoder changed the vector-query architecture again, not postprocess thresholds: the first dense-seed geometry prediction is fed back as a detached geometry-conditioned query input, then the same lane/stop-line/crosswalk vector decoders run a second refinement pass. It trained a real CUDA `2x64` smoke on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, and created no dataset copy. The initial implementation hit an autograd in-place version error; the fixed path detaches the self-conditioned geometry before normalization, then completed training with skipped steps `0`. Fixed val4 epoch-2 is still a hard reject: lane/stop/cross `0.0000 / 0.0000 / 0.0000`, TP/FP/FN lane `0 / 52 / 86`, stop-line `0 / 0 / 2`, crosswalk `0 / 0 / 7`, phase objective `0.2316818087`. Exact-val128, broader-val512, and larger training are skipped because the first fixed gate recovered no matched lane-family TP. The failed run was removed; the retained run is CSV/summary/history only, about `8.4M`, with checkpoints, TensorBoard, and root `yolo26s.pt` / `yolo26n.pt` pruned.

Latest stop-line endpoint-fusion source-router changed the source candidate geometry itself: when primary projection-comp and stop-line specialist lines agree, it aligns endpoints and averages them into a new `endpoint_fusion` source mode before router/oracle selection. It trained real MLP routers on the existing `pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, and created no dataset copy. Train64 smoke preserved lane/crosswalk and remained stop-line-uninformative (`0 / 3 / 2`). Train64 exact-val128 was negative: fixed `endpoint_fusion` was `31 / 29 / 29`, F1 `0.5167`, and learned-router was `30 / 28 / 30`, F1 `0.5085`, both below primary projection-comp `32 / 28 / 28`, F1 `0.5333`. The larger train256 exact audit recovered primary TP but still added one FP: learned-router `32 / 29 / 28`, F1 `0.5289`; fixed `endpoint_fusion` stayed `31 / 29 / 29`, F1 `0.5167`; oracle routing remained `34 / 2 / 26`, F1 `0.7083`. Broader-val512 is skipped. Artifacts are CSV/summary only, `56K / 56K / 56K`, and root `yolo26s.pt` / `yolo26n.pt` were pruned.

Latest stop-line lane-topology source-router changed the two-checkpoint router quality signal again: instead of output stats, dense rasters, raw patches, or line profiles, it appends no-GT geometry features measuring each stop-line source against retained lane predictions. It trained real MLP routers on the existing `pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, and created no dataset copy. Train64 smoke preserved lane/crosswalk and was stop-line-uninformative because primary/specialist/oracle all had `0` TP. Train64 exact-val128 was negative: learned-router stop-line `30 / 27 / 30`, F1 `0.5128`, below primary projection-comp `32 / 28 / 28`, F1 `0.5333`. The larger train256 exact audit recovered the same TP as primary but added one FP: learned-router `32 / 29 / 28`, F1 `0.5289`, still below primary, while oracle routing remained `34 / 2 / 26`, F1 `0.7083`. Broader-val512 is skipped. Artifacts are CSV/summary only, `48K / 52K / 52K`, and root `yolo26s.pt` / `yolo26n.pt` were pruned.

Latest stop-line line-profile source-router changed the two-checkpoint router quality signal from scalar summaries / low-res rasters to fixed along-axis raw+dense profiles sampled on the primary, specialist, union, and agreement stop-line sources. It trained real MLP routers on the existing `pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, and created no dataset copy. Train64 smoke preserved lane/crosswalk and stayed stop-line-flat (`0 / 3 / 2`). Train64 exact-val128 was negative: learned-router stop-line was `25 / 23 / 35`, F1 `0.4630`, below primary projection-comp `32 / 28 / 28`, F1 `0.5333`, while oracle routing remained `34 / 2 / 26`, F1 `0.7083`. The user-requested larger train256 exact audit improved FP but still lost TP: learned-router stop-line `25 / 20 / 35`, F1 `0.4762`, below primary. Broader-val512 is skipped. Artifacts are CSV/summary only, `48K / 52K / 52K`, and root `yolo26s.pt` was pruned.

Latest lane union-pool ranker changed the dropped-candidate lane verifier contract instead of retuning append/suppress thresholds. It trains one no-GT line-ROI MLP over retained row-scan lanes plus raw bbox/area-dropped lane candidates, then reselects a fixed-count lane set from the union pool. It reused the existing `pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, and created no dataset copy. The first plain fixed-count gate regressed fixed val4 lane `0.5839 -> 0.5547`, TP/FP/FN `40 / 11 / 46 -> 38 / 13 / 48`. A follow-up geometry-aware fixed-count contract added retained-lane safety bias plus no-GT set-geometry support. Train64 smoke became weak-positive (`0.5839 -> 0.5957`, `40 / 11 / 46 -> 42 / 13 / 44`), but exact-val128 failed: train64 and the larger train256 audit both moved lane `0.5888 -> 0.5769`, TP/FP/FN `1202 / 491 / 1188 -> 1214 / 605 / 1176`. Stop-line/crosswalk stayed unchanged. Broader-val512 is skipped because exact failed and FP grew much faster than TP. Retained artifacts are CSV/summary only, about `92K / 448K / 628K`, and the temporary root `yolo26s.pt` was pruned.

Latest stop-line hard-mined sample-id sampler changed the data-feeding contract using train-split failure evidence rather than copying data or sweeping generic positive sampling. It mined `92` hard stop-line train samples from the retained train candidate-feature CSV (`gt_stop_line_count>0` and either no oracle-positive candidate or min nearest-GT distance `>35px`), added a reusable `sample_ids:<manifest.csv>` sampler, and trained a real CUDA `2x64` stop-line-only static-trunk smoke on the existing `pv26_exhaustive_od_lane_dataset`, indexing `429350` records in place with skipped steps `0`. Fixed val4 recovered no stop-line TP: best retained lane variant lane/stop/cross `0.5839 / 0.0000 / 0.5455`, TP/FP/FN `40 / 11 / 46`, `0 / 3 / 2`, `3 / 1 / 4`. Fixed exact-val128 was also below the projection-comp gate: lane/stop/cross `0.5888 / 0.4576 / 0.5988`, stop-line TP/FP/FN `27 / 31 / 33`, versus projection-competition exact `0.5167` (`31 / 29 / 29`) and primary exact `0.5333` (`32 / 28 / 28`). Broader-val512 and larger training were skipped because the exact gate failed. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `1.3M`, tracked manifest is about `8K`, and the full generated manifest artifact is about `36K`.

Latest stop-line segment metric-objectness static train changed the segment-set training contract rather than a postprocess threshold: primary segment objectness can now use nearest-GT endpoint-distance metric quality instead of hard Hungarian matched/unmatched labels. It trained a real CUDA `2x64` stop-line-only static-trunk smoke on the existing `pv26_exhaustive_od_lane_dataset`, indexed `429350` records in place, had skipped steps `0`, and created no dataset copy. Fixed val4 recovered no stop-line TP: best retained lane variant lane/stop/cross `0.5839 / 0.0000 / 0.5455`, TP/FP/FN `40 / 11 / 46`, `0 / 3 / 2`, `3 / 1 / 4`. Fixed exact-val128 was below the projection-comp gate: lane/stop/cross `0.5888 / 0.4628 / 0.5988`, stop-line TP/FP/FN `28 / 33 / 32`, versus projection-competition exact `0.5167` (`31 / 29 / 29`) and primary exact `0.5333` (`32 / 28 / 28`). Broader-val512 and larger training were skipped because the exact gate failed. Negative checkpoints, TensorBoard, and root `yolo26n.pt` / `yolo26s.pt` were pruned; retained run size is about `1.3M`.

Latest stop-line lane-context fusion train changed the stop-line dense head layer contract rather than a postprocess threshold: stop-line dense features get an opt-in zero-initialized residual from frozen lane centerline/support probability maps. It trained a real CUDA `2x64` stop-line-only static-trunk smoke on the existing `pv26_exhaustive_od_lane_dataset`, indexed `429350` records in place, had skipped steps `0`, and created no dataset copy. Fixed val4 preserved the retained lane/crosswalk runtime exactly but recovered no stop-line TP: lane/stop/cross `0.5839 / 0.0000 / 0.5455`, TP/FP/FN `40 / 11 / 46`, `0 / 3 / 2`, `3 / 1 / 4`. Fixed exact-val128 was also negative: lane/stop/cross `0.5888 / 0.4590 / 0.5988`, stop-line TP/FP/FN `28 / 34 / 32`, below projection-competition exact `0.5167` and primary exact `0.5333`. Broader-val512 and larger training were skipped because the exact gate failed. Negative checkpoints, TensorBoard, and root `yolo26n.pt` / `yolo26s.pt` were pruned; retained run size is about `1.3M`.

Latest lane conditional denoise hard-negative row training added train-only feature-backed negative denoise seeds next to GT lane bottom anchors, so the conditional row MLP sees plausible off-lane seeds as objectness `0` examples instead of only zero-feature invalid slots. It trained a real CUDA `2x64` smoke checkpoint on the existing `pv26_exhaustive_od_lane_dataset`, indexed `429350` records in place, had skipped steps `0`, and created no dataset copy. It is smoke-flat/negative: fixed val4 epoch-2 best with `flip_centerline_avg_lane_cross_comp050` stayed lane/stop/cross `0.5839 / 0.0000 / 0.5455`, TP/FP/FN lane `40 / 11 / 46`, stop-line `0 / 3 / 2`, and crosswalk `3 / 1 / 4`. Exact-val128, broader-val512, and larger training were skipped because the new learned hard-negative signal did not move runtime TP/FP/FN. Negative checkpoints, TensorBoard, and root `yolo26n.pt` / `yolo26s.pt` were pruned; retained run size is about `772K`.
...
Latest current-family dense-seeded query decoder changed the proposal contract more materially: each task gets a supervised dense seed heatmap, top-K seed locations gather decoder memory features, and those features seed the same current-family vector heads. It trained a real CUDA `2x64` smoke on the existing canonical root, indexed `429350` records, had skipped steps `0`, and created no dataset copy. It is still strongly smoke-negative: phase-4 val4 epoch-2 reached lane/stop/cross `0.0000 / 0.0000 / 0.0000`, lane TP/FP/FN `0 / 97 / 86`, stop-line `0 / 0 / 2`, and crosswalk `0 / 0 / 7`. Dense seed supervision removed the anchor-query-seed crosswalk FP but did not create any matched lane-family object and made lane FP-heavy, so exact-val128, broader-val512, and larger training were skipped. Negative checkpoints, TensorBoard, and root `yolo26n.pt` / `yolo26s.pt` were pruned; retained run size is about `348K`.

Latest current-family metric-objectness vector decoder tested the next permitted proposal-to-metric quality contract: matched query objectness can now use raw metric quality instead of the previous `0.35` positive floor. It trained a real CUDA `2x64` smoke on the same existing canonical root, indexed `429350` records, had skipped steps `0`, and created no dataset copy. It is also strongly smoke-negative: phase-4 val4 epoch-2 reached lane/stop/cross `0.0000 / 0.0000 / 0.0000`, lane TP/FP/FN `0 / 0 / 86`, stop-line `0 / 0 / 2`, and crosswalk `0 / 0 / 7`; epoch-1 also had all task TP at `0`. Exact-val128, broader-val512, and larger training were skipped because lowering the objectness target floor suppressed unmatched predictions but still created no matched lane-family object. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `348K`.

Latest current-family dense-seed + denoise vector decoder tested the next decoder warm-start contract: runtime queries still come from supervised dense seed top-K memory features, while train-only GT-noised geometry queries exercise the same vector decoders. It trained a real CUDA `2x64` smoke on the same existing canonical root, indexed `429350` records, had skipped steps `0`, and created no dataset copy. It is strongly smoke-negative: phase-4 val4 epoch-1 had lane/stop/cross TP all `0` with lane/stop-line FP `23 / 5`, and epoch-2 collapsed to lane/stop/cross `0.0000 / 0.0000 / 0.0000`, TP/FP/FN lane `0 / 0 / 86`, stop-line `0 / 0 / 2`, crosswalk `0 / 0 / 7`. Exact-val128, broader-val512, and larger training were skipped because the denoise warm-start did not create any matched lane-family object. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `348K`.

Latest current-family teacher-runtime vector target tested the last documented current-family reopen premise: train the query-vector losses from retained runtime teacher predictions instead of encoded GT set rows. It adds an opt-in teacher cache path that decodes the native teacher with retained row-scan/projection-comp/hull postprocess and re-encodes those runtime predictions as lane/stop-line/crosswalk query targets. Corrected real CUDA `2x64` smoke reused the existing canonical root, indexed `429350` records, had skipped steps `0`, and created no dataset copy. It is still strongly smoke-negative: phase-4 val4 epoch-1 reached lane/stop/cross `0.0000 / 0.0000 / 0.0000`, TP/FP/FN lane `0 / 1 / 79`, stop-line `0 / 0 / 2`, crosswalk `0 / 0 / 5`; epoch-2 stayed TP-zero and became lane-FP-heavy at lane `0 / 205 / 86`, stop-line `0 / 0 / 2`, crosswalk `0 / 0 / 7`. Exact-val128, broader-val512, and larger training were skipped because the teacher-runtime target failed the first fixed gate. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained corrected run size is about `356K`.
...
Latest co-occurrence stop/cross lane-frozen retention smoke kept the co-occurring lane/stop-line/crosswalk positive sampler but froze the lane head with `lane_family_stop_cross_heads_only`, so the changed axis was stop-line/crosswalk exposure under lane-retention pressure. It reused the existing `pv26_exhaustive_od_lane_dataset` in place, indexed `429350` records, and train history confirmed every logged batch had `lane/stop_line/crosswalk_source_samples=4` with no det-source samples. It is still smoke-negative: fixed val4 with `flip_centerline_avg_lane_cross_comp050` reached lane/stop/cross `0.5547 / 0.0000 / 0.5455`, lane TP/FP/FN `38 / 13 / 48`, stop-line `0 / 3 / 2`, and crosswalk `3 / 1 / 4`. A same-turn cooccur heads-only rerun was also negative at lane/stop/cross `0.5152 / 0.0000 / 0.5455`. Exact-val128, broader-val512, and larger training were skipped because both recovered no stop-line TP and stayed below the retained fixed lane reference. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run sizes are about `796K` and `808K`.
...
Latest lane repair hypothesis selector trained a no-GT selector over a fixed bank of deterministic lane repairs (`translate_x`, local snap, affine snap, component-row projection, row-profile softargmax) instead of relying on one repair mode or a direct residual MLP. It reused the existing canonical dataset root in place and created no checkpoint. Fixed val4 was not informative because the candidate bank had `0 / 255` oracle-positive hypotheses; the selector picked `4 / 0` oracle-positive candidates and lane stayed flat at `40 / 11 / 46`, F1 `0.5839`. Exact-val128 did contain candidate headroom (`60 / 8465` oracle-positive hypotheses), but the learned selector chose only `31` candidates with `1` oracle-positive and damaged lane `0.5888 -> 0.5873`, TP/FP/FN `1202 / 491 / 1188 -> 1199 / 494 / 1191`. Stop-line/crosswalk stayed `0.5333 / 0.5988`. Broader-val512 was skipped because exact lost lane TP and added FP. Retained artifacts are CSV/summary-only, about `392K` smoke and `2.0M` exact, with no dataset copy.
...
Latest lane area-ROI ensemble-stability verifier added a no-GT uncertainty signal over the same dropped-candidate MLP: three independently seeded verifier members are trained on the existing canonical root, and runtime selection uses `mean_minus_std` probability so unstable candidates are penalized. It reused the existing `429350`-record dataset root in place and wrote only CSV/summary artifacts. Smoke train64/val4 was positive (`0.5839 -> 0.5915`, lane TP/FP/FN `40 / 11 / 46 -> 42 / 14 / 44`). Train384 exact-val128 slightly improved the area-ROI exact frontier (`0.5888 -> 0.5963`, `1202 / 491 / 1188 -> 1240 / 529 / 1150`) but still missed `0.60`. Broader-val512 was not enough: lane moved `0.5628 -> 0.5706`, `4532 / 2097 / 4945 -> 4690 / 2273 / 4787`, below the previous area-ROI broader `0.5821`; stop-line and crosswalk stayed unchanged. This closes ensemble stability as a standalone FP-control path.

Latest lane retained-instance suppressor changed the lane verifier from dropped-candidate append to suppress-only scoring over the retained row-scan lane instances themselves. It trained on canonical train predictions from the existing `429350`-record dataset root, used no dataset copy, and wrote only a `48K` CSV/summary artifact. The fixed train64/val4 smoke is sharply negative: lane FP improves `11 -> 4`, but TP collapses `40 -> 29`, so lane F1 falls `0.5839 -> 0.4874`; stop-line and crosswalk are unchanged. Exact-val128 and broader-val512 are skipped. This closes retained-instance suppressor as a threshold/cap/line-ROI/context/side-contrast MLP path unless a materially different TP-preserving instance-quality signal appears.
...
Latest lane conditional denoise-row train added train-only GT-denoised conditional row queries to the seg-first lane head, so the conditional row MLP sees jittered GT bottom-anchor seed features and row targets during training while runtime still uses the existing conditional-row emit path. It trained a real CUDA `2x64` smoke on the existing canonical dataset root, indexed `429350` records, had skipped steps `0`, and created no dataset copy. It is smoke-flat/negative: fixed val4 epoch-2 best with `flip_centerline_avg_lane_cross_comp050` exactly matched the retained fixed reference at lane/stop/cross `0.5839 / 0.0000 / 0.5455`, lane TP/FP/FN `40 / 11 / 46`, stop-line `0 / 3 / 2`, and crosswalk `3 / 1 / 4`. Exact-val128, broader-val512, and larger training were skipped because the new training contract did not move runtime TP/FP/FN at the first fixed gate. Negative checkpoints and TensorBoard were pruned; retained run size is about `760K`.

Latest lane area-ROI cross-task conflict verifier added stop-line/crosswalk dense-map evidence to the dropped-candidate learned verifier and trained/evaluated it on the existing `429350`-record dataset root without copying data. It is exact-negative: train256 exact moved lane `0.5888 -> 0.5943`, TP/FP/FN `1202 / 491 / 1188 -> 1251 / 569 / 1139`; larger train512 save/load exact moved only `0.5888 -> 0.5944`, `1202 / 491 / 1188 -> 1240 / 542 / 1150`, below the previous area-ROI exact frontiers. Stop-line/crosswalk were unchanged, broader-val512 was skipped, and temporary verifier `.pt` plus root weights were pruned.

Latest current-family dense-seed geometry-prior vector decoder changed the vector-query runtime initialization more directly than the earlier dense-seed decoder: dense seed coordinates are added back as per-image shape templates before sigmoid scaling, so lane row-x, stop-line points, and crosswalk points start around the selected seed instead of only receiving seed-conditioned query features. It trained a real CUDA `2x64` smoke on the existing `429350`-record dataset root without copying data, then ran a fixed val4 epoch-2 evaluator replay. It is strongly smoke-negative: fixed val4 lane/stop/cross stayed `0.0000 / 0.0000 / 0.0000`, with TP/FP/FN lane `0 / 1 / 86`, stop-line `0 / 0 / 2`, and crosswalk `0 / 44 / 7`. Exact-val128, broader-val512, and larger training were skipped because the first fixed gate recovered no matched lane-family TP. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `8.8M`.

Latest fixed lateral-duplicate runtime smoke added one opt-in lane variant on top of the retained `flip_centerline_avg_lane_cross_comp050` path: it preserves the original lane and appends at most one `±52px` network-x shifted copy only when the shifted track is supported by the current dense centerline/support maps and is not already within the evaluator lane distance of an existing prediction. This was a one-shot check of the known nearby-track bucket, not a sweep and not a training success claim. Fixed val4 epoch-2 is negative: lane falls `0.5839 -> 0.5797`, TP/FP/FN `40 / 11 / 46 -> 40 / 12 / 46`; stop-line and crosswalk stay `0.0000` and `0.5455`. The variant added `1` duplicate candidate over `16` samples / `51` source lanes, recovered no TP, and only added FP, so exact-val128/broader-val512 are skipped. The temporary root `yolo26s.pt` download was pruned; retained artifact size is about `388K`.

Latest lane row-link offset field train changed the seg-first lane assignment contract, not a postprocess threshold: the target encoder now renders a dense per-pixel row-to-row x-delta field, the lane head predicts `lane_seg_row_link_delta`, the loss can supervise it with `lane_segfirst_row_link_aux_weight`, and the vectorizer can opt into `row_scan_row_link`. It trained real CUDA checkpoints on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, and created no dataset copy. The initial `2x64` train was negative at fixed val4: lane/stop/cross `0.5000 / 0.0000 / 0.5455`, TP/FP/FN lane `35 / 19 / 51`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`. Because the user asked to actually train and look at a larger range, a `4x512` scale audit was also run; internal phase objective rose to `0.6640`, but fixed val4 remained below the retained lane reference at lane/stop/cross `0.5180 / 0.0000 / 0.5455`, TP/FP/FN lane `36 / 17 / 50`. Exact-val128 and broader-val512 are skipped because both fixed gates lost lane TP and added FP versus retained fixed lane `0.5839`, `40 / 11 / 46`. The failed first run was removed; negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained row-link run directories are about `6.6M` and `6.7M`.

Latest lane anchor-vote instance grouping changed the lane instance grouping contract again: the target encoder renders dense per-pixel bottom-anchor x offsets, the lane head predicts `lane_seg_anchor_offset`, and runtime `row_scan_anchor_vote` groups rows by predicted anchor agreement instead of treating bottom-anchor supervision as aux-only. It trained a real CUDA `2x64` smoke checkpoint on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, had skipped steps `0`, and created no dataset copy. Fixed val4 epoch-1 best is strongly negative: lane/stop/cross `0.4000 / 0.0000 / 0.5455`, TP/FP/FN lane `28 / 26 / 58`, stop-line `0 / 3 / 2`, crosswalk `3 / 1 / 4`, versus retained fixed lane `0.5839`, `40 / 11 / 46`. Exact-val128, broader-val512, and larger training are skipped because the first fixed gate lost `12` lane TP and added `15` lane FP. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `6.6M`.

Latest lane hard-FN sample-id sampler changed the data-feeding contract using train-split lane FN evidence rather than copying data or sweeping generic lane positives. The lane FN audit now supports train-split sample identity export, produced a tracked manifest with `41` hard train samples from a CUDA train64 forward pass, and trained a real CUDA `2x64` heads-only checkpoint on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexing `429350` records in place with no dataset copy. Fixed val4 epoch-2 is negative against the same-config seed replay: seed `merged_lane_head.pt` gives lane/stop/cross `0.5507 / 0.0000 / 0.5455`, lane TP/FP/FN `38 / 14 / 48`; trained `best_lane.pt` gives `0.5455 / 0.0000 / 0.4000`, lane `36 / 10 / 50`; trained objective-best gives `0.5263 / 0.0000 / 0.4000`, lane `35 / 12 / 51`. Exact-val128, broader-val512, and larger training are skipped because fixed val4 lost lane TP and crosswalk TP. Negative checkpoints, TensorBoard, and root `yolo26s.pt` were pruned; retained run size is about `16M`.

Latest lane pairwise union-pool ranker changed the learned lane candidate-selection loss, not the candidate pool or a threshold: the area-ROI verifier now supports `verifier_loss_mode=sample_pairwise_rank`, ranking positive lane candidates above negative candidates within each sample before the existing fixed-count union geometry replay. It trained a real CUDA train64 verifier on the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexing `429350` records in place with no dataset copy. Fixed val4 was flat: baseline and pairwise replay both stayed lane/stop/cross `0.5839 / 0.0000 / 0.5455`, with lane TP/FP/FN `40 / 11 / 46`. The verifier selected `51` candidates, all retained lanes, and left `5` dropped-area oracle-positive candidates unselected, so it did not convert the known dropped-candidate headroom into TP. Exact-val128, broader-val512, and larger training are skipped because the first fixed gate had zero TP/FP/FN movement. Artifact size is about `92K`; no checkpoint was saved, and temporary root `yolo26s.pt` was pruned.
...
- latest stop-line mask-first det-negative specialist exact audit metrics: `runs/pv26_exhaustive_od_lane_train/lane60_stopline_mask_first_det_negative_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_042201/analysis_exports/source_router_exact_val128_epoch1_best/metrics.csv`
- latest stop-line utility-regression source-router train64 exact metrics: `runs/pv26_exhaustive_od_lane_train/stopline_source_router_utility_output_stats_train64_exact_val128_20260531/metrics.csv`
- latest stop-line utility-regression source-router train64 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_source_router_utility_output_stats_train64_exact_val128_20260531/summary.json`
- latest stop-line utility-regression source-router train128 exact metrics: `runs/pv26_exhaustive_od_lane_train/stopline_source_router_utility_output_stats_train128_exact_val128_20260531/metrics.csv`
- latest stop-line utility-regression source-router train128 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_source_router_utility_output_stats_train128_exact_val128_20260531/summary.json`
- latest stop-line temporal-neighbor candidate verifier smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_candidates_train64_smoke_val4_20260531/summary.json`
- latest stop-line temporal-neighbor candidate verifier exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_candidates_train64_exact_val128_20260531/summary.json`
- latest stop-line phase-correlation temporal alignment smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_phasecorr_train64_smoke_val4_20260531/summary.json`
- latest stop-line phase-correlation temporal alignment train64 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_phasecorr_train64_exact_val128_20260531/summary.json`
- latest stop-line phase-correlation temporal alignment train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_phasecorr_train256_exact_val128_20260531/summary.json`
- latest stop-line sparse-affine temporal alignment smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_sparse_affine_train64_smoke_val4_20260531/summary.json`
- latest stop-line sparse-affine temporal alignment train64 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_sparse_affine_train64_exact_val128_20260531/summary.json`
- latest stop-line sparse-affine temporal alignment train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_sparse_affine_train256_exact_val128_20260531/summary.json`
- latest stop-line sparse-affine temporal union selector smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_union_selector_sparse_affine_train64_smoke_val4_20260531/summary.json`
- latest stop-line sparse-affine temporal union selector train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_union_selector_sparse_affine_train256_exact_val128_20260531/summary.json`
- latest stop-line temporal endpoint-envelope smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_envelope_sparse_affine_train64_smoke_val4_20260531/summary.json`
- latest stop-line temporal endpoint-envelope train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_envelope_sparse_affine_train256_exact_val128_20260531/summary.json`
- latest stop-line temporal dense-component smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_dense_component_train64_smoke_val4_20260531/summary.json`
- latest stop-line temporal dense-component train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_dense_component_train256_exact_val128_20260531/summary.json`
- latest stop-line temporal dense-component feature-augmented train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_dense_component_features_train256_exact_val128_20260531/summary.json`
- latest stop-line temporal pair-consensus smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_pair_consensus_train64_smoke_val4_20260531/summary.json`
- latest stop-line temporal pair-consensus train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_pair_consensus_train256_exact_val128_20260531/summary.json`
- latest stop-line lane-homography temporal smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_lane_homography_train64_smoke_val4_20260601/summary.json`
- latest stop-line lane-homography temporal train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_temporal_lane_homography_train256_exact_val128_20260601/summary.json`
- latest stop-line crosswalk-hull edge candidate verifier smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_crosswalk_edge_candidates_train64_smoke_val4_20260531/summary.json`
- latest stop-line crosswalk-hull long-edge candidate verifier train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_crosswalk_edge_candidates_train256_exact_val128_20260531/summary.json`
- latest lane TTA-consistency union-pool verifier smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_tta_consistency_union_train64_smoke_val4_20260531/summary.json`
- latest lane TTA-consistency union-pool verifier train256 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_tta_consistency_union_train256_exact_val128_20260531/summary.json`
- latest lane temporal-neighbor verifier smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_temporal_verifier_train64_smoke_val4_20260531/summary.json`
- latest lane temporal-neighbor verifier train256 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_temporal_verifier_train256_exact_val128_20260531/summary.json`
- latest lane phase-correlation temporal alignment smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_temporal_phase_translation_train64_smoke_val4_20260531/summary.json`
- latest lane phase-correlation temporal alignment train256 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_temporal_phase_translation_train256_exact_val128_20260531/summary.json`
- latest lane area-ROI stacked-quality verifier smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_stacked_quality_train64_smoke_val4_20260531/summary.json`
- latest lane area-ROI stacked-quality verifier train256 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_stacked_quality_train256_exact_val128_20260531/summary.json`
- latest lane area-ROI stacked-quality verifier broader chunk128 summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_stacked_quality_train256_broader_val512_chunk128_20260531/summary.json`
- latest lane area-ROI stacked-quality verifier broader chunk256 summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_stacked_quality_train256_broader_val512_chunk256_20260531/summary.json`
- latest lane area-ROI stacked-quality verifier broader chunk384 summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_stacked_quality_train256_broader_val512_chunk384_20260531/summary.json`
- latest lane area-ROI stacked-quality verifier broader aggregate summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_stacked_quality_train256_broader_val512_aggregate_20260531/summary.json`
- latest lane area-ROI stacked-quality verifier broader aggregate metrics: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_stacked_quality_train256_broader_val512_aggregate_20260531/metrics.csv`
- latest lane area-ROI focal-BCE verifier smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_focal_bce_train64_smoke_val4_20260531/summary.json`
- latest lane area-ROI focal-BCE verifier train256 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_focal_bce_train256_exact_val128_20260531/summary.json`
- latest stop-line dense-map metric-quality objectness decoder smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_dense_map_metric_quality_decoder_train64_smoke_val4_20260531/summary.json`
- latest stop-line dense-map metric-quality objectness decoder train256 exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_dense_map_metric_quality_decoder_train256_exact_val128_20260531/summary.json`
...
- latest lane-family det-source confident distill fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_lane_family_det_source_confident_distill_only_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_143129/analysis_exports/fixed_val4_epoch1_best/metrics.csv`
- latest stop-line hard-mined sample-id sampler tracked manifest: `docs/manifests/stopline_hardmine_train256_20260530_sample_ids.txt`
- latest stop-line hard-mined sample-id sampler full generated manifest: `runs/pv26_exhaustive_od_lane_train/stopline_hardmine_manifest_train256_20260530/train_sample_ids.csv`
- latest stop-line hard-mined sample-id sampler fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_stopline_hardmine_static_sampler_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_235146/analysis_exports/flip_tta_val4_epoch2/metrics.csv`
- latest stop-line hard-mined sample-id sampler exact metrics: `runs/pv26_exhaustive_od_lane_train/lane60_stopline_hardmine_static_sampler_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_235146/analysis_exports/flip_tta_val128_epoch2/metrics.csv`
- latest stop-line lane-context fusion fixed exact metrics: `runs/pv26_exhaustive_od_lane_train/lane60_stopline_lane_context_fusion_static_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_231705/analysis_exports/fixed_exact_val128_epoch2_best/metrics.csv`
...
- latest co-occurrence heads-only rerun fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_cooccur_lane_stop_cross_sampler_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_203227/analysis_exports/fixed_val4_flip_comp_epoch1_best/metrics.csv`
- latest co-occurrence stop/cross lane-frozen fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_cooccur_stop_cross_lane_frozen_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_204015/analysis_exports/fixed_val4_epoch2_best/metrics.csv`
- latest fixed-alpha stop-line-head interpolation exact metrics from previous larger stop-line-priority checkpoint: `runs/pv26_exhaustive_od_lane_train/checkpoint_interpolation_stopline_priority_alpha050/exact_val128_epoch2/metrics.csv`
- latest stop-line-head interpolation train64 fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_stopline_priority_positive_sampler_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_205902/analysis_exports/stopline_head_alpha050/fixed_val4_epoch2/metrics.csv`
- latest stop-line-priority train256 raw fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_stopline_priority_positive_sampler_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_210827/analysis_exports/fixed_val4_epoch2/metrics.csv`
- latest stop-line-head interpolation train256 fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_stopline_priority_positive_sampler_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_210827/analysis_exports/stopline_head_alpha050/fixed_val4_epoch2/metrics.csv`
...
- latest current-family dense-seeded query decoder phase history: `runs/pv26_exhaustive_od_lane_train/lane60_current_family_dense_seed_vector_decoder_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_185008/phase_4/history/epochs.jsonl`
- latest current-family dense-seed + denoise vector decoder phase history: `runs/pv26_exhaustive_od_lane_train/lane60_current_family_dense_denoise_vector_decoder_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_191304/phase_4/history/epochs.jsonl`
- latest current-family teacher-runtime vector target phase history: `runs/pv26_exhaustive_od_lane_train/lane60_current_family_teacher_runtime_vector_decoder_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_193445/phase_4/history/epochs.jsonl`
- latest current-family dense-seed geometry-prior vector decoder fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_current_family_dense_seed_geometry_vector_decoder_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_222945/analysis_exports/fixed_val4_epoch2/metrics.csv`
- latest current-family self-conditioned geometry-refine vector decoder fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_current_family_dense_seed_geometry_refine_vector_decoder_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_021034/analysis_exports/fixed_val4_epoch2_best/metrics.csv`
- latest current-family anchor-query zero-residual vector decoder fixed val4 metrics:
  - epoch-1 best: `runs/pv26_exhaustive_od_lane_train/lane60_current_family_anchor_query_zerores_vector_decoder_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_084405/analysis_exports/fixed_val4_epoch1_best/metrics.csv`
  - epoch-2 last: `runs/pv26_exhaustive_od_lane_train/lane60_current_family_anchor_query_zerores_vector_decoder_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_084405/analysis_exports/fixed_val4_epoch2_last/metrics.csv`
...
- latest lane row-link offset field smoke metrics: `runs/pv26_exhaustive_od_lane_train/lane60_lane_row_link_offset_field_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_025148/analysis_exports/fixed_val4_epoch2_best/metrics.csv`
- latest lane row-link offset field scale audit metrics: `runs/pv26_exhaustive_od_lane_train/lane60_lane_row_link_offset_field_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_025621/analysis_exports/fixed_val4_epoch2_best/metrics.csv`
- latest lane anchor-vote instance grouping fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_lane_anchor_vote_instance_grouping_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_032301/analysis_exports/fixed_val4_epoch1_best/metrics.csv`
- latest lane hard-FN sample-id sampler tracked manifest: `docs/manifests/lane_hard_fn_train64_20260531_sample_ids.csv`
- latest lane hard-FN sample-id sampler seed fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_lane_head_transplant_original_stop_pca_20260512/analysis_exports/lane_hard_fn_sampler_seed_fixed_val4_epoch2/metrics.csv`
- latest lane hard-FN sample-id sampler trained fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_lane_hard_fn_sample_id_sampler_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260531_034353/analysis_exports/fixed_val4_epoch2_best_lane/metrics.csv`
...
- latest lane conditional teacher-runtime row fixed val4 metrics:
  - enabled: `runs/pv26_exhaustive_od_lane_train/lane60_lane_conditional_teacher_runtime_row_from_lane60_core_centerline_refine_cross_retain_from_exhaustive_od_lane_default_20260505_032217_default_20260510_003412_default_20260531_211934/analysis_exports/fixed_val4_condrow_enabled_epoch2/metrics.csv`
  - disabled: `runs/pv26_exhaustive_od_lane_train/lane60_lane_conditional_teacher_runtime_row_from_lane60_core_centerline_refine_cross_retain_from_exhaustive_od_lane_default_20260505_032217_default_20260510_003412_default_20260531_211934/analysis_exports/fixed_val4_condrow_disabled_epoch2/metrics.csv`
...
- latest lane conditional denoise-row fixed val4 metrics: `runs/pv26_exhaustive_od_lane_train/lane60_lane_conditional_denoise_row_from_lane60_lane_head_transplant_original_stop_pca_20260512_default_20260530_182111/analysis_exports/fixed_val4_epoch2_best/metrics.csv`
...
- latest lane repair hypothesis selector smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_repair_hypothesis_selector_train64_smoke_val4_20260530/summary.json`
- latest lane repair hypothesis selector exact summary: `runs/pv26_exhaustive_od_lane_train/lane_repair_hypothesis_selector_train64_exact_val128_20260530/summary.json`
...
- latest lane area-ROI raw-image-line train64 smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_raw_image_line_verifier_train64_smoke_val4_20260530/summary.json`
- latest lane area-ROI ensemble-stability train64 smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_ensemble_stability_train64_smoke_val4_20260530/summary.json`
- latest lane area-ROI ensemble-stability train384 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_ensemble_stability_train384_exact_val128_20260530/summary.json`
- latest lane area-ROI ensemble-stability train384 broader summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_ensemble_stability_train384_broader_val512_20260530/summary.json`
- latest lane union-pool geometry train64 smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_union_geometry_train64_smoke_val4_20260531/summary.json`
- latest lane union-pool geometry train64 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_union_geometry_train64_exact_val128_20260531/summary.json`
- latest lane union-pool geometry train256 exact summary: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_union_geometry_train256_exact_val128_20260531/summary.json`
- latest lane retained-instance suppressor smoke summary: `runs/pv26_exhaustive_od_lane_train/lane_retained_instance_suppressor_train64_smoke_val4_20260530/summary.json`
...
- latest stop-line dense-map set decoder smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_dense_map_set_decoder_train64_smoke_val4_20260531/summary.json`
- latest stop-line dense-map set decoder exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_dense_map_set_decoder_train256_exact_val128_20260531/summary.json`
...
- latest stop-line flip-consistency source-router exact train64 summary: `runs/pv26_exhaustive_od_lane_train/stopline_flip_consistency_source_router_train64_exact_val128_20260531/summary.json`
- latest stop-line flip-consistency source-router exact train256 summary: `runs/pv26_exhaustive_od_lane_train/stopline_flip_consistency_source_router_train256_exact_val128_20260531/summary.json`
- latest stop-line dense-map union selector smoke summary: `runs/pv26_exhaustive_od_lane_train/stopline_dense_map_union_selector_train64_smoke_val4_20260531_rerun/summary.json`
- latest stop-line dense-map union selector exact summary: `runs/pv26_exhaustive_od_lane_train/stopline_dense_map_union_selector_train256_exact_val128_20260531/summary.json`
...
Latest lane repair hypothesis selector probe result:

- branch/worktree: `exp/lane-family-f1/lane-repair-hypothesis-selector`.
- artifacts:
  - fixed smoke: `runs/pv26_exhaustive_od_lane_train/lane_repair_hypothesis_selector_train64_smoke_val4_20260530/summary.json`.
  - exact-val128: `runs/pv26_exhaustive_od_lane_train/lane_repair_hypothesis_selector_train64_exact_val128_20260530/summary.json`.
- changed axis: generate a fixed bank of no-GT deterministic lane repair hypotheses for each decoded lane (`translate_x`, `local_2d_snap`, `affine_2d_snap`, `component_row_project`, `row_profile_softargmax`), then train a small selector on train split labels for actual FP-to-TP conversion. Runtime selection uses only dense lane feature samples, hypothesis geometry stats, repair mode, selector probability, a fixed per-sample cap, and mean-move gates.
- storage/runtime contract: the probe reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root directly, indexed `429350` records, wrote CSV/summary artifacts only, and created no checkpoint or dataset copy. Retained output sizes are about `392K` smoke and `2.0M` exact.
- train data: `3630` train hypotheses from `64` batches, with `46` positives and `3584` negatives; feature dim `1092`.
- fixed val4: baseline lane/stop/cross `0.5839 / 0.0000 / 0.5455`, lane TP/FP/FN `40 / 11 / 46`. The val4 hypothesis bank had `0 / 255` oracle-positive hypotheses, selector selected `4 / 0` oracle-positive candidates, and metrics stayed flat.
- exact-val128: the hypothesis bank had `60 / 8465` oracle-positive hypotheses (`local_2d_snap=17`, `affine_2d_snap=18`, `component_row_project=22`, `row_profile_softargmax=2`, `translate_x=1`), but selector selection was weak: `31` selected, only `1` oracle-positive.
- exact-val128 metrics: baseline lane/stop/cross `0.5888 / 0.5333 / 0.5988`, lane TP/FP/FN `1202 / 491 / 1188`; repaired lane/stop/cross `0.5873 / 0.5333 / 0.5988`, lane TP/FP/FN `1199 / 494 / 1191`.
- 판단: deterministic no-GT repair hypotheses do contain a little exact headroom, but the learned selector does not separate positive repairs from harmful repairs and worsens TP/FP/FN. Broader-val512 was skipped. Do not repeat this as repair-mode subset, selector threshold, MLP size, epoch, train-batch count, or mean-move gate tuning without a materially new alignment/quality signal.
...
- artifacts:
  ...
  - raw-image-line train64 smoke: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_raw_image_line_verifier_train64_smoke_val4_20260530/summary.json`.
  - cross-task conflict train64 smoke: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_cross_task_conflict_train64_smoke_val4_20260530/summary.json`.
  - cross-task conflict train256 exact-val128: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_cross_task_conflict_train256_exact_val128_20260530/summary.json`.
  - cross-task conflict train512 exact-val128: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_cross_task_conflict_train512_exact_val128_loaded_20260530/summary.json`.
  - ensemble-stability train64 smoke: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_ensemble_stability_train64_smoke_val4_20260530/summary.json`.
  - ensemble-stability train384 exact-val128: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_ensemble_stability_train384_exact_val128_20260530/summary.json`.
  - ensemble-stability train384 broader-val512: `runs/pv26_exhaustive_od_lane_train/lane_area_roi_ensemble_stability_train384_broader_val512_20260530/summary.json`.
...
- storage/runtime contract: the probe reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root and created no dataset copy. The stress-audit update added chunked validation replay plus save/load for the small verifier model so val512 can be evaluated under runtime limits without holding all raw batches in memory. The ensemble-stability and cross-task-conflict updates also reused the existing `429350`-record root in place and wrote CSV/summary artifacts only after pruning temporary `.pt` files. Retained output sizes are `36K` smoke, `152K` train256 exact, `216K` train384 exact, `580K` saved-model val4, about `472K` across the four val512 chunks plus `8K` aggregate, `40K / 192K / 436K` for ensemble smoke/exact/broader, and `36K / 160K / 88K` for cross-task-conflict smoke/train256 exact/train512 exact.
...
- raw-image-line train64 smoke: baseline lane/stop/cross `0.5839 / 0.0000 / 0.5455`, lane TP/FP/FN `40 / 11 / 46`; raw-image-line append result `0.5833 / 0.0000 / 0.5455`, lane TP/FP/FN `42 / 16 / 44`, selected candidates `7`, selected oracle-positive `2`. The run trained on `439` examples (`66` positive / `373` negative), feature dim `426`, reused the existing `429350`-record dataset root, and created no checkpoint or dataset copy. Exact-val128 and broader-val512 were skipped because fixed smoke F1 regressed and the highest-score val rows mixed positives with high-confidence negatives.
- cross-task conflict train64/256/512: added stop-line/crosswalk dense-map evidence sampled along each dropped lane candidate (`stop_line_*` and `crosswalk_*` probability summaries), raising feature dim to `400`. Smoke train64/val4 moved lane `0.5839 -> 0.5915`, TP/FP/FN `40 / 11 / 46 -> 42 / 14 / 44`, selected `5`, oracle-positive `2`. Train256 exact-val128 moved only `0.5888 -> 0.5943`, `1202 / 491 / 1188 -> 1251 / 569 / 1139`, selected `127`, oracle-positive `54`. Larger train512 exact-val128 used saved/load replay to avoid long-process validation kills and moved only `0.5888 -> 0.5944`, `1202 / 491 / 1188 -> 1240 / 542 / 1150`, selected `89`, oracle-positive `42`. Stop-line/crosswalk stayed unchanged, broader-val512 was skipped because exact stayed below the existing area-ROI frontier and FP still grew faster than TP.
- ensemble-stability train64/384 smoke/exact/broader: smoke lane `0.5839 -> 0.5915`, TP/FP/FN `40 / 11 / 46 -> 42 / 14 / 44`, selected `5`, oracle-positive `2`; exact-val128 `0.5888 -> 0.5963`, `1202 / 491 / 1188 -> 1240 / 529 / 1150`, selected `76`, oracle-positive `41`; broader-val512 `0.5628 -> 0.5706`, `4532 / 2097 / 4945 -> 4690 / 2273 / 4787`, selected `334`, oracle-positive `167`. Stop-line and crosswalk were unchanged.
- union-pool geometry train64/256: added a retained-lane safety bias and fixed no-GT set-geometry support before dropped candidates can enter the fixed-count retained+dropped union. Train64 smoke was weak-positive (`0.5839 -> 0.5957`, `40 / 11 / 46 -> 42 / 13 / 44`), but train64 exact and larger train256 exact both regressed lane `0.5888 -> 0.5769`, TP/FP/FN `1202 / 491 / 1188 -> 1214 / 605 / 1176`. Stop-line/crosswalk stayed unchanged, and broader-val512 was skipped.
- 판단: the learned verifier finds real recall signal in raw dropped candidates, but FP-control remains too weak even after larger train exposure, broader chunked evaluation, nearest-retained-lane alignment-context features, dense side-contrast features, raw-image line features, cross-task conflict features, fixed-count nearest-lane replacement, ensemble-stability scoring, and fixed-count union-pool geometry support. Exact ensemble TP gain `+38` came with FP `+38` and still stayed below `0.60`; cross-task conflict train512 exact TP gain `+38` came with FP `+51`; union-pool geometry exact TP gain `+12` came with FP `+114`; broader ensemble TP gain `+158` came with FP `+176` and fell below the previous area-ROI broader `0.5821`. Lane remains below `0.60`, stop-line is unchanged, and crosswalk is below `0.60` on the stress slice. Do not repeat this as quality-threshold, hidden-dim, train-batch, max-append/max-replace, duplicate-distance, bbox-area, candidate-distance, nearest-retained-lane context, side-contrast, raw-image-line contrast, stop/cross dense-map conflict features, replace-nearest distance, verifier ensemble size, ensemble probability mode, member-seed, union source flag, retained-lane bias, set-geometry support threshold, or union fixed-count tuning without a materially new instance-quality/alignment signal that first improves fixed TP/FP/FN.

Latest lane retained-instance suppressor probe result:

- branch/worktree: `exp/lane-family-f1/current-family-dense-denoise`.
- artifact: `runs/pv26_exhaustive_od_lane_train/lane_retained_instance_suppressor_train64_smoke_val4_20260530/summary.json`.
- changed axis: train the same no-GT line-ROI verifier surface on retained row-scan lane instances, not dropped area-ROI rescue candidates, then use it suppress-only. Candidate labels are train-split matched retained lanes versus unmatched retained lanes; runtime selection uses only verifier probability, fixed threshold `0.50`, and a fixed max `2` suppressions per sample.
- storage/runtime contract: the probe reused the existing `seg_dataset/pv26_exhaustive_od_lane_dataset` root, indexed `429350` records in place, wrote CSV/summary artifacts only, and created no dataset copy. Retained output size is about `48K`; temporary root `yolo26s.pt` was removed after the run.
- train data: `726` retained-lane examples from `64` train batches, with `459` positives and `267` negatives; feature dim `420` from dense line-ROI, nearest-retained-lane context, and side-contrast features.
- fixed val4: baseline lane/stop/cross `0.5839 / 0.0000 / 0.5455`, lane TP/FP/FN `40 / 11 / 46`; suppress-only result `0.4874 / 0.0000 / 0.5455`, lane TP/FP/FN `29 / 4 / 57`; suppressed rows `18`, suppressed positives `11`.
- 판단: the retained-instance suppressor does remove FP, but it removes TP even faster. The smoke gate fails on recall collapse (`-11 TP` for `-7 FP`), so exact-val128 and broader-val512 are intentionally skipped. Do not repeat this as retained-lane verifier threshold, max-suppression cap, train-batch count, hidden-dim, line-ROI feature, nearest-lane context, or side-contrast MLP tuning without a materially different TP-preserving instance-quality signal.
