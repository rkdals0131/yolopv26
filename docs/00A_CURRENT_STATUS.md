...
- 남은 stop-line 방향은 새 query row, endpoint-delta channel, local score window, pairwise component split, selector-map gate가 아니라, mask/row/x signal을 valid line proposal로 결합하는 train-time representation과 readout이 직접 맞는 stronger geometry contract를 찾는 것이다.
