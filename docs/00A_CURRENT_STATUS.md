...
Latest implementation-only stop-line lane-homography temporal alignment adds a projective predicted-lane motion model to the temporal candidate probe. Unlike the closed lane-affine mode, it samples matched current/neighbor lane tracks, estimates a RANSAC homography, and can warp neighbor stop-line candidates through the existing homography path. This is not yet F1 evidence and must not be reported as progress toward `0.60` until a real smoke/exact run is recorded. Focused tests pass, no dataset was copied, no training checkpoint was produced, and artifact cleanup still leaves only the retained frontier checkpoints plus compact CSV/JSON evidence.
