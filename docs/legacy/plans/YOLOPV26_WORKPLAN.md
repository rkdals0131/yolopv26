...
1. `tools/data_analysis/bdd/convert_bdd_type_a.py` (변환)
2. `tools/data_analysis/bdd/validate_pv26_dataset.py` (선택)
3. `tools/data_analysis/bdd/pv26_qc_report.py` (항상)
4. `tools/debug/render_pv26_debug_masks.py` (선택)
...
### 6.1 데이터 QC (변환 직후)
- `tools/data_analysis/bdd/pv26_qc_report.py` 결과(`meta/qc_report.json`)로:
  - split별 `has_*` 분포
...
- [x] **1. 변환 재현성 확보 (지금 컨버팅 중이면 여기부터 체크)**
  ...
  - [x] 1.3 `tools/data_analysis/bdd/validate_pv26_dataset.py` 통과(권장)
