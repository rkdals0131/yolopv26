...
- Projection replay tooling and candidate-pool manifest generation are restored for reproducibility. Regenerate the replay input with `tools/probe_pv26_stopline_candidate_pool.py --dataset-root /home/kai/yolopv26/seg_dataset/pv26_exhaustive_od_lane_dataset --proposal-min-gap 4` when needed, but treat that as tooling only. It is not a new stop-line branch and does not reopen projection-competition or candidate-row feature sweeps.
