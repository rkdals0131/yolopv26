...
| --- | ---: | ---: | --- |
| lane | `0.5577` | `+0.0423` | flip-centerline current composite |
...
- Projection-competition stop-line replay can be combined with the current flip-centerline lane/crosswalk reference as an artifact-only lower bound: lane/stop-line/crosswalk F1 `0.5577 / 0.5164 / 0.6187`, mean/min F1 `0.5643 / 0.5164`. This improves task balance but still leaves lane and stop-line below `0.60`, so the next work is not another recombination of known partial positives.
...
- `exp/lane-family-f1/stopline-fragment-axis-contract` is the current model-side follow-up to the axis-offset budget, not a repeat of the closed 2D fragment extent aux sweep. Commit `6cc3f52` adds opt-in `stopline_fragment_extent_offset_loss_mode="axis"` so fragment-center supervision compares only along the stop-line axis, plus matching opt-in axis-projected fragment decode. It has passed code/runtime wiring: `75` focused tests and a CUDA smoke probe (`1` epoch, `2` train batches, `2` val batches, `skipped_steps=0`). The first exact-val128 attempt completed epoch-1 train and validation metric summarization, then failed during checkpoint save because the root filesystem was full (`df` showed `/` at `100%`, `0-45MB` available). No task F1 was persisted, so this is a storage failure and remains performance-unknown.
- lane은 row-scan/tangent-link로 broader `0.5407`, segment-MIL lane-head-only composite로 `0.5480`, flip-centerline TTA로 `0.5577`까지 올랐지만 아직 0.6에는 부족하다. GT tangent-axis read-only oracle은 exact val128 lane F1을 `0.5633 -> 0.5611`로 올리지 못했고, GT centerline-core oracle은 `0.6778`까지 올렸다. Centerline threshold audit도 best global threshold `0.5641`, sample-wise dense-core oracle `0.5482`에 그쳐 scalar calibration-only가 0.6 path가 아님을 확인했다. Instance-balanced positive core, soft-shell, soft-skeleton topology, and dense instance-embedding row-link losses produced only tiny or negative exact movement with stop-line regression. 다음 lane 축은 tangent/link cost, threshold sweep, flip max/union sweep, centerline auxiliary weight/iteration sweep, or embedding-distance/weight sweep이 아니라 더 강한 predicted centerline evidence 또는 instance-level recovery다.
...
- The area-rescue center-score gate is also closed as weak partial/negative evidence. It improved exact val128 over ungated area rescue (`0.5716 -> 0.5846`) but broader val512 lane F1 stayed at `0.5548`, below the current broader best `0.5577`, with stop-line still `0.4083`. Do not repeat it as a `min_centerline`, min-area, or max-per-sample sweep without a new FP-control signal beyond track mean.
...
- segment-MIL lane + proposal-rank stop-line task-head merge는 exact val128 objective를 `0.6248`까지 올리고 lane/stop/cross F1 `0.5660 / 0.4918 / 0.5926`을 만들었지만, broader-val512에서는 objective `0.6130`, lane/stop/cross F1 `0.5480 / 0.3976 / 0.6185`, stop-line TP/FP/FN `100 / 132 / 171`로 current broader best objective `0.6216`와 stop-line `0.4235`보다 낮다. Exact-val task-head recombination is not a broader stop-line fix.
...
- active next candidate remains `exp/lane-family-f1/stopline-fragment-axis-contract`: free checkpoint space first, then rerun exact val128 from commit `6cc3f52`, using `core_centerline_refine_stop_fragment_axis_extent`, and compare against the closed learned-fragment checkpoint plus PCA/angle-mask/projection-competition references. Do not broaden until exact stop-line improves without lane/crosswalk regression; smoke-only `val_batches=2` metrics and the disk-full exact attempt are not performance evidence.
...
- broader-val512 composite lower-bound는 row-scan tangent lane + stop-line `mask=0.80`, `min_instance_score=0.94` + crosswalk `polygon_mode=hull`을 묶어 objective `0.6165`, lane/stop/cross F1 `0.5407 / 0.4235 / 0.6187`을 만들었다. Segment-MIL lane-head-only checkpoint에 같은 stop/cross overrides를 묶으면 objective는 `0.6167526016`, lane/stop/cross F1은 `0.5480 / 0.4184 / 0.6185`가 된다. Transplanting only the segment-MIL lane head onto the original stop/cross base reaches objective `0.6176617972`, lane/stop/cross F1 `0.5480 / 0.4235 / 0.6187`; adding flip-centerline averaging on top is the cleaner current best at objective `0.6216194906`, lane/stop/cross F1 `0.5577 / 0.4235 / 0.6187`. The artifact-only projection-competition + flip-centerline task-balance replay reaches lane/stop/cross F1 `0.5577 / 0.5164 / 0.6187`, mean/min F1 `0.5643 / 0.5164`, but still leaves lane and stop-line below `0.60`.
...
- `core_centerline_refine_row_scan_tangent_segment_mil_lane_head_only`는 segment-MIL 신호를 lane head에만 적용해 stop/cross head corruption을 막는 retention schedule을 시험했다. Epoch2 exact objective는 `0.6193428422`, lane/stop/cross F1은 `0.5660 / 0.4483 / 0.5926`, lane TP/FP/FN은 `1162 / 554 / 1228`이다. Broader-val512 full-checkpoint composite replay with stop threshold + hull gives objective `0.6167526016`, lane/stop/cross F1 `0.5480 / 0.4184 / 0.6185`. Transplanting only its lane head onto the original stop/cross base is slightly cleaner at objective `0.6176617972`, and flip-centerline TTA lifts that same runtime composite to objective `0.6216194906`, lane/stop/cross F1 `0.5577 / 0.4235 / 0.6187`, but still not all-task success.
...
- `exp/lane-family-f1/stopline-projcomp-flip-composite`는 known best lane/cross partial-positive와 projection-competition stop-line readout을 합쳐도 broader task F1이 `0.5577 / 0.5164 / 0.6187`에 그친다는 artifact-only task-balance evidence로 보관한다.
...
- `exp/lane-family-f1/lane-area-rescue-center-score-gate`는 area-rescue candidate에 track centerline mean gate를 추가하면 exact val128은 `0.5846`까지 회복하지만 broader val512 lane F1 `0.5548`로 current broader best `0.5577`를 넘지 못한다는 weak partial/negative evidence로 보관한다.
...
- `exp/lane-family-f1/stopline-fragment-axis-contract` currently holds the axis-scalar fragment contract implementation. Commit `6cc3f52` changes the opt-in fragment-center offset loss from 2D xy SmoothL1 to along-axis scalar SmoothL1 and aligns fragment decode with the same axis projection. Focused tests passed (`75` tests), and the CUDA smoke probe completed with `skipped_steps=0`. The exact-val128 run `20260513_121943` is invalid/incomplete: epoch-1 validation reached `metrics_ready`, but checkpoint save failed with `torch.save` iostream error because the root filesystem was full, and no task F1 was persisted.
