...
- stop-line dense-seeded segment-set을 같은 top-K seed logits, one-shot endpoint MLP, segment-score threshold, max-segment, endpoint-loss-weight, head-LR, or longer-run sweep으로 반복하지 않는다. 2026-05-28 real 3-epoch train was stable, but exact val128 task-best stop-line was only `27 / 27 / 33 / 0.4737`, below projection-competition `31 / 29 / 29 / 0.5167`, and broader val512 task-best was lane/stop/cross `0.5419 / 0.4033 / 0.6122`, below the retained runtime composite on lane/stop-line.
...
- lane task-conflict negative loss is closed at smoke as assignment-flat. A same-budget val4 comparison between `core_centerline_refine_cross_retain` and `core_centerline_cross_conflict_negative` kept lane/stop/cross F1 exactly `0.5156 / 0.0000 / 0.7273` and TP/FP/FN unchanged, with the conflict-negative variant slightly lower on phase objective `0.6250663426 -> 0.6250358501`. Do not broaden or repeat as conflict-source/weight/margin tuning without a new assignment-moving signal.
...
- exported repairability ranker 위의 row-profile softargmax projection을 radius/window/softmax sweep으로 반복하지 않는다. Val4에서 all-selected movement와 `81` moved points가 있었지만 lane TP/FP/FN/F1은 `41 / 12 / 45 / 0.5899`로 flat이었다.
...
- Candidate-pool generation can now add `--projection-competition-replay` to write the fixed projection-competition reference in the same run. Use it as a reproduction gate only; exact val128 still shows top-oracle/misrank/no-oracle split `32 / 3 / 15` among GT-positive candidate-bearing samples, and broader val512 inline replay reproduces the known stop-line reference F1 `0.5164`, TP/FP/FN `126 / 91 / 145`. The next stop-line branch must change candidate-generation/midpoint recovery or add a materially different FP-control signal.
- Stop-line seeded segment-set is now closed in its simple top-K seed MLP form. It trained at real scale (`3` epochs, `512` train batches), but exact val128 task-best stop-line reached only `0.4737`, TP/FP/FN `27 / 27 / 33`, and broader val512 reached only `0.4033`, TP/FP/FN `97 / 113 / 174`. Reopen segment-set only with a materially different seed/verifier/objectness contract and explicit no-oracle candidate-recovery evidence.
...
- Lane repairability ranker tooling is restored in active `develop`: `tools/analyze_pv26_lane_repairability_model_replay.py` can regenerate the no-GT scorer and `repairability_model_parameters.json`, reproducing broad label AUC/AP `0.6821 / 0.7491` and top500 oracle-repair lane F1 `0.6077` from the archived broader unmatched rows. This is still only selection evidence; it must be paired with real no-GT geometry movement before any lane success claim.
- Lane repair replay tooling is restored in active `develop`: `tools/probe_pv26_lane_instance_evidence.py`, `tools/probe_pv26_lane_fn_recovery_audit.py`, and `tools/replay_pv26_lane_point_repair.py` run with focused tests, and a one-batch oracle smoke moved lane TP/FP/FN/F1 from `10 / 6 / 11 / 0.5405` to `16 / 0 / 5 / 0.8649`. This is replay machinery only, not a no-GT decoder success; use it to verify the next real lane geometry/instance-alignment branch by actual TP/FP/FN movement.
- Lane repair and ranked-repair replay tooling now accept the fixed `flip_centerline_avg_lane_cross_comp050` current-best lane variant. This is a reproduction/plumbing gate only; it does not reopen task-mask source/strength tuning and does not prove no-GT repair success.
- The restored lane FN recovery export also regenerates exact-val128 repair rows from current `develop`: baseline lane TP/FP/FN/F1 `1200 / 510 / 1190 / 0.5854`, unmatched predictions `510`, tight repairable rows `128`, broad repairable rows `322`. This is a reproduction gate only; the next lane branch must still implement no-GT repair and prove actual TP/FP/FN movement.
...
- The ranked row-profile softargmax projection follow-up is also closed flat. It selected and moved all `4` val4 repair rows (`81` moved points) using a fixed radius `16` row-wise soft centerline profile, but lane TP/FP/FN/F1 stayed exactly `41 / 12 / 45 / 0.5899`. Do not repeat row-profile projection as a radius/window/softmax sweep without a new signal that first changes assignment metrics.
...
- Lane temporal-neighbor union is closed at smoke. Immediate `sample_id` neighbors existed for all `16` val4 samples, but fixed current-centerline support plus dedupe selected `6` neighbor lane candidates, none matched a baseline FN, and lane F1 fell `0.5899 -> 0.5655`, TP/FP/FN `41 / 12 / 45 -> 41 / 18 / 45`. Do not broaden or repeat as neighbor gap, center-threshold, dedupe-distance, or add-cap tuning without a new alignment/FP-control signal.
...
- `exp/lane-family-f1/lane-attr-agnostic-duplicate-suppression-smoke`는 cross-attribute row-scan duplicate suppression도 smoke에서 `0` rows를 suppress해 lane TP/FP/FN/F1을 전혀 움직이지 않는 no-op/negative evidence로 보관한다.
