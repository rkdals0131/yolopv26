...
- `exp/lane-family-f1/stopline-fragment-axis-contract` is the current model-side follow-up to the axis-offset budget, not a repeat of the closed 2D fragment extent aux sweep. Commit `6cc3f52` adds opt-in `stopline_fragment_extent_offset_loss_mode="axis"` so fragment-center supervision compares only along the stop-line axis, plus matching opt-in axis-projected fragment decode. It has only passed code/runtime wiring so far: `75` focused tests and a CUDA smoke probe (`1` epoch, `2` train batches, `2` val batches, `skipped_steps=0`). Treat it as ready for exact val128 probing, not as performance evidence yet.
...
- active next candidate is `exp/lane-family-f1/stopline-fragment-axis-contract`: run exact val128 first from commit `6cc3f52`, using `core_centerline_refine_stop_fragment_axis_extent`, and compare against the closed learned-fragment checkpoint plus PCA/angle-mask/projection-competition references. Do not broaden until exact stop-line improves without lane/crosswalk regression; smoke-only `val_batches=2` metrics are not meaningful.
