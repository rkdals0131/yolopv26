...
- `exp/lane-family-f1/lane-repair-geometry-export` adds point JSON columns to the lane FN/unmatched repair audit. This is not a success signal, but it is the required input contract for the next real lane repair replay: use actual `pred_points_json` / `nearest_fn_gt_points_json` or live predictions to move geometry, then report TP/FP/FN. Do not fall back to aggregate-distance-only claims.
