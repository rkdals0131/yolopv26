...
- stop-line을 재개한다면 새 query proposal, endpoint-delta channel, local score-window extraction, point-pair split, simple rowx-band selector target, 단순 center/selector threshold proposal을 얹기보다 candidate pool 확대와 FP suppression을 동시에 검증하는 proposal ranking/validation contract가 필요하다.
