...
- stop-line candidate pool에는 valid segment가 있고 rich map-local feature에는 row-level signal이 있지만, score/length-only filter와 dense proposal ranking loss-only는 reference를 넘지 못했다. 다음 stop-line 축은 model-side candidate instance validator처럼 decoded geometry, selector/mask/center local evidence, per-sample competition을 함께 보는 target이거나, Gate 3 lane predicted centerline instance stability로 돌아가는 쪽이다.
