...
- Lane repairability model replay is positive planning evidence, still not production success. A 2-fold out-of-fold no-GT logistic ranker over unmatched-track/context features gives tight-label AUC/AP `0.7627 / 0.4750`; tight top-750 replay nearly reaches lane `0.60` (`0.5985`, `330` repairs) and top-1000 reaches `0.6065` only with low precision `0.3950`. The broad label is more actionable: AUC/AP `0.6810 / 0.7488`, top-500 selects `405 / 500` repairable rows and gives oracle-repair lane F1 `0.6077`, while top-1000 gives `0.6527`. The next lane branch may use this as a budgeted broad repair-ranker premise, but it must implement a real no-GT geometry/instance-alignment repair and report actual TP/FP/FN. Do not count the replay itself as success or repeat it as a threshold/top-K gate sweep.
