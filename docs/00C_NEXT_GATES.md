...
- Fragment-union readout over the current-composite candidate CSV is a partial broader positive: best `union_a12_o36_s080_c2_fallback_top` reaches stop-line F1 `0.4948`, TP/FP/FN `120 / 94 / 151`; non-fallback `union_a16_o48_s080_c2` reaches `0.4936`, TP/FP/FN `116 / 83 / 155`. It should be productionized as an opt-in model-output postprocess/evaluator replay before any training decision.
...
- partial weak-positive only. broader-val512 best stop-line F1은 fragment-union CSV replay 기준 `0.4948`이고 목표 미달이다.
