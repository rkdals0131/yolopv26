...
- Stop-line live teacher-cache distill is now executable but not yet metric evidence. Branch `exp/lane-family-f1/stopline-live-distill-smoke` adds opt-in frozen-checkpoint teacher-cache plumbing and passed a real CUDA one-batch smoke with `status ok`, `successful_batches 1`, `loss_total_mean 17.4609`. The next valid distill axis must keep this single-axis: stop-line-only distill weight, exact val128 first, broader replay only after a real stop-line metric gain.
