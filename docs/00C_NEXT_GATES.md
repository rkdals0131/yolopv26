...
- stop-line candidate pool에는 valid segment가 있고 rich map-local feature에는 row-level signal이 있지만, held-out task replay로는 PCA reference도 넘지 못했다. 다음 stop-line 축은 CSV threshold가 아니라 model-side candidate instance validator처럼 decoded geometry, selector/mask/center local evidence, per-sample competition을 학습 계약으로 묶는 target이거나, Gate 3 lane predicted centerline instance stability로 돌아가는 쪽이다.
