...
- partial weak-positive only. broader-val512 ungated stop-line reference는 projection-competition fragment-union CSV replay 기준 `0.5164`이고, val-selected single-feature selector audit best는 `0.5250`이다. Opt-in evaluator/postprocess로 재현된 fragment-union 기준은 `0.4948`이다. 모두 목표 미달이다.
