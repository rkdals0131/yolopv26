...
- `exp/lane-family-f1/lane-repair-geometry-export` adds point JSON columns to the lane FN/unmatched repair audit. Val128 confirms the export contract at scale: baseline lane TP/FP/FN/F1 `1200 / 510 / 1190 / 0.5854`, `510` unmatched predictions, `128` tight repairable rows, and `322` broad repairable rows, with point JSON populated in both unmatched-prediction and FN CSVs. This is not a success signal, but it is the required input contract for the next real lane repair replay: use actual `pred_points_json` / `nearest_fn_gt_points_json` or live predictions to move geometry, then report TP/FP/FN. Do not fall back to aggregate-distance-only claims.
