...
- stop-line candidate pool에는 valid segment가 있지만 score/length-only filter는 broader reference도 못 넘었다. 다음 stop-line 축은 candidate validator/classifier 또는 contrastive proposal ranking target처럼 FP 후보를 직접 누르는 contract여야 한다.
