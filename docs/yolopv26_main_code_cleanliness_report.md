...
`model/engine/trainer.py`도 reporting/runtime compatibility alias를 걷어내고 core trainer façade + thin compatibility shim만 남기도록 다시 좁혀졌다.
...
이후 `TeacherPredictionRow`를 shared `sweep_types.py` contract로 승격하고
`TeacherJobManifestPayload`, `SourcePrepManifest`, `FinalDatasetPublishMarker`/`FinalDatasetSourceKind`를 더해
`exhaustive_od.py`, `final_dataset.py`, `sweep.py`, `source/prepare.py`의 summary/publish/source manifest payload까지 정적 계약을 넓혔다.
...
- build/source summary row static contract는 `SourcePrepManifest`, `TeacherJobManifestPayload`, `FinalDatasetPublishMarker` 기준으로 마감
...
- `build/`는 구조는 좋고, manifest typing + summary/publish naming contract도 `SourcePrepManifest`, `TeacherJobManifestPayload`, `FinalDatasetPublishMarker` 쪽으로 마감됐다.
