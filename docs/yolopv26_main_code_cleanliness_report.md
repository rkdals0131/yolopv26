...
| `tools/od_bootstrap/source/aihub/pipeline.py` | 682 | AIHUB 전처리 orchestration monolith |
...
- `tools/od_bootstrap/source/aihub/pipeline.py`
...
`tools/pv26_train/config.py`가 이를 재사용하도록 바뀌었다.
...
`tools/pv26_train/config.py` 일부 call-site가 공용 helper를 재사용하게 됐다.
...
그 다음 wave에서는 `model/engine/det_geometry.py`, `model/engine/train_summary.py`, `model/engine/trainer_progress.py`, `model/engine/trainer_runtime.py`가 추가되면서
public/shared helper 진입점이 `loss.py`, `postprocess.py`, trainer runtime 호출부, 테스트로 확장됐다.
`model/engine/trainer.py`도 reporting/runtime compatibility alias를 걷어내고 thin compatibility shim 성격의 core trainer façade로 다시 좁혀졌다.
...
tools/
  ...
  check_env/scan.py         # workspace scan / summary load
  check_env/actions.py      # action catalog / blockers / advisory
  check_env/tui.py          # render / input / interactive loop
  check_env/launch.py       # subprocess launch / resume 실행
```
...
workspace scan은 `tools/check_env/scan.py`,
action catalog는 `tools/check_env/actions.py`,
rich rendering은 `tools/check_env/tui.py`로 분리됐다.
그 다음 wave에서는 input handling / subprocess launch / resume candidate 흐름도
`tools/check_env/launch.py`로 이동하면서 facade가 더 얇아졌다.
...
`tools/pv26_train/config.py`, `tools/pv26_train/artifacts.py`에서 함수들을 underscore alias로 많이 끌고 온다.
...
현재 `run_pv26_train.py` 내부 호출은 `pv26_train/config.py`, `pv26_train/artifacts.py`의 public module API를 기준으로 읽히고,
...
tools/
  ...
  pv26_train/scenario.py    # preset assembly + scenario/resume loading
  pv26_train/runtime.py     # phase runtime orchestration
...
즉, 겉으로는 `source/aihub/pipeline.py`와 `source/bdd100k.py`가 병렬인 형제 모듈인데,  
...
그리고 이 과정에서 `runtime/progress.py`, `runtime/tensorboard.py`, `runtime/resume.py`, `runtime_artifacts.py`의 private helper를 다수 alias로 끌어온다.
...
`runtime/progress.py`, `runtime/tensorboard.py`, `calibrate.py`가 duration/timing/tensorboard helper를 공유하게 됐다.
남은 핵심은 calibrate 흐름의 추가 축소와 runner 본체 덩치다.
후속 wave에서는 `tools/od_bootstrap/teacher/runtime/trainer.py`가 dataloader kwargs/build helper,
callback builder, resume/runtime-state helper, trainer subclass/runtime family를 받아가며
`ultralytics_runner.py`를 thin orchestration facade로 줄였다.
같은 시점에 `common/train_runtime.py`는 duration/timing/tensorboard helper에 더해
`join_status_segments()`, `progress_meter()`, `build_progress_status()`까지 맡게 됐고,
teacher/PV26 runtime은 framework-specific progress renderer만 local로 남긴 채 shared progress status helper를 재사용한다.
...
후속 wave에서는 `build/artifacts.py`, `build/sweep_types.py`, `exhaustive_od.py`,
`source/types.py`, `final_dataset.py`에 typed surface를 분산 배치해
`ImageListEntryPayload`, `RunManifestPayload`, `TeacherJobManifestPayload`, `TeacherPredictionRow`,
`SourcePrepManifest`, `FinalDatasetPublishMarker`, `FinalDatasetSourceKind`까지 source/build manifest contract에 포함시켰다.
`exhaustive_od.py`, `final_dataset.py`, `sweep.py`, `teacher/policy.py`가 이 계약을 재사용하도록 마감했다.
또 `final_dataset.py`는 publish marker/source/image row를 `Literal`/`TypedDict` 기반으로 고정해
summary/publish payload의 loose dict 면적도 줄였다.
...
- `source/shared/io.py` / `source/aihub/pipeline.py`는 hardlink/copy + existing skip 계약이다. (`source/shared/io.py`는 parent-dir 준비와 JSON read는 `common.io`를 재사용한다.)
- `build/teacher_dataset.py` / `build/final_dataset.py`는 copy_images 여부와 overwrite 금지 publish 흐름을 함께 가진다.
- `teacher/runtime/artifacts.py`는 latest alias를 refresh하기 위해 remove + relink를 허용한다.
...
그 다음 wave에서는 `check_env/scan.py`, `tools/od_bootstrap/cli.py`,
...
- `tools/od_bootstrap/teacher/runtime/progress.py`
- `tools/od_bootstrap/source/shared/io.py`
...
- `tools/od_bootstrap/source/aihub/pipeline.py: run_standardization()`
...
- `common/` + `common/train_runtime.py` 후속 policy-sensitive residue 점검 (`append_jsonl`, time helper, local renderer policy)
- `model/engine/` public/internal surface 추가 정리
- `tools/od_bootstrap/teacher/ultralytics_runner.py` bulk 축소
- policy-sensitive local surface (`raw_common.py` UTC timestamp / `calibrate.py` default=str / `final_dataset.py` overwrite publish semantics) 점검
