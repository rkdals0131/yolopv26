...
그 다음 follow-up에서는 `model/engine/det_geometry.py`, `model/engine/train_summary.py`, `model/engine/trainer_progress.py`가
public shared surface로 승격되고, `trainer.py`가 들고 있던 reporting/tensorboard/summary compatibility export는
repo 내부 사용처 기준으로 제거됐다. `loss.py`, `postprocess.py`, `_trainer_epochs.py`,
`tools/run_pv26_train.py`, 테스트가 이 public module을 직접 참조하면서 `_det_geometry.py`,
`_trainer_progress.py`는 thin compatibility shim만 남게 됐다.
즉, rank-4에서 남아 있던 engine public/internal surface 경계와 private cross-import mesh도
이 시점 기준으로는 체크리스트 레벨에서 정리됐다.
...
- teacher/runtime와 build/source 쪽 남은 summary/publish contract 고정
...
- 그 다음은 `tools/od_bootstrap/teacher/ultralytics_runner.py` bulk 축소와 build/source summary contract 마감이다.
